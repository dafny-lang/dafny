...
and then invokes the target language's compiler on those files as a subsidiary process.
...
One can also attempt to run the target prograomming language compiler manually in your system to make sure it is functional.
...
## **Error: Error: '_feature_' is not an element of the {TargetId} compiler's UnsupportedFeatures set**
...
Such programs can be verified, but in order to
be compiled to something executable, the type
must be given an actual definition.
...
The yield statement is inherently nondeterministic and so iterators in general are not permitted
along with `--enforce-determinism`.
...
If the iterator is non-ghost, it could be executed anyway.
...
To create an executable program, the compiler needs to know which is the method to call to start the program,
typically known as the 'main' method. `dafny` chooses such a method based on a few rules (cf. [details in the Dafny User Guide](#sec-user-guide-main)).

But there are restrictions (cf. [more details in the Dafny User Guide](#sec-user-guide-main)) on which methods may be used as a main entry point.
Most importantly
...
In all the files (including included ones), dafny found more than one method marked with
`{:main}`. The error location is that of one of the methods and the error refers to another.
...
In all the files (including included ones), dafny found more than one method marked with
`{:main}`. The error location is that of one of the methods and the error text refers to another.
...
In all the files (including included ones), dafny found more than one method named
`Main`. The error location is that of one of the methods and the error text refers to another.
...
It is an error if more than one is found. This error occurs if the one that is found
...
The `{:synthesize}` attribute is used to create a mock object for methods that do not have bodies.
...
## **Error: {0} '{1}' is marked as :handle, so all the traits it extends must be be marked as :handle as well: {2}**

TODO

## **Error: {0} '{1}' is marked as :handle, so all its non-static members must be ghost: {2}**
...
If the assumption is not actually valid, then the validity of the entire program is in question.
...
A method may be parsed and verified even if a loop is missing a body. Note that a missing body
is different than an empty body, which is just `{ }`.
However, the body must be supplied before the program can be compiled,
even if the method is `ghost`. Body-less loops in ghost methods are 
similar to unchecked assumptions.
...
The `--enforce-determinism` option requires a target program to be deterministic and predictable:
there may be no program statements that have an arbitrary, even if deterministic, result.
Hence this 'non-deterministic if' statement is not permitted with `--enforce-determinism`.
...
The `--enforce-determinism` option requires a target program to be deterministic and predictable:
there may be no program statements that have an arbitrary, even if deterministic, result.
The [`if` with a binding guard](../DafnyRef/DafnyRef#sec-binding-guards) checks that the 
given predicate is satisfiable by some value. If not, then the 'else' branch is executed;
but if so, the 'then' branch is executed with an arbitrary value that saitsifies the predicate.
Becuase of this arbitrary selection, the if-with-binding-guard is not permitted with `--enforce-determinism`,
...
The `--enforce-determinism` option requires a target program to be deterministic and predictable:
there may be no program statements that have an arbitrary, even if deterministic, result.
Hence this 'non-deterministic while' statement is not permitted with `--enforce-determinism`.
...
To enforce a deterministic order to the evaluation, use a series of `if` statements in which the
then-branch ends in a continue or a chain of if-then-else statements.
...
## **Error: Stubbing fields is not recommended (field {0} of object {1} inside method {2})**
...
Accordingly the attribute has an argument that names the dll and. possbly, the entry point within the dll.
