...
expressions are [well-formed](#sec-well-formedness):
<!-- %no-check -->
```dafny
a != null ==> 0 <= a.Length
0 <= a.Length <== a != null
...
```
but these expressions are not [well-formed](#sec-well-formedness), since well-formedness
requires the left (and right, respectively) operand, `a.Length < 0`,
to be [well-formed](#sec-well-formedness) by itself.
...
a == null || 0 <= a.Length
```
is [well-formed](#sec-well-formedness), whereas
<!-- %no-check -->
```dafny
...
swap statement above is [well-formed](#sec-well-formedness) only if:
<!-- %no-check -->
```dafny
...
```
is not [well-formed](#sec-well-formedness), since `Cons(5, Nil).tail` does not necessarily satisfy
`Cons?`.
...
The [well-formedness](#sec-well-formedness) of default-value expressions is checked independent
