...
value.  The body must be [well-formed](#sec-well-formedness) for all possible values of the
...
```
would not be [well-formed](#sec-well-formedness) if the `requires` clause were omitted,
