...
compiled function Apply<A,B>(f: A ~> B, x: A): B
  reads f.reads(x);
...
compiled function Apply'<A,B>(f: A ~> B) : A ~> B
{
  x reads f.reads(x)
    requires f.requires(x)
    => f(x)
...
compiled function Compose<A,B,C>(f: B ~> C, g:A ~> B): A ~> C
{
  x reads g.reads(x)
...
compiled function W<A>(f : (A,A) ~> A): A ~> A
{
...
compiled function Curry<A,B,C>(f : (A,B) ~> C) : A ~> B ~> C
{
...
compiled function Uncurry<A,B,C>(f : A ~> B ~> C) : (A,B) ~> C
{
...
compiled function S<A,B,C>(f : (A,B) ~> C, g : A ~> B): A ~> C
{
