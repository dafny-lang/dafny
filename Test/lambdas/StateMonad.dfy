...
//   type S // doesn't work because S is used left of an arrow
  ...
  lemma LeftIdentity<A,B>(x : A, f : A -> M<B>)
    ensures Bind(Return(x), f) == f(x) // return x >>= f  ==  f x
  {
  ...
  ensures Bind(Bind(m, f), g) == Bind(m, x => Bind(f(x), g))
  {
    match m case State(h) =>
      ...
      calc {
        ...
        State(s =>
          match h(s) case (a, newState) =>
            match (x => Bind(f(x), g))(a) case State(g2) => g2(newState));
        ...
        == { FunEq(
              (s => match h(s) case (a, newState) => match Bind(f(a), g) case State(g2) => g2(newState)),
              (s => match h(s) case (a, newState) => match
        ...
        State(s =>
          match h(s) case (a, newState) =>
            match f(a) case State(h2) =>
                ...
                case State(g3) => 
              g3(newState));
        == { FunEq(s => match h(s) case (a, newState) => match f(a) case State(h2) =>
                    match State(s => match h2(s) case (a2, newState2) => 
                      match g(a2) case State(g2) => g2(newState2)) case State(g3) => g3(newState),
                   s => match h(s) case (a, newState) => match f(a) case State(h2) =>
                      (s => match h2(s) case (a2, newState2) => match g(a2) case State(g2) =>
