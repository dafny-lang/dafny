...
module Functions {
  newtype Newtype = x | 0 <= x < 500 {
    compiled function InstanceN<Bn(0)>(b: Bn): Bn { b }
    static compiled function StaticN<Bn(0)>(b: Bn): Bn { b }
  }

  datatype Datatype<Ad(0)> = Dt0 | Dt1 {
    compiled function InstanceD<Bd(0)>(a: Ad, b: Bd): (Ad, Bd) { (a, b) }
    static compiled function StaticD<Bd(0)>(a: Ad, b: Bd): (Ad, Bd) { (a, b) }
  }

  trait UberTrait<Au(0)> {
    compiled function InstanceU0<Bu(0)>(a: Au, b: Bu): (Au, Bu) { (a, b) }
    compiled function InstanceU1<Bu(0)>(a: Au, b: Bu): (Au, Bu)
    compiled function InstanceU2<Bu(0)>(a: Au, b: Bu): (Au, Bu)
    compiled function InstanceU3<Bu(0)>(a: Au, b: Bu): (Au, Bu)
  }

  trait InBetween<Ai(0)> extends UberTrait<Ai> {
    compiled function InstanceU1<Bi(0)>(a: Ai, b: Bi): (Ai, Bi) { (a, b) }
  }

  trait Trait<At(0)> extends InBetween<At> {
    compiled function InstanceU2<Bt(0)>(a: At, b: Bt): (At, Bt) { (a, b) }
    compiled function InstanceT<Bt(0)>(a: At, b: Bt): (At, Bt)
    compiled function InstanceTBody<Bt(0)>(a: At, b: Bt): (At, Bt) { (a, b) }
    static compiled function StaticT<Bt(0)>(a: At, b: Bt): (At, Bt) { (a, b) }
  }

  class Class<Ac(0)> extends Trait<Ac> {
    compiled function InstanceU3<Bc(0)>(a: Ac, b: Bc): (Ac, Bc) { (a, b) }
    compiled function InstanceT<Bc(0)>(a: Ac, b: Bc): (Ac, Bc) { (a, b) }
    compiled function InstanceC<Bc(0)>(a: Ac, b: Bc): (Ac, Bc) { (a, b) }
    static compiled function StaticC<Bc(0)>(a: Ac, b: Bc): (Ac, Bc) { (a, b) }
  }
...
module FunctionsAsValues {
  trait Trait<G(0), H, J(0)> {
    compiled function Select(g: G, h: H, j: J): G { g }

    compiled function ApplySelect(g: G, h: H, j: J): G {
      var f := Select;
      f(g, h, j)
    }
  }
