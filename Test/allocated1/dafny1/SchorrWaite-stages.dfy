// RUN: %dafny /allocated:1 /compile:0 /dprint:"%t.dprint" "%s" > "%t"
// RUN: %diff "%s.expect" "%t"
...
abstract module M0 {
    ...
    decreases p
  {
    ...
    case Extend(prefix, n) => n in S && allocated(n) && sink in n.children && ReachableVia(source, prefix, n, S)
  }

  method SchorrWaite(root: Node, ghost S: set<Node>)
    requires root in S
    // S is closed under 'children':
    requires forall n :: n in S ==> n != null && allocated(n) &&
    ...
    decreases *  // leave termination checking for a later refinement
  {
    ...
    while true
      ...
      invariant forall n :: n in stackNodes ==> allocated(n) && n.childrenVisited < |n.children|
