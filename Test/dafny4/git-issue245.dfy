...
trait T {
  ...
  compiled function d(x: nat): nat
    ensures d(x) < 10 + x
  // f names the result in both trait and class
  compiled function f(x: nat): (res: nat)
    ensures res < 10 + x
  // g names the result in just the class
  compiled function g(x: nat): nat
    ensures g(x) < 10 + x
  // h names the result in just the trait
  compiled function h(x: nat): (res: nat)
    ensures res < 10 + x
}

class C extends T {
  // d does not name the result in either trait or class
  compiled function d(y: nat): nat
    ensures d(y) < 8 + y
  { 5 + y }
  // f names the result in both trait and class
  compiled function f(y: nat): (result: nat)
    ensures result < 8 + y
  { 5 + y }
  // g names the result in just the class
  compiled function g(y: nat): (result: nat)
    ensures result < 8 + y
  { 5 + y }
  // h names the result in just the trait
  compiled function h(y: nat): nat
    ensures h(y) < 8 + y
  { 5 + y }
}
...
class F extends T {
  // d does not name the result in either trait or class
  compiled function d(y: nat): nat  // error: postcondition violation
    ensures d(y) < 8 + y
  { 20 + y }
  // f names the result in both trait and class
  compiled function f(y: nat): (result: nat)  // error: postcondition violation
    ensures result < 8 + y
  { 20 + y }
  // g names the result in just the class
  compiled function g(y: nat): (result: nat)  // error: postcondition violation
    ensures result < 8 + y
  { 20 + y }
  // h names the result in just the trait
  compiled function h(y: nat): nat  // error: postcondition violation
    ensures h(y) < 8 + y
  { 20 + y }
}
