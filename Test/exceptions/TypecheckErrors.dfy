...
trait BadOutcome1 {
    // compiled predicate IsFailure() // <-- deliberately commented out
    // compiled function PropagateFailure(): BadOutcome1 requires IsFailure() // <-- deliberately commented out
    // compiled function Extract(): nat requires !IsFailure() // <-- deliberately commented out
}

trait BadOutcome2 {
    compiled predicate IsFailure()
    // compiled function PropagateFailure(): BadOutcome2 requires IsFailure() // <-- deliberately commented out
    compiled function Extract(): nat requires !IsFailure()
}

trait BadOutcome3 {
    compiled predicate IsFailure()
    compiled function PropagateFailure(): BadOutcome3 requires IsFailure()
    // compiled function Extract(): nat requires !IsFailure() // <-- deliberately commented out
}
...
trait BadVoidOutcome1 {
    // compiled predicate IsFailure() // <-- deliberately commented out
    // compiled function PropagateFailure(): BadVoidOutcome1 requires IsFailure() // <-- deliberately commented out
}

trait BadVoidOutcome2 {
    compiled predicate IsFailure()
    // compiled function PropagateFailure(): BadVoidOutcome2 requires IsFailure() // <-- deliberately commented out
}

trait BadVoidOutcome3 {
    compiled predicate IsFailure()
    compiled function PropagateFailure(): BadVoidOutcome3 requires IsFailure()
    compiled function Extract(): nat requires !IsFailure() // <-- deliberately added, even though Void error handling must not have it
}
