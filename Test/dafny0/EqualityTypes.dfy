...
module H {
  ...
  method M<T(==)>(x: T)
  { }
  compiled function F<T>(x: BulkyList<T>, y: BulkyList<T>): int
  { if x == y then 5 else 7 }  // this equality is allowed
  compiled function G<T>(x: Dt<T>, y: Dt<T>): int
...
module Gh {
  ...
  compiled function F(b: bool, ghost g: bool): int { 6 }
...
module Deep {
  ...
  compiled function G<Y(==)>(y: Y, z: Y): int { if y == z then 5 else 7 }
...
module EqualitySupportingTypes {
  method P<T>()
  compiled function G<T>(): int
  class AClass<V(==),Y> {
    static compiled function H<W,X(==)>(): bool
    static method Q<A,B(==)>()
  }

  method Callee<T(==)>()
  compiled function FCallee<T>(): T
