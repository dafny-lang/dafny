...
method Main()
{
  ...
  PrintSet(q);
  q := set b: bool :: if b then 3 else 7;
  var d := set b:D;
  var test := forall d:D {:nowarn} :: d == A || d == B; // Ignoring the warning as we're only compiling here
  PrintSet(q);
  var m := set k <- q :: 2*k;
  PrintSet(m);
  PrintSet(set k <- q | k % 2 == 0);
  var sq := [30, 40, 20];
  PrintSet(set k <- sq, i | 0 <= i < k && i % 7 == 0 :: k + i);
  var bb := forall k <- sq, i {:nowarn} | 0 <= i < k && i % 7 == 0 :: k + i == 17; // Ignoring the warning as we're only compiling here
}
...
method SetComprehensionBoxAntecedents()
{
  ...
  case true =>
    assert a == b;
  case true =>
    assert forall x <- a :: x in b;
    assert forall x <- b :: x in a;
  case true =>
