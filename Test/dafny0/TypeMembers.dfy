...
datatype DaTy = Yes {
  compiled function W(): int { 10 }
  static compiled function Q(): int { 13 }
}

newtype Pos = x | 0 < x witness 1
{
  static compiled function Func(y: Pos): int { (y + 3) as int }
  compiled function Gittit(): int { (this + 2) as int }
...
datatype Dt<A> = Blue | Bucket(diameter: real) | Business(trendy: bool, a: A)
{
  ...
  static compiled function G(x: int): int {
    2 * x
  }
...
codatatype Co<A> = Cobalt | Continues(next: Co<A>)
{
  ...
  static const g: int
  compiled function F(x: int): int { 19 }
  static compiled function G(x: int): int { x + 12 }
...
newtype Primes = x | 2 <= x && forall y :: 2 <= y < x ==> x % y != 0 witness 2
{
  ...
  compiled function F(x: int): int { 2 * x + this as int }
  static compiled function G(x: int): int { 100 - x }
