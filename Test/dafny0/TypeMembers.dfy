...
datatype DaTy = Yes {
  function W(): int { 10 }
  static ghost function Q(): int { 13 }
}

newtype Pos = x | 0 < x witness 1
{
  static ghost function Func(y: Pos): int { (y + 3) as int }
...
datatype Dt<A> = Blue | Bucket(diameter: real) | Business(trendy: bool, a: A)
{
  ...
  static ghost function G(x: int): int {
    2 * x
  }
...
codatatype Co<A> = Cobalt | Continues(next: Co<A>)
{
  ...
  static ghost function G(x: int): int { x + 12 }
...
newtype Primes = x | 2 <= x && forall y :: 2 <= y < x ==> x % y != 0 witness 2
{
  ...
  static ghost function G(x: int): int { 100 - x }
