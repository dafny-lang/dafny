...
module Actuals {
  compiled function F(x: int, y: int, z: int): int
...
module Formals {
  class C<GG> {
    ...
    compiled function Generic0<X>(): int
    compiled function Generic1<X>(p: X): int

    method T0(x: int := u + this.u + v + this.v, y: int := if this != null then 0 else 2)
    compiled function T1(x: int := u + this.u + v + this.v, y: int := if this != null then 0 else 2): int
...
module InfiniteExpansion {
  compiled function C(n: nat := C()): nat // error: default-value expression has infinite expansion
  ...
  compiled function InfiniteLoopFunction(n: nat := InfiniteLoopFunction()): nat // error: InfiniteLoopFunction used in its own default value
...
module Modes {
  ...
  compiled function FG(ghost u: int): int
  compiled function FC(u: int): int
  ...
  compiled function MG2(      x: int, y: int := FG(x), ghost z: int := FC(x), w: int := FC(x)): int
  compiled function MG3(ghost x: int, y: int := FG(x), ghost z: int := FC(x), w: int := FC(x)): int // error: call to FC passes in a ghost expression
...
module Overriding {
  trait Trait {
    compiled function M(x: int := 5): int
    compiled function N(x: int := 5): int
    compiled function O(x: int): int
  }
  class Class extends Trait {
    compiled function M(x: int := 6): int { 2 * x } // it's fine to change default-value expressions
    compiled function N(x: int): int { 2 * x } // it's fine to drop a default-value expression
    compiled function O(x: int := 6): int { 2 * x } // it's fine to add a default-value expression
  }
...
module RefinementA {
  ...
  compiled function FF(x: int := 5): (r: int)
    ensures r == x
...
module RefinementB refines RefinementA {
  compiled function M(x: int := 6): int { 2 * x } // error: refinement module cannot indicate default value on refined parameter
  compiled function N(x: int): int { 2 * x }
  compiled function O(x: int := 6): int { 2 * x } // error: refinement module cannot indicate default value on refined parameter
  ...
  compiled function FF(x: int := 6): int { x } // error: refinement module cannot indicate default value on refined parameter
...
module TwoState {
  class C {
    ...
    compiled function F(u: int := old(x)): int // error: old not allowed in this context
...
module NameOnlyParameters {
  class C {
    ...
    compiled function F(a: int, nameonly b: int, c: int := 100): int
