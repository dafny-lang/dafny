...
pub mod _module {
    ...
    impl _default {
        /// rust-stdlib-smoke-test.dfy(4,1)
        pub fn Main(_noArgsParameter: &Sequence<Sequence<DafnyChar>>) -> () {
            let mut opt: Rc<Option<DafnyInt>> = Rc::new(Option::Some::<DafnyInt> {
                        value: int!(42)
                    });
            let mut _source0: Rc<Option<DafnyInt>> = opt.clone();
            if matches!((&_source0).as_ref(), None{ .. }) {
                print!("{}", DafnyPrintWrapper(&string_of("Option is empty\n")))
            } else {
                let mut ___mcc_h0: DafnyInt = _source0.value().clone();
                let mut value: DafnyInt = ___mcc_h0.clone();
                print!("{}", DafnyPrintWrapper(&string_of("Option contains: ")));
            ...
            let mut _source1: Rc<Result<DafnyInt, Sequence<DafnyChar>>> = result.clone();
            if matches!((&_source1).as_ref(), Success{ .. }) {
                let mut ___mcc_h1: DafnyInt = _source1.value().clone();
                let mut value: DafnyInt = ___mcc_h1.clone();
                print!("{}", DafnyPrintWrapper(&string_of("Result success: ")));
                print!("{}", DafnyPrintWrapper(&value));
                print!("{}", DafnyPrintWrapper(&string_of("\n")))
            } else {
                let mut ___mcc_h2: Sequence<DafnyChar> = _source1.error().clone();
                let mut error: Sequence<DafnyChar> = ___mcc_h2.clone();
                print!("{}", DafnyPrintWrapper(&string_of("Result failure: ")));
            ...
            let _x = crate::FileIOInternalExterns::_default::_INTERNAL_ReadBytesFromFile(&string_of("/local/home/mimayere/dafny2/rust-stdlib-smoke-test.dfy"));
            _out0 = _x.0;
            _out1 = _x.1;
            _out2 = _x.2;
            isError = _out0;
            ...
            if !isError {
                print!("{}", DafnyPrintWrapper(&string_of("FileIO read successful, got ")));
                print!("{}", DafnyPrintWrapper(&bytesRead.cardinality()));
                print!("{}", DafnyPrintWrapper(&string_of(" bytes\n")))
            } else {
                print!("{}", DafnyPrintWrapper(&string_of("FileIO read failed: ")));
                print!("{}", DafnyPrintWrapper(&errorMsg));
                print!("{}", DafnyPrintWrapper(&string_of("\n")))
            };
            return ();
        }
