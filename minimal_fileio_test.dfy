// Minimal FileIO test that bypasses standard library issues
      ...
      decreases *
    {
      ...
      for i := 0 to |content| {
        if content[i] as int <= 255 {
          ...
          // Handle non-ASCII characters by encoding as UTF-8 (simplified)
          bytes := bytes + [63 as bv8]; // '?' character for unsupported chars
        }
      }
