...
namespace DafnyTests {
  ...
  public class LitTestConvertor {
    ...
    private static readonly string[] DAFNY_OUTPUT_FLAGS = {
      "print",
      "dprint",
      "rprint",
    };
    ...
    private static Dictionary<string, string> ParseDafnyCommandArguments(string dafnyCommand) {
      ...
      IEnumerable<string> arguments = RemoveInputFileAndOutput(dafnyCommand.Substring(LIT_DAFNY.Length));
      
      // Check the arguments for anything non-standard
      var testConfig = new Dictionary<string, string>();
      foreach (var argument in arguments) {
        KeyValuePair<string, string> pair = ParseDafnyArgument(argument);
        if (!(DAFNY_OUTPUT_FLAGS.Contains(pair.Key) && pair.Value.Contains("%t"))) {
    ...
    private static IEnumerable<string> RemoveInputFileAndOutput(string line) {
      var arguments = line.Split((char[])null, StringSplitOptions.RemoveEmptyEntries).ToList();
      
      // Ensure the last two parts are > "%t" or >> "%t"
      if (arguments.Count < 3) {
        throw new ArgumentException("Not enough arguments to %dafny command: " + line);
      }
      if (!arguments[^1].Equals("\"%t\"") 
          || !(arguments[^2].Equals(">") || arguments[^2].Equals(">>")))
      {
        throw new ArgumentException("Non-standard output in %dafny command: " + line);
      }
      arguments.RemoveRange(arguments.Count - 2, 2);

      if (!arguments.Remove("\"%s\"")) {
        throw new ArgumentException("Test file (\"%s\") does not appear as input in %dafny command: " + line);
      }
