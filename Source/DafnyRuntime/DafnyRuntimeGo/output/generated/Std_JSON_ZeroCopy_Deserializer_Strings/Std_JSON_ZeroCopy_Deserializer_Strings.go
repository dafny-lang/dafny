...
func (_static *CompanionStruct_Default___) StringBody(cs Std_JSON_Utils_Cursors.Cursor__) Std_Wrappers.Result {
  ...
  _ = pr
  var _622_escaped bool
  _ = _622_escaped
  _622_escaped = false
  var _hi3 uint32 = (cs).Dtor_end()
  _ = _hi3
  for _623_point_k := (cs).Dtor_point(); _623_point_k < _hi3; _623_point_k++ {
    var _624_byte uint8
    _ = _624_byte
    _624_byte = ((cs).Dtor_s()).Select(uint32(_623_point_k)).(uint8)
    if (((_624_byte) == (uint8(_dafny.CodePoint('"')))) && (!(_622_escaped))) {
      pr = Std_Wrappers.Companion_Result_.Create_Success_(Std_JSON_Utils_Cursors.Companion_Cursor___.Create_Cursor_((cs).Dtor_s(), (cs).Dtor_beg(), _623_point_k, (cs).Dtor_end()))
      return pr
    } else if ((_624_byte) == (uint8(_dafny.CodePoint('\\')))) {
...
func (_static *CompanionStruct_Default___) Quote(cs Std_JSON_Utils_Cursors.Cursor__) Std_Wrappers.Result {
  var _625_valueOrError0 Std_Wrappers.Result = (cs).AssertChar(_dafny.CodePoint('"'))
  _ = _625_valueOrError0
  if ((_625_valueOrError0).IsFailure()) {
    return (_625_valueOrError0).PropagateFailure()
  } else {
    var _626_cs Std_JSON_Utils_Cursors.Cursor__ = (_625_valueOrError0).Extract().(Std_JSON_Utils_Cursors.Cursor__)
    _ = _626_cs
    return Std_Wrappers.Companion_Result_.Create_Success_((_626_cs).Split())
  }
}
func (_static *CompanionStruct_Default___) String(cs Std_JSON_Utils_Cursors.Cursor__) Std_Wrappers.Result {
  var _627_valueOrError0 Std_Wrappers.Result = Companion_Default___.Quote(cs)
  _ = _627_valueOrError0
  if ((_627_valueOrError0).IsFailure()) {
    return (_627_valueOrError0).PropagateFailure()
  } else {
    var _let_tmp_rhs24 Std_JSON_Utils_Cursors.Split = (_627_valueOrError0).Extract().(Std_JSON_Utils_Cursors.Split)
    _ = _let_tmp_rhs24
    var _628_lq Std_JSON_Utils_Views_Core.View__ = _let_tmp_rhs24.Get_().(Std_JSON_Utils_Cursors.Split_SP).T.(Std_JSON_Utils_Views_Core.View__)
    _ = _628_lq
    var _629_cs Std_JSON_Utils_Cursors.Cursor__ = _let_tmp_rhs24.Get_().(Std_JSON_Utils_Cursors.Split_SP).Cs
    _ = _629_cs
    var _630_valueOrError1 Std_Wrappers.Result = Companion_Default___.StringBody(_629_cs)
    _ = _630_valueOrError1
    if ((_630_valueOrError1).IsFailure()) {
      return (_630_valueOrError1).PropagateFailure()
    } else {
      var _631_contents Std_JSON_Utils_Cursors.Cursor__ = (_630_valueOrError1).Extract().(Std_JSON_Utils_Cursors.Cursor__)
      _ = _631_contents
      var _let_tmp_rhs25 Std_JSON_Utils_Cursors.Split = (_631_contents).Split()
      _ = _let_tmp_rhs25
      var _632_contents Std_JSON_Utils_Views_Core.View__ = _let_tmp_rhs25.Get_().(Std_JSON_Utils_Cursors.Split_SP).T.(Std_JSON_Utils_Views_Core.View__)
      _ = _632_contents
      var _633_cs Std_JSON_Utils_Cursors.Cursor__ = _let_tmp_rhs25.Get_().(Std_JSON_Utils_Cursors.Split_SP).Cs
      _ = _633_cs
      var _634_valueOrError2 Std_Wrappers.Result = Companion_Default___.Quote(_633_cs)
      _ = _634_valueOrError2
      if ((_634_valueOrError2).IsFailure()) {
        return (_634_valueOrError2).PropagateFailure()
      } else {
        var _let_tmp_rhs26 Std_JSON_Utils_Cursors.Split = (_634_valueOrError2).Extract().(Std_JSON_Utils_Cursors.Split)
        _ = _let_tmp_rhs26
        var _635_rq Std_JSON_Utils_Views_Core.View__ = _let_tmp_rhs26.Get_().(Std_JSON_Utils_Cursors.Split_SP).T.(Std_JSON_Utils_Views_Core.View__)
        _ = _635_rq
        var _636_cs Std_JSON_Utils_Cursors.Cursor__ = _let_tmp_rhs26.Get_().(Std_JSON_Utils_Cursors.Split_SP).Cs
        _ = _636_cs
        return Std_Wrappers.Companion_Result_.Create_Success_(Std_JSON_Utils_Cursors.Companion_Split_.Create_SP_(Std_JSON_Grammar.Companion_Jstring_.Create_JString_(_628_lq, _632_contents, _635_rq), _636_cs))
      }
    }
  }
}
