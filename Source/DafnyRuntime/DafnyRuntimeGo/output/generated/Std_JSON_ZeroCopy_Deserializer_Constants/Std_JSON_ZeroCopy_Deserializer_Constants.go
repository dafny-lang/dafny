...
func (_static *CompanionStruct_Default___) Constant(cs Std_JSON_Utils_Cursors.Cursor__, expected _dafny.Sequence) Std_Wrappers.Result {
  var _731_valueOrError0 Std_Wrappers.Result = (cs).AssertBytes(expected, uint32(0))
  _ = _731_valueOrError0
  if ((_731_valueOrError0).IsFailure()) {
    return (_731_valueOrError0).PropagateFailure()
  } else {
    var _732_cs Std_JSON_Utils_Cursors.Cursor__ = (_731_valueOrError0).Extract().(Std_JSON_Utils_Cursors.Cursor__)
    _ = _732_cs
    return Std_Wrappers.Companion_Result_.Create_Success_((_732_cs).Split())
  }
}
