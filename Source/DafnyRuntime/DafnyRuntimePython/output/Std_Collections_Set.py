...
class default__:
    ...
    def Map(f, xs):
        def iife5_():
            ...
            for compr_4_ in (xs).Elements:
                d_111_x_: TypeVar('X__') = compr_4_
                if (d_111_x_) in (xs):
                    coll4_ = coll4_.union(_dafny.Set([f(d_111_x_)]))
    ...
    def Filter(f, xs):
        def iife6_():
            ...
            for compr_5_ in (xs).Elements:
                d_113_x_: TypeVar('X__') = compr_5_
                if ((d_113_x_) in (xs)) and (f(d_113_x_)):
                    coll5_ = coll5_.union(_dafny.Set([d_113_x_]))
    ...
    @staticmethod
    def SetRange(a, b):
        d_114___accumulator_ = _dafny.Set({})
        while True:
            with _dafny.label():
                if (a) == (b):
                    return (_dafny.Set({})) | (d_114___accumulator_)
                elif True:
                    d_114___accumulator_ = (d_114___accumulator_) | (_dafny.Set({a}))
                    in30_ = (a) + (1)
                    in31_ = b
                    a = in30_
                    b = in31_
