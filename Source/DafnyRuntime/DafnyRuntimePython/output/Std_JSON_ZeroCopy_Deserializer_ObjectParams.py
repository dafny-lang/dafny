...
class default__:
    ...
    @staticmethod
    def Colon(cs):
        d_655_valueOrError0_ = (cs).AssertChar(_dafny.CodePoint(':'))
        if (d_655_valueOrError0_).IsFailure():
            return (d_655_valueOrError0_).PropagateFailure()
        elif True:
            d_656_cs_ = (d_655_valueOrError0_).Extract()
            return Std_Wrappers.Result_Success((d_656_cs_).Split())

    @staticmethod
    def KeyValueFromParts(k, colon, v):
        d_657_sp_ = Std_JSON_Utils_Cursors.Split_SP(Std_JSON_Grammar.jKeyValue_KeyValue((k).t, (colon).t, (v).t), (v).cs)
        return d_657_sp_
    ...
    @staticmethod
    def Element(cs, json):
        d_658_valueOrError0_ = Std_JSON_ZeroCopy_Deserializer_Strings.default__.String(cs)
        if (d_658_valueOrError0_).IsFailure():
            return (d_658_valueOrError0_).PropagateFailure()
        elif True:
            d_659_k_ = (d_658_valueOrError0_).Extract()
            d_660_p_ = default__.Colon
            d_661_valueOrError1_ = Std_JSON_ZeroCopy_Deserializer_Core.default__.Structural((d_659_k_).cs, d_660_p_)
            if (d_661_valueOrError1_).IsFailure():
                return (d_661_valueOrError1_).PropagateFailure()
            elif True:
                d_662_colon_ = (d_661_valueOrError1_).Extract()
                d_663_valueOrError2_ = (json)((d_662_colon_).cs)
                if (d_663_valueOrError2_).IsFailure():
                    return (d_663_valueOrError2_).PropagateFailure()
                elif True:
                    d_664_v_ = (d_663_valueOrError2_).Extract()
                    d_665_kv_ = default__.KeyValueFromParts(d_659_k_, d_662_colon_, d_664_v_)
                    return Std_Wrappers.Result_Success(d_665_kv_)
