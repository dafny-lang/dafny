...
class default__:
    ...
    @staticmethod
    def Merge(lv, rv):
        d_343_dt__update__tmp_h0_ = lv
        d_344_dt__update_hend_h0_ = (rv).end
        return View___View((d_343_dt__update__tmp_h0_).s, (d_343_dt__update__tmp_h0_).beg, d_344_dt__update_hend_h0_)
...
class View__:
    ...
    @staticmethod
    def OfString(s):
        return _dafny.SeqWithoutIsStrInference([ord((s)[d_345_i_]) for d_345_i_ in range(len(s))])
    ...
    def CopyTo(self, dest, start):
        hi0_ = (self).Length()
        for d_346_idx_ in range(0, hi0_):
            index6_ = (start) + (d_346_idx_)
            (dest)[index6_] = ((self).s)[((self).beg) + (d_346_idx_)]
