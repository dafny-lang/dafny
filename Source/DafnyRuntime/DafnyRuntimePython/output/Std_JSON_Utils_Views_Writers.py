...
class Chain:
    ...
    def Length(self):
        d_347___accumulator_ = 0
        _this = self
        while True:
            with _dafny.label():
                if (_this).is_Empty:
                    return (0) + (d_347___accumulator_)
                elif True:
                    d_347___accumulator_ = (((_this).v).Length()) + (d_347___accumulator_)
    ...
    def Count(self):
        d_348___accumulator_ = 0
        _this = self
        while True:
            with _dafny.label():
                if (_this).is_Empty:
                    return (0) + (d_348___accumulator_)
                elif True:
                    d_348___accumulator_ = (1) + (d_348___accumulator_)
    ...
    def Bytes(self):
        d_349___accumulator_ = _dafny.SeqWithoutIsStrInference([])
        _this = self
        while True:
            with _dafny.label():
                if (_this).is_Empty:
                    return (_dafny.SeqWithoutIsStrInference([])) + (d_349___accumulator_)
                elif True:
                    d_349___accumulator_ = (((_this).v).Bytes()) + (d_349___accumulator_)
    ...
    def CopyTo(self, dest, end):
        ...
        while True:
            with _dafny.label():
                if (_this).is_Chain:
                    d_350_end_: int
                    d_350_end_ = (end) - (((_this).v).Length())
                    ((_this).v).CopyTo(dest, d_350_end_)
