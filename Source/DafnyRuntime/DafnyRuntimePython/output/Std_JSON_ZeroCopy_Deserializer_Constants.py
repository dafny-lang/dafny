...
class default__:
    ...
    def Constant(cs, expected):
        d_722_valueOrError0_ = (cs).AssertBytes(expected, 0)
        if (d_722_valueOrError0_).IsFailure():
            return (d_722_valueOrError0_).PropagateFailure()
        elif True:
            d_723_cs_ = (d_722_valueOrError0_).Extract()
            return Std_Wrappers.Result_Success((d_723_cs_).Split())
