...
namespace Dafny
{
  ...
  public interface IMap<out U, out V>
  {
    ...
    IMap<UU, VV> DowncastClone<UU, VV>();
  }
  ...
  public class Map<U, V> : IMap<U, V>
  {
    ...
    public IMap<UU, VV> DowncastClone<UU, VV>() {
      if (this is IMap<UU, VV> th) {
        return th;
      } else {
        var d = ImmutableDictionary<UU, VV>.Empty.ToBuilder();
        foreach (var item in this.dict) {
          var k = (UU)(object)item.Key;
          var v = (VV)(object)item.Value;
          d.Add(k, v);
        }
