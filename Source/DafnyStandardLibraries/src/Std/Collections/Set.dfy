...
module Std.Collections.Set {
  ...
  function Map<X(!new), Y>(f: X --> Y, xs: set<X>): (ys: set<Y>)
    reads f.reads
    requires forall x {:trigger f.requires(x)} :: f.requires(x)
    requires Injective(f)
  ...
  true. */
  function Filter<X(!new)>(f: X ~> bool, xs: set<X>): (ys: set<X>)
  ...
  function SetRange(a: int, b: int): (s: set<int>)
    requires a <= b
  ...
  function SetRangeZeroBound(n: int): (s: set<int>)
    requires n >= 0
