...
module Std.Collections.Map {
  ...
  opaque function ToImap<X, Y>(m: map<X, Y>): (m': imap<X, Y>)
  ...
  opaque function RemoveKeys<X, Y>(m: map<X, Y>, xs: set<X>): (m': map<X, Y>)
  ...
  opaque function Remove<X, Y>(m: map<X, Y>, x: X): (m': map<X, Y>)
  ...
  opaque function Restrict<X, Y>(m: map<X, Y>, xs: set<X>): (m': map<X, Y>)
  ...
  opaque function Union<X, Y>(m: map<X, Y>, m': map<X, Y>): (r: map<X, Y>)
  ...
  opaque ghost predicate Injective<X, Y>(m: map<X, Y>)
  {
  ...
  opaque ghost function Invert<X, Y>(m: map<X, Y>): map<Y, X>
  {
  ...
  opaque ghost predicate Total<X(!new), Y>(m: map<X, Y>)
  {
    forall i {:trigger m[i]}{:trigger i in m} :: i in m
  }

  /* True iff a map is monotonic. */
  opaque ghost predicate Monotonic(m: map<int, int>)
  {
  ...
  equal to start. */
  opaque ghost predicate MonotonicFrom(m: map<int, int>, start: int)
  {
