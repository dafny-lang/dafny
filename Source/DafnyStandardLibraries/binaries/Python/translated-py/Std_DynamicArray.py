...
class DynamicArray:
    ...
    def Ensure(self, reserved, defaultValue):
        d_105_newCapacity_: int
        d_105_newCapacity_ = self.capacity
        while (reserved) > ((d_105_newCapacity_) - (self.size)):
            d_105_newCapacity_ = (self).DefaultNewCapacity(d_105_newCapacity_)
        if (d_105_newCapacity_) > (self.capacity):
            (self).Realloc(defaultValue, d_105_newCapacity_)
    ...
    def Realloc(self, defaultValue, newCapacity):
        d_106_oldData_: _dafny.Array
        d_107_oldCapacity_: int
        rhs2_ = self.data
        rhs3_ = self.capacity
        d_106_oldData_ = rhs2_
        d_107_oldCapacity_ = rhs3_
        def lambda5_(d_108_defaultValue_):
            def lambda6_(d_109___v0_):
                return d_108_defaultValue_

            return lambda6_

        init3_ = lambda5_(defaultValue)
        ...
        (self).CopyFrom(d_106_oldData_, d_107_oldCapacity_)
    ...
    def CopyFrom(self, newData, count):
        ...
        for guard_loop_0_ in _dafny.IntegerRange(0, count):
            d_110_index_: int = guard_loop_0_
            if (True) and (((0) <= (d_110_index_)) and ((d_110_index_) < (count))):
                arr2_ = self.data
                arr2_[(d_110_index_)] = (newData)[d_110_index_]
