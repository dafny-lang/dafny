    ...
    def ToImap(m):
        def iife1_():
            ...
            for compr_1_ in (m).keys.Elements:
                d_95_x_: TypeVar('X__') = compr_1_
                if (d_95_x_) in (m):
                    coll1_[d_95_x_] = (m)[d_95_x_]
    ...
    def Remove(m, x):
        def iife2_():
            ...
            for compr_2_ in (m).keys.Elements:
                d_97_x_k_: TypeVar('X__') = compr_2_
                if ((d_97_x_k_) in (m)) and ((d_97_x_k_) != (x)):
                    coll2_[d_97_x_k_] = (m)[d_97_x_k_]
    ...
    def Restrict(m, xs):
        def iife3_():
            ...
            for compr_3_ in (xs).Elements:
                d_98_x_: TypeVar('X__') = compr_3_
                if ((d_98_x_) in (xs)) and ((d_98_x_) in (m)):
                    coll3_[d_98_x_] = (m)[d_98_x_]
