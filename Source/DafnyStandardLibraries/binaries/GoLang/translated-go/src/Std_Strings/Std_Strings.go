...
func (_static *CompanionStruct_Default___) Join(sep _dafny.Sequence, strs _dafny.Sequence) _dafny.Sequence {
  var _161___accumulator _dafny.Sequence = _dafny.SeqOf()
  _ = _161___accumulator
  goto TAIL_CALL_START
  TAIL_CALL_START:
  if ((_dafny.IntOfUint32((strs).Cardinality())).Sign() == 0) {
    return _dafny.Companion_Sequence_.Concatenate(_161___accumulator, _dafny.UnicodeSeqOfUtf8Bytes(""))
  } else if ((_dafny.IntOfUint32((strs).Cardinality())).Cmp(_dafny.One) == 0) {
    return _dafny.Companion_Sequence_.Concatenate(_161___accumulator, (strs).Select(0).(_dafny.Sequence))
  } else {
    _161___accumulator = _dafny.Companion_Sequence_.Concatenate(_161___accumulator, _dafny.Companion_Sequence_.Concatenate((strs).Select(0).(_dafny.Sequence), sep))
    var _in58 _dafny.Sequence = sep
    _ = _in58
    var _in59 _dafny.Sequence = (strs).Drop(1)
    _ = _in59
    sep = _in58
    strs = _in59
    goto TAIL_CALL_START
...
func (_static *CompanionStruct_Default___) Concat(strs _dafny.Sequence) _dafny.Sequence {
  var _162___accumulator _dafny.Sequence = _dafny.SeqOf()
  _ = _162___accumulator
  goto TAIL_CALL_START
  TAIL_CALL_START:
  if ((_dafny.IntOfUint32((strs).Cardinality())).Sign() == 0) {
    return _dafny.Companion_Sequence_.Concatenate(_162___accumulator, _dafny.UnicodeSeqOfUtf8Bytes(""))
  } else {
    _162___accumulator = _dafny.Companion_Sequence_.Concatenate(_162___accumulator, (strs).Select(0).(_dafny.Sequence))
    var _in60 _dafny.Sequence = (strs).Drop(1)
    _ = _in60
    strs = _in60
    goto TAIL_CALL_START
  }
