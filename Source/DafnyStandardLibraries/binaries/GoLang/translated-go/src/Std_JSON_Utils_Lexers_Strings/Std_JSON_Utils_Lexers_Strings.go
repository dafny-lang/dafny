...
func (_static *CompanionStruct_Default___) String(st StringLexerState, byte_ int16) Std_JSON_Utils_Lexers_Core.LexerResult {
  ...
  if (_source13.Is_Start()) {
    ...
    var _352___mcc_h0 bool = _source13.Get_().(StringLexerState_Body).Escaped
    _ = _352___mcc_h0
    var _353_escaped bool = _352___mcc_h0
    _ = _353_escaped
    if ((byte_) == (int16(_dafny.CodePoint('\\')))) {
      return Std_JSON_Utils_Lexers_Core.Companion_LexerResult_.Create_Partial_(Companion_StringLexerState_.Create_Body_(!(_353_escaped)))
    } else if (((byte_) == (int16(_dafny.CodePoint('"')))) && (!(_353_escaped))) {
