...
/// *****************************************************************************
/// Source/DafnyStandardLibraries/examples/StandaloneFileIO.dfy(7,1)
pub mod StandaloneFileIO {
    pub use ::dafny_runtime::Sequence;
    pub use ::dafny_runtime::DafnyChar;
    pub use ::dafny_runtime::string_of;
    pub use ::std::rc::Rc;
    pub use crate::StandaloneFileIO::Result::Success;
    ...
    impl _default {
        pub fn ReadBytesFromFile(
            ...
            crate::StandaloneFileIO_externs::_default::ReadBytesFromFile(path)
        }

        /// Source/DafnyStandardLibraries/examples/StandaloneFileIO.dfy(16,3)
        pub fn Main(_noArgsParameter: &Sequence<Sequence<DafnyChar>>) -> () {
            let mut filename: Sequence<DafnyChar> = string_of("Source/DafnyStandardLibraries/examples/StandaloneFileIO.dfy");
            ...
            if matches!((&_source0).as_ref(), Success{ .. }) {
                ...
                print!("{}", DafnyPrintWrapper(&string_of("Standalone FileIO test: PASSED\n")))
            } else {
                ...
                print!("{}", DafnyPrintWrapper(&string_of("Standalone FileIO test: FAILED\n")))
            };
            return ();
        }
    }

    /// Source/DafnyStandardLibraries/examples/StandaloneFileIO.dfy(10,3)
    #[derive(Clone)]
        ...
        fn fmt_print(&self, _formatter: &mut Formatter, _in_seq: bool) -> std::fmt::Result {
            match self {
                Result::Success{value, } => {
                    write!(_formatter, "StandaloneFileIO.Result.Success(")?;
                    DafnyPrint::fmt_print(value, _formatter, false)?;
                    write!(_formatter, ")")?;
                    Ok(())
                },
                Result::Failure{error, } => {
                    write!(_formatter, "StandaloneFileIO.Result.Failure(")?;
...
fn main() {
  ...
  crate::StandaloneFileIO::_default::Main(&dafny_args);
}