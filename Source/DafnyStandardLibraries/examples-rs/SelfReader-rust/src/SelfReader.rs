...
/// *****************************************************************************
/// Source/DafnyStandardLibraries/examples/SelfReader.dfy(7,1)
pub mod SelfReader {
    ...
    impl _default {
        /// Source/DafnyStandardLibraries/examples/SelfReader.dfy(11,3)
        pub fn Main(_noArgsParameter: &Sequence<Sequence<DafnyChar>>) -> () {
            let mut filename: Sequence<DafnyChar> = string_of("Source/DafnyStandardLibraries/examples/SelfReader.dfy");
            ...
            if matches!((&_source0).as_ref(), Success{ .. }) {
                ...
                let mut contentStr: Sequence<DafnyChar> = {
                        let _initializer = {
                                let content: Sequence<u8> = content.clone();
                                {
                                    let mut content = content.clone();
                                    Rc::new(move |i: &DafnyInt| -> DafnyChar{
            DafnyChar(content.get(i) as char)
        }) as Rc<dyn ::std::ops::Fn(&_) -> _>
                                }
                            };
                        integer_range(Zero::zero(), content.cardinality()).map(move |i| _initializer(&i)).collect::<Sequence<_>>()
                    };
                let mut lines: Sequence<Sequence<DafnyChar>>;
                let mut _out1: Sequence<Sequence<DafnyChar>> = _default::SplitLines(&contentStr);
                lines = _out1.clone();
                print!("{}", DafnyPrintWrapper(&string_of("First few lines of this file:\n")));
                let mut linesToShow: DafnyInt;
                if lines.cardinality() < int!(10) {
                    linesToShow = lines.cardinality();
                } else {
                    linesToShow = int!(10);
                };
                let mut _hi0: DafnyInt = linesToShow.clone();
                for i in integer_range(int!(0), _hi0.clone()) {
                    print!("{}", DafnyPrintWrapper(&string_of("  ")));
                    print!("{}", DafnyPrintWrapper(&lines.get(&i)));
                    print!("{}", DafnyPrintWrapper(&string_of("\n")))
                }
                print!("{}", DafnyPrintWrapper(&string_of("This program successfully read itself using Rust FileIO!\n")))
            } else {
        ...
        /// Source/DafnyStandardLibraries/examples/SelfReader.dfy(39,3)
        pub fn SplitLines(content: &Sequence<DafnyChar>) -> Sequence<Sequence<DafnyChar>> {
            let mut lines: Sequence<Sequence<DafnyChar>> = seq![] as Sequence<Sequence<DafnyChar>>;
            let mut currentLine: Sequence<DafnyChar> = string_of("");
            let mut _hi0: DafnyInt = content.cardinality();
            for i in integer_range(int!(0), _hi0.clone()) {
                if content.get(&i) == DafnyChar(char::from_u32(10).unwrap()) {
                    lines = lines.concat(&seq![currentLine.clone()]);
                    currentLine = string_of("");
                } else {
                    currentLine = currentLine.concat(&seq![content.get(&i)]);
                }
            }
            if int!(0) < currentLine.cardinality() {
                lines = lines.concat(&seq![currentLine.clone()]);
            };
            return lines.clone();
        }
...
fn main() {
  ...
  crate::SelfReader::_default::Main(&dafny_args);
}