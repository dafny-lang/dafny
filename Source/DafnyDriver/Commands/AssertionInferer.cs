...
public static class AssertionInferer {

  static AssertionInferer() {
  ...
  public static Command Create() {
    var result = new Command("assertInferer", "Infer position of missing assertions.");
  ...
  public static async Task<int> HandleVerification(DafnyOptions options) {
    ...
    if (resolution != null) {
      ...
      InferAssertionsCreated(compilation, resolution, verificationResults);
  ...
  public static void InferAssertionsCreated(CliCompilation compilation, ResolutionResult resolution,
    IObservable<CanVerifyResult> verificationResults) {
    verificationResults.Subscribe(result => {
      // We use an intermediate reporter so we can sort the diagnostics from all parts by token
      var failedTasks = result.Results
        .Where(r => r.Result.Outcome !=  SolverOutcome.Valid)
        .ToList();
      if (failedTasks.Count == 0) {
        return;
      }  
      // Print a summary for the ICanVerify symbol
      var canVerify = result.CanVerify;
      var location = canVerify.NavigationToken.GetFilePosition();
      var locationString = $"{location.Uri.LocalPath}:{location.Position.Line}:{location.Position.ToString()}";
      var name = canVerify.FullDafnyName;
      compilation.Options.OutputWriter.WriteLine($"Verification failed in {name} at {locationString}:");

      // Previous gives information of lemma/function etc 

      // Need to find how to get more informations and use ast to moodify the methods(tricky)
    }); 
  }
