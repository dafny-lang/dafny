  ...
  SyntaxContainer CompileRBranch(MatchTempInfo mti, List<Expression> matchees, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        ...
        List<Statement> insideBranch;
        if(insideContainer is SStmt){
          insideBranch = new List<Statement>();
          insideBranch.Add(((SStmt)insideContainer).Body);
        }else if (insideContainer is SStmts){
          insideBranch = ((SStmts)insideContainer).Body;
        }else { //is SExpr
          insideBranch = new List<Statement>();
        }
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  if (rb is SStmt){
    ...
    for(int id = 0; id < mti.BranchIDCount.Length; id++){
      ...
      // rb should be a StmtContainer, this is unreachable
