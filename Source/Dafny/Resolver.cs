...
BlockStmt BlockStmtOfStmtContainer(IToken tok, IToken endTok, StmtContainer con){
  if (con is CStmts){
    var stmts = ((CStmts)con).Body;
    return new BlockStmt(tok, endTok, stmts);
  } else if (con is CStmt){
    var stmt = ((CStmt)con).Body;
...
// Assumes that all SyntaxContainer are of the same subclass
...
//              List<BoundVar> freshBV = ctor.Value.Formals.ConvertAll(x => new BoundVar(new AutoGeneratedToken(x.Tok), FreshTempVarName("_mcc#", codeContext), SubstType(x.Type, subst)));
      ...
      if(mti.Debug) Console.WriteLine("{0}===[3**]=== Base type? {1}", new String('\t',mti.DebugLevel), currMatcheeType.ToString());
      ...
      return MakeIfFromContainers(mti, currMatchee, currBlocks, defaultBlock);
