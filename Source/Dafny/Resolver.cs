...
public class MatchTempInfo{
  ...
  // Record the number of copies of each branch
  public int[] BranchIDCount;

  // true if we are desugaring a MatchStmt, false if a MatchExpr
  public bool isStmt;
 ...
 public MatchTempInfo(IToken tok, bool isstmt,  int branchidnum){
...
public abstract class StmtContainer:SyntaxContainer{
...
public class CStmt:StmtContainer{
...
public class CStmts:StmtContainer{
  public List<Statement> Body;

  public CStmts(List<Statement> body){
    this.Body = body;
  }
}
...
SyntaxContainer PackBody(RBranch branch){
  if(branch is RBranchStmt){
    return new CStmts(((RBranchStmt)branch).Body);
  } else if (branch is RBranchExpr) {
    return new CExpr(((RBranchExpr)branch).Body);
  } else {
     throw new System.ArgumentException("unexpected subclass of RBranch");
  }
}
...
List<Statement> UnboxStmtContainer(SyntaxContainer con){
  if(con is CStmt){
    ...
    return r;
  } else if (con is CStmts){
    return ((CStmts)con).Body;
  } else {
    throw new System.ArgumentException("expected a StmtContainer");
  }
}
...
void LetBindNonWildCard(RBranch branch, CasePattern<BoundVar> cp, Expression expr){
 ...
 MatchCase MakeMatchCaseFromContainer(IToken tok,KeyValuePair<string, DatatypeCtor> ctor,List<BoundVar> freshPatBV,SyntaxContainer insideContainer){
   ...
   if(insideContainer is StmtContainer){
...
string CasePatternToString(CasePattern<BoundVar> pat, ICodeContext context){
  ...
  if (pat.Arguments != null) {
    List<string> cps = pat.Arguments.ConvertAll(x => CasePatternToString(x, context));
    s+= string.Format(", ({0})",String.Join(",", cps));
  } else {
    s += ", null args";
  }  
...
/// 3* - if head-matchee is a tuple (should only happen at top level), create one level of matching at the arity of the tuple, break down the casepattern and recur
/// 3 - if some of the head-patterns are constructors, create one level of matching at the type of the head-matchee, 
...
/// </summary>
  SyntaxContainer CompileRBranch(MatchTempInfo mti, List<Expression> matchees, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(matchees.Count == 0){
      ...
      return PackBody(branches.First());
    }
    ...
    if(dtd is TupleTypeDecl){
      if(mti.Debug) Console.WriteLine("{0}===[3*]=== Tuple Case of arity {1}", new String('\t',mti.DebugLevel), dtd.TypeArgs.Count);
      
      // Prepare fresh variables to match inside the tuple
      List<BoundVar> freshPatBV = currMatcheeType.TypeArgs.ConvertAll(x => new BoundVar(new AutoGeneratedToken(currMatchee.tok), FreshTempVarName("_mcc#", codeContext), PartiallyResolveTypeForMemberSelection(currMatchee.tok, x).NormalizeExpand() as UserDefinedType));
      ...
      var insideContainer = CompileRBranch(mti, cmatchees, currBranches, codeContext);
      // Get the right constructor for tuples (copied from ResolveMatchStmt)
      var tuple = (TupleTypeDecl)dtd;
      var dims = tuple.Dims;
      var ctorId = BuiltIns.TupleTypeCtorNamePrefix + dims;
      DatatypeCtor ctor = null;
      if (!ctors.TryGetValue(ctorId, out ctor)){
        reporter.Error(MessageSource.Resolver, currMatchee.tok, "Tuples constructor {0} has not been found", ctorId);

      } 
      MatchCase newMatchCase = MakeMatchCaseFromContainer(new AutoGeneratedToken(currMatchee.tok), new KeyValuePair<string, DatatypeCtor>("", ctor), freshPatBV,insideContainer);
      var newMatchCases = new List<MatchCase>();
      newMatchCases.Insert(0, newMatchCase);
      // Generate and pack the right kind of Match
      if(mti.Debug){          
        Console.WriteLine("{0}return on tuple", new String('\t',mti.DebugLevel));
        mti.DebugLevel--;
      } 
      if(mti.isStmt){
        var newMatchStmt = new MatchStmt(new AutoGeneratedToken(currMatchee.tok), new AutoGeneratedToken(currMatchee.tok), currMatchee, newMatchCases.ConvertAll(x => (MatchCaseStmt) x), true);
        return new CStmt(newMatchStmt);
      } else {
        var newMatchExpr = new MatchExpr(new AutoGeneratedToken(currMatchee.tok), currMatchee, newMatchCases.ConvertAll(x => (MatchCaseExpr) x), true);
    ...
    // ==[3]== If some of the patternHeads are constructor, create a new match with a branch for each constructor
      ...
      foreach(var ctor in ctors){
        ...
        if(insideContainer is null){
          ...
          MatchCase newMatchCase = MakeMatchCaseFromContainer(new AutoGeneratedToken(currMatchee.tok), ctor, freshPatBV,insideContainer);
          newMatchCases.Insert(0, newMatchCase);
        }
      }
      // Generate and pack the right kind of Match
...
void CompileMatchExpr(MatchExpr e, ICodeContext codeContext){
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  for(int id = 0; id < e.Cases.Count(); id++){
    ...
    branches.Add(RBranchOfMatchCaseExpr(id, branch));
  ...
  if (rb is null) {
  ...
  } else if (rb is CExpr && ((CExpr)rb).Body is MatchExpr){
    // replace e with desugared match
    var newME = (MatchExpr)((CExpr)rb).Body;
    e.UpdateSource(newME.Source);
    e.UpdateCases(newME.Cases);
    ...
    // this comes up if we reach the (1) no-matchee case without performing any (3) mixed-case
    // there wasn't any work to do (i.e. just variable renaming, leave e as is)
    reporter.Warning(MessageSource.Resolver, mti.Tok, "MatchExpr without any refinements");
  } else {
...
/// On output, the MatchStmt has been flattened and all of its patterns are non-overlapping
/// </summary>
void CompileMatchStmt(MatchStmt s, ICodeContext codeContext) {
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  if (rb is null) {
    ...
    // replace s with desugared match
    var newMS = (MatchStmt)((CStmt)rb).Body;
    ...
    //there wasn't any work to do (i.e. just variable renaming, leave s as is)
    reporter.Warning(MessageSource.Resolver, mti.Tok, "MatchStmt without any refinements");
  } else {
...
void CheckLinearMatchCase(Type type, MatchCase mc, bool debug = false){
  ...
  if (type.AsDatatype is TupleTypeDecl) {
      ...
      if(udt.TypeArgs.Count != mc.CasePatterns.Count) reporter.Error(MessageSource.Resolver, mc.tok, "<In Linear> case arguments count does not match source arguments count");

      var pairTP = udt.TypeArgs.Zip(mc.CasePatterns, (x,y) => new Tuple<Type,CasePattern<BoundVar>>(x,y));

      foreach (var tp in pairTP) {
        var t = PartiallyResolveTypeForMemberSelection(mc.tok, tp.Item1).NormalizeExpand() as UserDefinedType;
        if(debug) Console.WriteLine("{0} is a tuple field of type {1}", tp.Item2.Id, t);
        CheckLinearPattern(t, tp.Item2);
      }
  } else {
