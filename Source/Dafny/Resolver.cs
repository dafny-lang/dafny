...
void LetBind(RBranch branch, BoundVar var, Expression expr){
  ...
  List<Statement> CompileInnerMatchStmt(List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(matchees.Count == 0){
      ...
      if (top) {
        ...
        } else { Console.WriteLine("DEBUG: (CompileInnerMatchStmt) natchees is empty, NOT top -> base case");
        }
    ...
    // TODO: need a different way to differenciate constructors and non-constructors, since Ctor is not filled in until resolution
    // e.g.: if Id is some Ctor in ctors, then constructor, otherwise BV
    if(patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        ...
        List<BoundVar> freshPatBV = ctor.Value.Formals.ConvertAll(x => new BoundVar(new AutoGeneratedToken(x.Tok), FreshTempVarName("_mcc#", codeContext), x.Type));
        ...
        foreach(var PB in pairPB){
          if(PB.Item1.Var != null){
            ...
            List<BoundVar> freshBV = ctor.Value.Formals.ConvertAll(x => new BoundVar(new AutoGeneratedToken(x.Tok), FreshTempVarName("_mcc#", codeContext), x.Type));
            ...
            LetBind(currBranch,PB.Item1.Var, rhsExpr);
            currBranches.Add(currBranch);
          } else if(ctor.Equals(PB.Item1.Ctor)){
        ...
        List<IdentifierExpr> freshMatchees = freshPatBV.ConvertAll(x => new IdentifierExpr(new AutoGeneratedToken(x.tok), x));
      ...
      foreach(Tuple<CasePattern<BoundVar>, RBranch> PB in pairPB){
          ...
          LetBind(PB.Item2, PB.Item1.Var, currMatchee);
      }
      r = CompileInnerMatchStmt(matchees.Skip(1).ToList(), false, pairPB.ToList().ConvertAll(new Converter<Tuple<CasePattern<BoundVar>,RBranch>, RBranch>(x => x.Item2)), codeContext);
    } 
    return r;
  }
