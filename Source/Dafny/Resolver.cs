...
//            else match one level of constructors, continue
  RBranch CompileRBranch(MatchTempInfo mti, List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(branches.Count == 0){
      ...
      return new RBranchStmt(-1, new List<CasePattern<BoundVar>>(), new List<Statement>());
    }
    if(matchees.Count == 0){
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        ...
        List<Statement> insideBranch = ((RBranchStmt)CompileRBranch(mti, cmatchees,false, currBranches, codeContext)).Body;
      ...
      return new RBranchStmt(-1, new List<CasePattern<BoundVar>>(), newBody);
    } else {
...
void CompileMatchCaseStmt(MatchStmt s, ICodeContext codeContext) {
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  } else if (rb is RBranchStmt){
    // replace s with desugared match
    var r = ((RBranchStmt)rb).Body;
    var newS = r.First();
