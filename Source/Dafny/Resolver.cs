...
#region CheckDividedConstructorInit
    ...
    // TODO is there already a library of convenience AST construction methods like this one?
    ...
    private Statement DesugarAssignOrReturnStmt(AssignOrReturnStmt s, ICodeContext codeContext) {
      // TODO how to mark this as autogenerated? Should we create an equivalent of AutoGeneratedExpression for statements?
      // TODO how to pass the token positions? Can we do better than placing all the code at s.Tok?
      var temp = FreshTempVarName("valueOrError", codeContext);
      return new BlockStmt(s.Tok, s.EndTok, new List<Statement> {
        // var temp := MethodOrExpression;
        new VarDeclStmt(s.Tok, s.Tok, new List<LocalVariable>() { new LocalVariable(s.Tok, s.Tok, temp, new InferredTypeProxy(), false) },
        ...
        new IfStmt(s.Tok, s.Tok, false, VarDotMethod(s.Tok, temp, "IsFailure"),
          ...
          new BlockStmt(s.Tok, s.Tok, new List<Statement>() { // TODO is this wrapping inside a BlockStmt needed? I just mimick what the parser does to cause no surprises
        ...
        // y := temp.Extract();
        new UpdateStmt(s.Tok, s.Tok, s.Lhss /*<--TODO do we need to clone this?*/, new List<AssignmentRhs>() {
    ...
    private void ResolveAssignOrReturnStmt(AssignOrReturnStmt s, ICodeContext codeContext) {
      s.ResolvedStmt = DesugarAssignOrReturnStmt(s, codeContext);
