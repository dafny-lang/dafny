...
//            else match one level of constructors, continue
  List<Statement> CompileInnerMatchStmt(MatchTempInfo mti, List<Expression> matchees, bool top, List<RBranchStmt> branches, ICodeContext codeContext) {
    ...
    var patternHeads = branches.ConvertAll(new Converter<RBranchStmt,CasePattern<BoundVar>>(getPatternHead));
    var newBranches = branches.ConvertAll(new Converter<RBranchStmt,RBranchStmt>(x => (RBranchStmt) (dropPatternHead(x))));
    var pairPB = patternHeads.Zip(newBranches, (x,y) => new Tuple<CasePattern<BoundVar>,RBranchStmt>(x, y));
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        var currBranches = new List<RBranchStmt>();
        ...
        foreach(var PB in pairPB){
          if(ctor.Key.Equals(PB.Item1.Id)){
            ...
            var currBranch = CloneRBranchStmt(PB.Item2);
      ...
      foreach(Tuple<CasePattern<BoundVar>, RBranchStmt> PB in pairPB){
          LetBind(PB.Item2, PB.Item1, currMatchee);
      }
      r = CompileInnerMatchStmt(mti, matchees, top, pairPB.ToList().ConvertAll(new Converter<Tuple<CasePattern<BoundVar>,RBranchStmt>, RBranchStmt>(x => x.Item2)), codeContext);
    } 
    return r;
  }
...
void CompileMatchCaseStmt(MatchStmt s, ICodeContext codeContext) {
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  List<RBranchStmt> branches = new List<RBranchStmt>();
