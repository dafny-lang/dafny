...
//            else match one level of constructors, continue
  List<Statement> CompileInnerMatchStmt(IToken tok, List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(branches.Count == 0){
      ...
      reporter.Warning(MessageSource.Resolver, tok, "non-exhaustive case-statement");
      return r;
    }
    if(matchees.Count == 0){
      ...
      if (top) {
        reporter.Warning(MessageSource.Resolver, tok, "pattern-match without any refinements");
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        ...
        List<Statement> insideBranch = CompileInnerMatchStmt(tok, cmatchees,false, currBranches, codeContext);
      ...
      r = CompileInnerMatchStmt(tok, matchees, top, pairPB.ToList().ConvertAll(new Converter<Tuple<CasePattern<BoundVar>,RBranch>, RBranch>(x => x.Item2)), codeContext);
    } 
    return r;
  }
...
void CompileMatchCaseStmt(MatchStmt s, ICodeContext codeContext) {
  ...
  var r = CompileInnerMatchStmt(s.Tok, matchees, true, branches, codeContext);
