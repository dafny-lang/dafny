...
//            else match one level of constructors, continue
  List<Statement> CompileRBranch(MatchTempInfo mti, List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(matchees.Count == 0){
      ...
      if (top) {
        ...
        return ((RBranchStmt)branches.First()).Body;
      }
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        ...
        List<Statement> insideBranch = CompileRBranch(mti, cmatchees,false, currBranches, codeContext);
      ...
      r = CompileRBranch(mti, matchees, top, pairPB.ToList().ConvertAll(new Converter<Tuple<CasePattern<BoundVar>,RBranch>, RBranch>(x => x.Item2)), codeContext);
    } 
    return r;
  }
...
void CompileMatchCaseStmt(MatchStmt s, ICodeContext codeContext) {
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  var r = CompileRBranch(mti, matchees, true, branches, codeContext);
