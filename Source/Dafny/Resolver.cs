...
#region CheckDividedConstructorInit
    ...
    void ResolveMatchStmt(MatchStmt s, ICodeContext codeContext) {
      ...
      if (dtd != null) {
        CompileMatchCaseStmt(s, codeContext);
...
/* Sigma is not needed since can bind eagerly on top of Body */
public class RBranch {
  public List<CasePattern<BoundVar>>  Patterns;
  /* public List<Tuple<BoundVar,Expression>> Sigma;  */
  public List<Statement> Body;

  public RBranch(){}
  public RBranch(List<CasePattern<BoundVar>> patterns,  List<Statement> body){
...
public static RBranch CloneRBrach(RBranch branch){
  AutoGeneratedTokenCloner cloner = new AutoGeneratedTokenCloner();
  return new RBranch(branch.Patterns.ConvertAll(x => cloner.CloneCasePattern(x)), branch.Body.ConvertAll(x=> cloner.CloneStmt(x)));
}
...
public static Tuple<CasePattern<BoundVar>,RBranch> popPattern(RBranch branch){
  // clone branch, then remove its first pattern
  var newBranch = new RBranch(branch.Patterns, branch.Body);
  newBranch.Patterns.RemoveAt(0);
  return Tuple.Create(branch.Patterns.FirstOrDefault(), newBranch);
}

// Let-bind the substitution in branch.sigma in reverse order on body
// At the moment, assumes cp is a boundvar
/* public static List<Statement> LetBindSigma(RBranch branch){
  AutoGeneratedTokenCloner cloner = new AutoGeneratedTokenCloner();
  var newBody = branch.Body.ConvertAll(cloner.CloneStmt);
  for(int i=0; i < branch.Sigma.Count; i++){
    var cSub = branch.Sigma[i];
    var cLVar = new LocalVariable(cSub.Item1.tok, cSub.Item1.tok, cSub.Item1.Name, cSub.Item1.Type, false);
    var cPat = new CasePattern<LocalVariable>(cLVar.EndTok, cLVar); 
    var cLet = new LetStmt(cSub.Item1.tok, cSub.Item1.tok, cPat, cSub.Item2);
    newBody.Add(cLet);
  }
  return newBody;
} */
void LetBind(RBranch branch, CasePattern<BoundVar> cp, Expression expr){
   BoundVar var = null;
   if (cp.Var == null){
    //Console.WriteLine("DEBUG: (LetBind) null var with id:{0} ", cp.Id);
    Type type = expr.Type; 
    var = new BoundVar(new AutoGeneratedToken(cp.tok), cp.Id, type);
   }else{
     var = cp.Var;
   }
    //Console.WriteLine("DEBUG: (LetBind) tok: {0}, name:{1}, type:{2}", var.tok == null? "null": var.tok.ToString(), var.Name == null? "null":var.Name.ToString(), var.Type == null? "null":var.Type.ToString());
    var cLVar = new LocalVariable(var.tok, var.tok, var.Name, var.Type, false);
    var cPat = new CasePattern<LocalVariable>(cLVar.EndTok, cLVar); 
    var cLet = new LetStmt(var.tok, var.tok, cPat, expr);
    branch.Body.Insert(0, cLet);
    return;
} 
...
//            else match one level of constructors, continue
  List<Statement> CompileInnerMatchStmt(List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {

    //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) Start");

    List<Statement> r = new List<Statement>();
    AutoGeneratedTokenCloner cloner = new AutoGeneratedTokenCloner();

    // TODO: check that for each branch, size of matchees is size of the list of pattern


    if(branches.Count == 0){
      // If no branch, then match is not exhaustive -- return empty list of statement, TODO: generate a warning
      //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) Not exhaustive");
      return r;
    }
    if(matchees.Count == 0){
      ...
      if (top) {
        //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) matchees is empty AND top");
        } else { //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) matchees is empty, NOT top -> base case");
        }
    ...
    matchees = matchees.Skip(1).ToList();
     ...
     // get the datatype of the matchee (copied from ResolveMatchStmt)
    var currMatcheeType = PartiallyResolveTypeForMemberSelection(currMatchee.tok, currMatchee.Type).NormalizeExpand();
    //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) currMatchee '{0}' has type {1} resolved to {2}", currMatchee.ToString(), currMatchee.Type, currMatcheeType);
    var dtd = currMatcheeType.AsDatatype; 
    // get all constructors of type matchee (copied from ResolveMatchStmt)
    var subst = new Dictionary<TypeParameter, Type>();
    Dictionary<string, DatatypeCtor> ctors;
    if (dtd == null) {
     // Console.WriteLine("DEBUG: (CompileInnerMatchStmt) type of the match source expression  was not a datatype");
     // reporter.Error(MessageSource.Resolver, currMatchee, "->CompileInnerMatchStmt, the type of the match source expression must be a datatype (instead found {0})", currMatchee.Type);
      ctors = null;
    } else {
      ctors = datatypeCtors[dtd];
      Contract.Assert(ctors != null);  // dtd should have been inserted into datatypeCtors during a previous resolution stage
      // build the type-parameter substitution map for this use of the datatype
      subst = TypeSubstitutionMap(dtd.TypeArgs, currMatcheeType.TypeArgs);
    }
    ...
    // If some of the patternHeads are constructor ...
    // e.g.: if Id is some Ctor in ctors, then constructor, otherwise BV
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) in mixed case!");

      // create new match
      var newMatchCases = new List<MatchCaseStmt>();
      // for each constructor
      foreach(var ctor in ctors){
        //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) -> for ctor {0}", ctor.Key);
        var currBranches = new List<RBranch>();
        // create a BV for each formal
        //foreach(Formal f in ctor.Value.Formals){
        //    Console.WriteLine("DEBUG: (CompileInnerMatchStmt) ---> with formals {0} of type {1}", f.Name, f.Type.ToString());

        //}
        List<BoundVar> freshPatBV = ctor.Value.Formals.ConvertAll(x => new BoundVar(new AutoGeneratedToken(x.Tok), FreshTempVarName("_mcc#", codeContext),  SubstType(x.Type, subst)));
        
        //List<CasePattern<BoundVar>> freshPatArgs = freshPatBV.ConvertAll(x => new CasePattern<BoundVar>(new AutoGeneratedToken(x.Tok), x));
        // new Pattern is a fresh pattern for this constructor
        //CasePattern<BoundVar> newPattern = new CasePattern<BoundVar>(new AutoGeneratedToken(currMatchee.tok), ctor.Key, freshPatArgs);
        //List<CasePattern<BoundVar>> newPatternList = new List<CasePattern<BoundVar>>();
        //newPatternList.Add(newPattern);
        // hand-resolve the new pattern
        //newPattern.Ctor = ctor.Value;
        //newPattern.AssembleExpr(ctor.Value.Formals.ConvertAll(x => x.Type));


        // create a rhs version of the pattern (copied from CasePattern's AssembleExpr)
        /* TODO: figure out why this is not working (Contract fails later in resolve, but current (currMatchee)should be equivalent
        var rhsArgs = freshPatBV == null ? new List<Expression>() : freshPatBV.ConvertAll(arg => (Expression)new IdentifierExpr(arg.tok, arg.Name) { Var = arg, Type = arg.Type});
        var rhsExpr = new DatatypeValue(new AutoGeneratedToken(currMatchee.tok), ctor.Value.EnclosingDatatype.Name, ctor.Key, rhsArgs);
        rhsExpr.Ctor = ctor.Value;  // resolve here
        var dtvTypeArgs = freshPatBV == null ? new List<Type>() : freshPatBV.ConvertAll(x => x.Type);
        rhsExpr.InferredTypeArgs.AddRange(dtvTypeArgs);  // resolve here
        rhsExpr.Type = new UserDefinedType(new AutoGeneratedToken(currMatchee.tok), ctor.Value.EnclosingDatatype.Name, ctor.Value.EnclosingDatatype, dtvTypeArgs); */
        var rhsExpr = currMatchee;
        // -- cfilter branches for each constructor
        foreach(var PB in pairPB){
          if(ctor.Key.Equals(PB.Item1.Id)){
            // If pattern is same constructor, push the arguments as patterns and add that branch to new match
            //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) --> branch is same ctor");
            var currBranch = CloneRBrach(PB.Item2);
            ...
            //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) --> branch is bound var");
            // If pattern is a bound variable, create sub-boundvariables for formals, 
            var currBranch = CloneRBrach(PB.Item2);
            // -- create n fresh bound variables of the right type (for ctor of arity n)
            // TODO: these may be useless, could reuse the fresh pattern variables instead of creating new names
        ...
        // -- for each branch, refine pattern if needed, let-bind refined pattern on body
        // -- recur
        // -- add constructor-branch in new match
        List<IdentifierExpr> freshMatchees = freshPatBV.ConvertAll(x => new IdentifierExpr(new AutoGeneratedToken(x.tok), x){ Var = x, Type = x.Type});
        List<Expression> cmatchees= matchees.ConvertAll(x => cloner.CloneExpr(x));
        cmatchees.AddRange(freshMatchees);
        // recursive call in every new branches
        List<Statement> insideBranch = CompileInnerMatchStmt(cmatchees,false, currBranches, codeContext);

        MatchCaseStmt newMatchCase = new MatchCaseStmt(new AutoGeneratedToken(currMatchee.tok), ctor.Key,  freshPatBV, insideBranch); 
        newMatchCase.Ctor = ctor.Value;
        newMatchCases.Insert(0, newMatchCase);
      }
      var newMatchStmt = new MatchStmt(new AutoGeneratedToken(currMatchee.tok), new AutoGeneratedToken(currMatchee.tok), currMatchee, newMatchCases, true);
      r.Insert(0, newMatchStmt);
    } else {
      //Console.WriteLine("DEBUG: (CompileInnerMatchStmt) in var case!");
      // Otherwise, bind each of the variables on body, recur.
      foreach(Tuple<CasePattern<BoundVar>, RBranch> PB in pairPB){
          // TODO: don't duplicate currMatchee on top?
          LetBind(PB.Item2, PB.Item1, currMatchee);
      }
      r = CompileInnerMatchStmt(matchees, false, pairPB.ToList().ConvertAll(new Converter<Tuple<CasePattern<BoundVar>,RBranch>, RBranch>(x => x.Item2)), codeContext);
    } 
    return r;
  }
...
void CompileMatchCaseStmt(MatchStmt s, ICodeContext codeContext) {
  ...
  if(r.Count == 1 && r.First() is MatchStmt){
    var newS = r.First();
