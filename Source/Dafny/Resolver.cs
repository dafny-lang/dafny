...
/// </summary>
  SyntaxContainer CompileRBranch(MatchTempInfo mti, List<Expression> matchees, List<RBranch> branches, ICodeContext codeContext) {
    ...
    // Assert that for each branch, number of matchees (n) is the number of patterns held by the branch
    Contract.Requires(branches.TrueForAll(x => matchees.Count.Equals(x.Patterns.Count)));
        ...
        foreach(var PB in pairPB){
          if(ctor.Key.Equals(PB.Item1.Id)){
            ...
            if(PB.Item1.Arguments != null){
              ...
              Contract.Ensures(PB.Item1.Arguments.Count.Equals(ctor.Value.Formals.Count));
              currBranch.Patterns.AddRange(PB.Item1.Arguments); 
            } else {
              Contract.Ensures(ctor.Value.Formals.Count.Equals(0));
            }
