...
public class SStmt:SyntaxContainer{
...
public class SStmts:SyntaxContainer{
...
//            else match one level of constructors, continue
  SyntaxContainer CompileRBranch(MatchTempInfo mti, List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(branches.Count == 0){
      ...
      return new SStmts(new List<Statement>());
    }
    if(matchees.Count == 0){
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        ...
        List<Statement> insideBranch = ((SStmts)CompileRBranch(mti, cmatchees,false, currBranches, codeContext)).Body;
      ...
      newBody.Insert(0, newMatchStmt);
      return new SStmts(newBody);
    } else {
      ...
      return CompileRBranch(mti, matchees, top, pairPB.ToList().ConvertAll(new Converter<Tuple<CasePattern<BoundVar>,RBranch>, RBranch>(x => x.Item2)), codeContext);
    } 
  }
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  if(rb == null){
    ...
    for(int id = 0; id < mti.BranchIDCount.Length; id++){
      ...
      // rb should be a RBranchStmt, this is unreachable
