    ...
    if(ctors != null &&  patternHeads.Exists(x => x is IdPattern && ctors.ContainsKey(((IdPattern) x).Id))) {
      ...
      foreach(var ctor in ctors) {
        ...
        var freshPatBV = ctor.Value.Formals.ConvertAll(
          x => new BoundVar(new AutoGeneratedToken(x.Tok),
          FreshTempVarName("_mcc#", codeContext),
          SubstType(x.Type, subst)));
//          new InferredTypeProxy()));
          ...
          if (PB.Item1 is IdPattern) {
            ...
            if(ctor.Key.Equals(item1.Id)) {
             ...
             if(item1.Arguments != null && item1.Arguments.Count != 0) {
              ...
              List<IdPattern> freshArgs = ctor.Value.Formals.ConvertAll(x => new IdPattern(new AutoGeneratedToken(x.Tok), FreshTempVarName("_mcc#", codeContext), SubstType(x.Type, subst), new List<ExtendedPattern>()));
              currBranch.Patterns.AddRange(freshArgs);

              LetBindNonWildCard(currBranch,item1.Id, item1.Type, rhsExpr);
              currBranches.Add(currBranch);
            }
          } else {
            Contract.Assert(false); throw new cce.UnreachableException();
      ...
      foreach(var currLit in alternatives) {
        ...
        foreach(var PB in pairPB) {
          if(PB.Item1 is LitPattern) {
            ...
            var item1 = (IdPattern)PB.Item1;
            var currBranch = CloneRBranch(PB.Item2);
            LetBindNonWildCard(currBranch,item1.Id, item1.Type, cloner.CloneExpr(currLit));
            mti.UpdateBranchID(PB.Item2.BranchID, 1);
            currBranches.Add(currBranch);
          } else {
            Contract.Assert(false); throw new cce.UnreachableException();
      ...
      foreach(var PB in pairPB) {
        if(PB.Item1 is IdPattern) {
          ...
          var item1 = (IdPattern)PB.Item1;
          var currBranch = CloneRBranch(PB.Item2);
          LetBindNonWildCard(currBranch,item1.Id, item1.Type, currMatchee);
          mti.UpdateBranchID(PB.Item2.BranchID, 1);
      ...
      foreach(Tuple<ExtendedPattern, RBranch> PB in pairPB) {
          ...
          if(item1.Arguments.Count != 0) {
            if(dtd == null) {
              ...
              SolveAllTypeConstraints();
              if(mti.Debug) Console.WriteLine("DEBUG: type:{0} currMatchee:{1}", PartiallyResolveTypeForMemberSelection(currMatchee.tok, currMatchee.Type).NormalizeExpand().ToString(), currMatcheeType.ToString());

              throw new NotImplementedException("Bug in CompileRExpr: non-nullary constructors of a non-datatype");
            } else {
          ...
          LetBindNonWildCard(PB.Item2, item1.Id, item1.Type, currMatchee);
      }
      if(mti.Debug) {
        Console.WriteLine("DEBUG: {0}return", new String('\t',mti.DebugLevel));
        mti.DebugLevel--;
...
void CompileNestedMatchExpr(NestedMatchExpr e, ICodeContext codeContext) {
  ...
  if (rb is null) {
    ...
    // rb should not be a StmtContainer, this is unreachable
     throw new InvalidOperationException("Returned container should be a CExpr, likely caused by a bug in CompileRBranch.");
  }
    ...
    if(pat is IdPattern) {
      ...
      if(udt.TypeArgs.Count != idpat.Arguments.Count){
        if(idpat.Arguments.Count == 0){
          ...
          reporter.Error(MessageSource.Resolver, pat.Tok, "<In Linear> case arguments count does not match source tuple arguments count");
        }
      }
...
void CheckLinearNestedMatchStmt(Type dtd, NestedMatchStmt ms) {
   ...
   /* Deprecated, this is checked by CheckLinearNestedMatchExpr/Stmt during CompileNestedMatchExpr/Stmt */
    ...
    public void ResolveExpressionX(Expression expr, ResolveOpts opts) {
      ...
      if (expr is ParensExpression) {
        ...
        Console.WriteLine("Resolving a NameSegment at {0}:{1}", e.tok.line,e.Name);
        ResolveNameSegment(e, true, null, opts, false);
        Console.WriteLine("Done Resolving NameSegment {0}", e.Name);
    ...
    void ResolveMatchExpr(MatchExpr me, ResolveOpts opts) {
      ...
      foreach (MatchCaseExpr mc in me.Cases) {
        ...
        Console.WriteLine("DEBUG: {0} Resolving body of MatchExpr", mc.tok.line);
        ResolveExpression(mc.Body, opts);
      	Console.WriteLine("DEBUG: {0} Done resolving body of MatchExpr", mc.tok.line);
    ...
    Expression ResolveNameSegment(NameSegment expr, bool isLastNameSegment, List<Expression> args, ResolveOpts opts, bool allowMethodCall) {
      ...
      if (v != null) {
        ...
        // ----- None of the above
        Console.WriteLine("OSDEBUG: Throwing unresolved ID ", expr.Name);
      ...
      if (r == null) {
        ...
        Console.WriteLine("OSDEBUG: ResolveNameSegment {0}, error thrown", expr.Name);
        expr.Type = new InferredTypeProxy();
      } else {
        Console.WriteLine("OSDEBUG: Resolved NameSegment {0} as {1}", expr.Name, Printer.ExprToString(r));
  ...
  class CoCallResolution
  {
    ...
    void CheckCoCalls(Expression expr, int destructionLevel, DatatypeValue coContext, List<CoCallInfo> coCandidates, Function functionYouMayWishWereAbstemious = null) {
      ...
      Console.WriteLine("Check COCall with {1}:{0}", expr.ToString(), Printer.ExprToString(expr));
      expr = expr.Resolved;
      if (expr is DatatypeValue) {
        var e = (DatatypeValue)expr;
        ...
        Console.WriteLine("Check COCall ITE");
        var e = (ITEExpr)expr;
