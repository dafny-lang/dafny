...
//            else match one level of constructors, continue
  List<Statement> CompileInnerMatchStmt(List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(patternHeads.Exists(x => x.Ctor != null )){
      ...
      foreach(var ctor in ctors){
        ...
        foreach(var PB in pairPB){
          if(PB.Item1.Var != null){
            ...
            LetBind(currBranch,PB.Item1.Var, newPattern.Expr);
        ...
        MatchCaseStmt newMatchCase = new MatchCaseStmt(new AutoGeneratedToken(newPattern.tok), ctor.ToString(),  newPatternList, insideBranch); 
