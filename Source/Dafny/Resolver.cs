...
/// </summary>
  SyntaxContainer CompileRBranch(MatchTempInfo mti, List<Expression> matchees, List<RBranch> branches, ICodeContext codeContext) {
    Contract.Requires(branches.TrueForAll(x => matchees.Count ==  x.Patterns.Count));
    ...
    if(!branches.TrueForAll(x => matchees.Count ==  x.Patterns.Count)){
      throw new InvalidOperationException(string.Format("Match at line {0} is malformed, make sure constructors are fully applied", mti.Tok.line));
    }
