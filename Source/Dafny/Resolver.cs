...
public class SExpr:SyntaxContainer{
  public Expression Body;

  public SExpr(Expression body){
    this.Body = body;
  }
}

public class SStmt:StmtContainer{
  public Statement Body;

  public SStmt(Statement body){
    this.Body = body;
  }
}

public class SStmts:StmtContainer{
  public List<Statement> Body;

  public SStmts(List<Statement> body){
    this.Body = body;
  }
}
...
SyntaxContainer ContainBody(RBranch branch){
  if(branch is RBranchStmt){
    return new SStmts(((RBranchStmt)branch).Body);
  } else if (branch is RBranchExpr) {
    return new SExpr(((RBranchExpr)branch).Body);
  } else {
...
List<Statement> UnboxStmtContainer(SyntaxContainer con){
  if(con is SStmt){
    ...
    r.Add(((SStmt)con).Body);
    return r;
  } else if (con is SStmts){
    return ((SStmts)con).Body;
  } else {
...
void LetBind(RBranch branch, CasePattern<BoundVar> cp, Expression expr){
 ...
 MatchCase MakeMatchCaseFromContainer(IToken tok,KeyValuePair<string, DatatypeCtor> ctor,List<BoundVar> freshPatBV,SyntaxContainer insideContainer){
   ...
   var insideBranch = ((SExpr)insideContainer).Body;
    newMatchCase = new MatchCaseExpr(tok, ctor.Key,  freshPatBV, insideBranch); 
 }
  newMatchCase.Ctor = ctor.Value;
  return newMatchCase;
 }
// Start with a list of n matchees and list of m branches
// if n = 0, return first branch
...
//            else match one level of constructors, continue
  SyntaxContainer CompileRBranch(MatchTempInfo mti, List<Expression> matchees, List<RBranch> branches, ICodeContext codeContext) {
    ...
    // TODO: assert that for each branch, size of matchees is size of the list of pattern


    if(branches.Count == 0){
      // If no branch, then match is not exhaustive -- regenerate warning and return null
    ...
    if(matchees.Count == 0){
      ...
      return ContainBody(branches.First());
    }
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        ...
        if(insideContainer is null){
          // match was not exhaustive, either continue or create empty case
          continue;
        } else {
      ...
      if(mti.isStmt){
        ...
        return new SExpr(newMatchExpr);
      }
    } else {
      // else all head patterns are bound variables:
...
// Create RBranch from an unresolved MatchCaseExpr
...
void CompileMatchExpr(MatchExpr e, ICodeContext codeContext){
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  if (rb is SExpr && ((SExpr)rb).Body is MatchExpr){
    // replace e with desugared match
    var newME = (MatchExpr)((SExpr)rb).Body;
    ...
    // this comes up if we reach the no-matchee case without performing any mixed-case
...
void CompileMatchStmt(MatchStmt s, ICodeContext codeContext) {
 ...
 // initialize the MatchTempInfo to record position and duplication information about each branch
  ...
  SyntaxContainer rb = CompileRBranch(mti, matchees, branches, codeContext);
  if (rb is SStmt && ((SStmt)rb).Body is MatchStmt){
    // replace s with desugared match
    var newMS = (MatchStmt)((SStmt)rb).Body;
  ...
  } else if(rb is SStmts){
    //there wasn't any work to do (i.e. just variable renaming, leave s as is)
    reporter.Warning(MessageSource.Resolver, mti.Tok, "pattern-match without any refinements");
  } else {
    // rb should be a StmtContainer with a MatchCase as Body, this is unreachable
    Contract.Requires(false);
  }
} 
