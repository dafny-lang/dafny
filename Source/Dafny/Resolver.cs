...
/* Temporary Information about Match being desugared  */
public class MatchTempInfo{
  public int[] BranchIDCount;
  public IToken Tok;
  public MatchTempInfo(IToken tok, int[] branchidcount){
    this.BranchIDCount = branchidcount;
    this.Tok = tok;
  }

 // Initialize BranchIDCount with #branchidnum 1s
 public MatchTempInfo(IToken tok, int branchidnum){
...
public class RBranch {
  ...
  public RBranch(){}
  public RBranch(int branchid, List<CasePattern<BoundVar>> patterns,  List<Statement> body){
...
public static RBranch CloneRBrach(RBranch branch){
  ...
  return new RBranch(branch.BranchID, branch.Patterns.ConvertAll(x => cloner.CloneCasePattern(x)), branch.Body.ConvertAll(x=> cloner.CloneStmt(x)));
}
...
public static Tuple<CasePattern<BoundVar>,RBranch> popPattern(RBranch branch){
  ...
  // TODO: whu is this not a clone?
  var newBranch = new RBranch(branch.BranchID, branch.Patterns, branch.Body);
...
//            else match one level of constructors, continue
  List<Statement> CompileInnerMatchStmt(MatchTempInfo mti, List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(matchees.Count == 0){
      ...
      // TODO: return an empty case instead
      if (top) {
        reporter.Warning(MessageSource.Resolver, mti.Tok, "pattern-match without any refinements");
        return new List<Statement>();
      } else { 
        // Decreate the count of every branches except the first one
        for(int i = 1; i < branches.Count(); i ++){
          mti.UpdateBranchID(branches.ElementAt(i).BranchID, -1);
        }
        return branches.First().Body;
      }
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      // create a new match with a branch for each constructor
      ...
      foreach(var ctor in ctors){
        ...
        foreach(var PB in pairPB){
          if(ctor.Key.Equals(PB.Item1.Id)){
            ...
            // If pattern is a bound variable, create new bound variables for each of the arguments of the constructor, and let-binds the matchee as original bound variable
        ...
        List<Statement> insideBranch = CompileInnerMatchStmt(mti, cmatchees,false, currBranches, codeContext);
      ...
      // All head patterns are bound variables:
      ...
      r = CompileInnerMatchStmt(mti, matchees, top, pairPB.ToList().ConvertAll(new Converter<Tuple<CasePattern<BoundVar>,RBranch>, RBranch>(x => x.Item2)), codeContext);
    } 
    return r;
  }
...
RBranch RBranchOfMatchCaseStmt(int branchid, MatchCaseStmt x){
  ...
  return new RBranch(branchid, PatternList,x.Body);
}
...
void CompileMatchCaseStmt(MatchStmt s, ICodeContext codeContext) {
  ...
  for(int id = 0; id < s.Cases.Count(); id++){
    branches.Add(RBranchOfMatchCaseStmt(id, s.Cases.ElementAt(id)));
  }
  MatchTempInfo mti = new MatchTempInfo(s.Tok, s.Cases.Count());

  List<Expression> matchees = new List<Expression>();
  matchees.Add(s.Source);
  var r = CompileInnerMatchStmt(mti, matchees, true, branches, codeContext);
  ...
  if(r.Count == 1 && r.First() is MatchStmt){
    ...
    for(int id = 0; id < mti.BranchIDCount.Length; id++){
      if(mti.BranchIDCount[id] <=0){
        reporter.Warning(MessageSource.Resolver, mti.Tok, "branch {0} is redundant ",id+1);
        //Console.WriteLine("DEBUG: (CompileMatchCaseStmt) At {0}, {1}th branch copied {2} time(s)", mti.Tok.line, id, mti.BranchIDCount[id]);
      }
    }
  } else {
