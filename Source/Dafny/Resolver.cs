...
void LetBind(RBranch branch, CasePattern<BoundVar> cp, Expression expr){
   if (cp.Var == null){
    Console.WriteLine("DEBUG: (LetBind) null var ");
    Type type = new InferredTypeProxy();
    cp.Var = new BoundVar(new AutoGeneratedToken(cp.tok), cp.Id, type);
   }
    var var = cp.Var;
  ...
  List<Statement> CompileInnerMatchStmt(List<Expression> matchees, bool top, List<RBranch> branches, ICodeContext codeContext) {
    ...
    if(ctors != null && patternHeads.Exists(x => ctors.ContainsKey(x.Id))){
      ...
      foreach(var ctor in ctors){
        ...
        var dtvTypeArgs = freshPatBV.ConvertAll(x => x.Type);
        ...
        // TODO: filter using pre-Resolved casePattern
