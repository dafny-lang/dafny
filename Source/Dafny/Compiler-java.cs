...
namespace Microsoft.Dafny{
  public class JavaCompiler : Compiler{
    ...
    protected class ClassWriter : IClassWriter{
      ...
      public ClassWriter(JavaCompiler compiler, BlockTargetWriter instanceMemberWriter,
        BlockTargetWriter staticMemberWriter = null){
      ...
      public BlockTargetWriter /*?*/ CreateFunction(string name, List<TypeParameter> /*?*/ typeArgs,
      ...
      public BlockTargetWriter /*?*/ CreateGetter(string name, Type resultType, Bpl.IToken tok, bool isStatic,
        bool createBody, MemberDecl /*?*/ member){
      ...
      public BlockTargetWriter /*?*/ CreateGetterSetter(string name, Type resultType, Bpl.IToken tok, bool isStatic,
        bool createBody, MemberDecl /*?*/ member, out TargetWriter setterWriter){
        return Compiler.CreateGetterSetter(name, resultType, tok, isStatic, createBody, out setterWriter,
          Writer(isStatic));
      }
    ...
    public BlockTargetWriter /*?*/ CreateGetterSetter(string name, Type resultType, Bpl.IToken tok, bool isStatic,
      ...
      if (createBody){
        setterWriter = wr.NewBlock(null, null, BlockTargetWriter.BraceStyle.Newline,
    ...
    protected void DeclareField(string name, bool isStatic, bool isConst, Type type, Bpl.IToken tok, string rhs,
      TargetWriter wr){
      wr.WriteLine("public {0}{1} {2} = {3};", isStatic ? "static " : "", TypeName(type, wr, tok), name,
        (rhs != null) ? rhs : DefaultValue(type, wr, tok));
    }
      ...
      if (isLValue && member is ConstantField){
        ...
        if (compiledName.Length != 0){
          wr.Write(".{0}{1}{2}", MemberSelectObjIsTrait && !sf.IsStatic ? "get_" : "", compiledName,
            MemberSelectObjIsTrait && !sf.IsStatic ? "()" : "");
        }
      }
...
//      }
      ...
      foreach (var ctor in dt.Ctors){
        if (dt.IsRecordType){
          ...
          wr.WriteLine("public boolean is_{0}() {{ return this instanceof {1}_{0}; }}", ctor.CompileName,
            dt.CompileName);
        }
      }
            ...
            if (!arg.IsGhost && arg.HasName){
              using (var wDtor = wr.NewNamedBlock("public {0} dtor_{1}()", TypeName(arg.Type, wr, arg.tok),
                ...
                if (dt.IsRecordType){
                  ...
                  wDtor.WriteLine("return (({0}_{1}{2})d).{3}; ", dt.CompileName,
                    dtor.EnclosingCtors[n - 1].CompileName, DtT_TypeArgs, IdName(arg));
                }
              }
            }
          }
        ...
        if (td.Witness != null){
          ...
          if (ArrowType.IsPartialArrowTypeName(td.Name)){
            ...
            return string.Format("(({0}) -> {1})",
              Util.Comma(", ", udt.TypeArgs.Count - 1,
                i => string.Format("{0} x{1}", TypeName(udt.TypeArgs[i], wr, udt.tok), i)),
              rangeDefaultValue);
          }
            ...
            return string.Format("new {0}[{1}]{2}", typeNameSansBrackets, Util.Comma(arrayClass.Dims, _ => "0"),
              brackets);
          }
          else{
            return "null";
