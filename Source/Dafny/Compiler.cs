...
namespace Microsoft.Dafny {
  public abstract class Compiler {
    ...
    /// In the above, if "type" is null, then it is replaced by "var" or "let".
    /// "tok" is allowed to be null if "type" is.
    /// "exactTypeArguments" is usually "false", but is "true" when the variable declared is a "builder". This is used with the way Java handles
    /// co-variant types. For example, to build up the elements generated by a set comprehension, the compilation may use an ArrayList<T>, whereas
    /// the set constructed will have the type Set<? extends T>.
    /// </summary>
    protected abstract TargetWriter DeclareLocalVar(string name, Type/*?*/ type, Bpl.IToken/*?*/ tok, TargetWriter wr, bool exactTypeArguments = false);
