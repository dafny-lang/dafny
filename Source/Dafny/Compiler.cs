...
namespace Microsoft.Dafny{
  public abstract class Compiler{
    ...
    Stack<TargetWriter>
      copyInstrWriters =
        new Stack<TargetWriter>(); // a buffer that stores copy instructions generated by letExpr that uses out param.
    ...
    protected abstract TargetWriter CreateModule(string moduleName, bool isDefault, bool isExtern,
    ...
    protected interface IClassWriter{
      ...
      BlockTargetWriter /*?*/ CreateFunction(string name, List<TypeParameter> /*?*/ typeArgs, List<Formal> formals,
        Type resultType, Bpl.IToken tok, bool isStatic, bool createBody, MemberDecl member);

      BlockTargetWriter /*?*/
        CreateGetter(string name, Type resultType, Bpl.IToken tok, bool isStatic, bool createBody,
          MemberDecl /*?*/ member); // returns null iff !createBody

      BlockTargetWriter /*?*/
        CreateGetterSetter(string name, Type resultType, Bpl.IToken tok, bool isStatic, bool createBody,
          MemberDecl /*?*/ member,
          out TargetWriter setterWriter); // if createBody, then result and setterWriter are non-null, else both are null
    ...
    /// </summary>
    protected abstract IClassWriter CreateClass(string name, bool isExtern, string /*?*/ fullPrintName,
      List<TypeParameter> /*?*/ typeParameters, List<Type> /*?*/ superClasses, Bpl.IToken tok, TargetWriter wr);

    /// <summary>
    /// "tok" can be "null" if "superClasses" is.
    /// </summary>
    protected abstract IClassWriter CreateTrait(string name, bool isExtern, List<Type> /*?*/ superClasses,
    ...
    protected virtual bool NeedsWrappersForInheritedFields{
      get => true;
    }

    protected virtual bool SupportsProperties{
      get => true;
    }
    ...
    protected virtual void GetNativeInfo(NativeType.Selection sel, out string name, out string literalSuffix,
    ...
    protected virtual int EmitRuntimeTypeDescriptorsActuals(List<Type> typeArgs, List<TypeParameter> formals,
    ...
    public abstract string TypeInitializationValue(Type type, TextWriter /*?*/ wr, Bpl.IToken /*?*/ tok,
      bool inAutoInitContext);

    protected abstract string TypeName_UDT(string fullCompileName, List<Type> typeArgs, TextWriter wr, Bpl.IToken tok);

    protected abstract string /*?*/
      TypeName_Companion(Type type, TextWriter wr, Bpl.IToken tok, MemberDecl /*?*/ member);
    ...
    protected abstract bool DeclareFormal(string prefix, string name, Type type, Bpl.IToken tok, bool isInParam,
      TextWriter wr);
    ...
    /// "tok" is allowed to be null if "type" is.
    /// </summary>
    protected abstract TargetWriter
      DeclareLocalVar(string name, Type /*?*/ type, Bpl.IToken /*?*/ tok, TargetWriter wr);

    protected virtual void
      DeclareOutCollector(string collectorVarName, TargetWriter wr){ } // called only for return-style calls

    protected virtual void
      DeclareSpecificOutCollector(string collectorVarName, TargetWriter wr, int outCount,
        Method m){ } // for languages that don't allow "let" or "var" expressions

    protected virtual bool UseReturnStyleOuts(Method m, int nonGhostOutCount) => false;

    protected virtual bool SupportsMultipleReturns{
      get => false;
    }

    protected virtual bool NeedsCastFromTypeParameter{
      get => false;
    }

    protected virtual bool SupportsAmbiguousTypeDecl{
      get => true;
    }

    protected virtual bool FieldsInTraits{
      get => true;
    } // True if language's "trait" equivalent allows for non-final field declarations, false otherwise

    protected bool
      MemberSelectObjIsTrait; // Example: if MemberSelect expression is T.x, this will be true if T is a trait, false otherwise

    /// The punctuation that comes at the end of a statement.  Note that
    /// statements are followed by newlines regardless.
    protected virtual string StmtTerminator{
      get => ";";
    }

    protected void EndStmt(TargetWriter wr){
      wr.WriteLine(StmtTerminator);
    }

    protected abstract void DeclareLocalOutVar(string name, Type type, Bpl.IToken tok, string rhs, TargetWriter wr);

    protected virtual void
      EmitActualOutArg(string actualOutParamName,
        TextWriter wr){ } // actualOutParamName is always the name of a local variable; called only for non-return-style outs

    protected virtual void EmitOutParameterSplits(string outCollector, List<string> actualOutParamNames,
      TargetWriter wr){ } // called only for return-style calls

    protected virtual void EmitCastOutParameterSplits(string outCollector, List<string> actualOutParamNames,
    ...
    protected virtual void EmitMultiAssignment(out List<TargetWriter> wLhss, List<Type> lhsTypes,
      out List<TargetWriter> wRhss, List<Type> rhsTypes, TargetWriter wr){
    ...
    protected virtual void EmitReturnExpr(Expression expr, bool inLetExprBody, TargetWriter wr){
      // emits "return <expr>;" for function bodies
      var w = EmitReturnExpr(wr);
      TrExpr(expr, w, inLetExprBody);
    }

    protected virtual void EmitReturnExpr(string returnExpr, TargetWriter wr){
    ...
    protected virtual TargetWriter EmitWhile(List<Statement> body, TargetWriter wr){
    ...
    /// </summary>
    protected abstract BlockTargetWriter CreateForeachLoop(string boundVar, Type /*?*/ boundVarType,
      out TargetWriter collectionWriter, TargetWriter wr, string /*?*/ altBoundVarName = null,
      Type /*?*/ altVarType = null, Bpl.IToken /*?*/ tok = null);
    ...
    protected abstract TargetWriter EmitBitvectorTruncation(BitvectorType bvType, bool surroundByUnchecked,
      TargetWriter wr);

    protected delegate void FCE_Arg_Translator(Expression e, TargetWriter wr, bool inLetExpr = false);

    protected abstract void EmitRotate(Expression e0, Expression e1, bool isRotateLeft, TargetWriter wr,
    ...
    /// </summary>
    protected virtual TargetWriter EmitCoercionIfNecessary(Type /*?*/ from, Type /*?*/ to, Bpl.IToken tok,
      TargetWriter wr){
      return wr;
    }
    ...
    protected virtual void EmitITE(Expression guard, Expression thn, Expression els, bool inLetExprBody,
      TargetWriter wr){
    ...
    protected abstract void GetSpecialFieldInfo(SpecialField.ID id, object idParam, out string compiledName,
      out string preString, out string postString);

    protected abstract TargetWriter EmitMemberSelect(MemberDecl member, bool isLValue, Type expectedType,
      TargetWriter wr);
    ...
    protected abstract TargetWriter EmitArraySelect(List<Expression> indices, Type elmtType, bool inLetExprBody,
      TargetWriter wr);
    ...
    protected abstract void EmitIndexCollectionUpdate(Expression source, Expression index, Expression value,
      bool inLetExprBody, TargetWriter wr, bool nativeIndex = false);

    protected virtual void EmitIndexCollectionUpdate(out TargetWriter wSource, out TargetWriter wIndex,
      out TargetWriter wValue, TargetWriter wr, bool nativeIndex = false){
      wSource = wr.Fork();
      wr.Write('[');
      wIndex = wr.Fork();
      wr.Write("] = ");
    ...
    protected abstract void EmitSeqSelectRange(Expression source, Expression /*?*/ lo, Expression /*?*/ hi,
      bool fromArray, bool inLetExprBody, TargetWriter wr);

    protected abstract void EmitMultiSetFormingExpr(MultiSetFormingExpr expr, bool inLetExprBody, TargetWriter wr);

    protected abstract void EmitApplyExpr(Type functionType, Bpl.IToken tok, Expression function,
      List<Expression> arguments, bool inLetExprBody, TargetWriter wr);

    protected abstract TargetWriter EmitBetaRedex(List<string> boundVars, List<Expression> arguments, string typeArgs,
    ...
    /// </summary>
    protected abstract void EmitDestructor(string source, Formal dtor, int formalNonGhostIndex, DatatypeCtor ctor,
      List<Type> typeArgs, Type bvType, TargetWriter wr);

    protected abstract BlockTargetWriter CreateLambda(List<Type> inTypes, Bpl.IToken tok, List<string> inNames,
      Type resultType, TargetWriter wr, bool untyped = false);

    protected abstract TargetWriter CreateIIFE_ExprBody(Expression source, bool inLetExprBody, Type sourceType,
      Bpl.IToken sourceTok, Type resultType, Bpl.IToken resultTok, string bvName,
      TargetWriter wr); // Immediately Invoked Function Expression

    protected abstract TargetWriter CreateIIFE_ExprBody(string source, Type sourceType, Bpl.IToken sourceTok,
      Type resultType, Bpl.IToken resultTok, string bvName, TargetWriter wr); // Immediately Invoked Function Expression

    protected abstract BlockTargetWriter
      CreateIIFE0(Type resultType, Bpl.IToken resultTok, TargetWriter wr); // Immediately Invoked Function Expression

    protected abstract BlockTargetWriter CreateIIFE1(int source, Type resultType, Bpl.IToken resultTok, string bvName,
      TargetWriter wr); // Immediately Invoked Function Expression

    public enum ResolvedUnaryOp{
      BoolNot,
      BitwiseNot,
      Cardinality
    }
    ...
    protected abstract void EmitCollectionDisplay(CollectionType ct, Bpl.IToken tok, List<Expression> elements,
      bool inLetExprBody, TargetWriter wr); // used for sets, multisets, and sequences
    ...
    protected abstract void EmitCollectionBuilder_Add(CollectionType ct, string collName, Expression elmt,
      bool inLetExprBody, TargetWriter wr);

    protected abstract TargetWriter EmitMapBuilder_Add(MapType mt, Bpl.IToken tok, string collName, Expression term,
      bool inLetExprBody, TargetWriter wr);

    protected abstract string GetCollectionBuilder_Build(CollectionType ct, Bpl.IToken tok, string collName,
      TargetWriter wr);
        ...
        foreach (TopLevelDecl d in m.TopLevelDecls){
          ...
          if (d is OpaqueTypeDecl){
            ...
            if (DafnyOptions.O.ForbidNondeterminism && iter.Outs.Count > 0){
              Error(iter.tok,
                "since yield parameters are initialized arbitrarily, iterators are forbidden by /definiteAssignment:3 option",
                wr);
            }
    ...
    protected void WriteFromFile(string inputFilename, TextWriter outputWriter){
      using (var rd =
        new StreamReader(new FileStream(inputFilename, System.IO.FileMode.Open, System.IO.FileAccess.Read))){
        while (true){
            ...
            foreach (var member in c.Members){
              ...
              if (m != null && IsMain(m)){
                if (mainMethod == null){
                  ...
                  Error(m.tok, "More than one method is declared as \"main\". First declaration appeared at {0}.", null,
        ...
        if (member.IsGhost){
          ...
          if (NeedsWrappersForInheritedFields){
            ...
            if (cf.Rhs == null){
              ...
              classWriter.DeclareField("_" + cf.CompileName, false, false, cf.Type, cf.tok,
                DefaultValue(cf.Type, errorWr, cf.tok, true));
            }
          ...
          if (NeedsWrappersForInheritedFields){
            ...
            classWriter.DeclareField("_" + f.CompileName, false, false, f.Type, f.tok,
            ...
            if (!FieldsInTraits && f is ConstantField cf && cf.Rhs != null){
              ...
              classWriter.DeclareField(f.CompileName, false, false, f.Type, f.tok,
                DefaultValue(f.Type, errorWr, f.tok, true));
            }
        ...
        if (member is Field){
          ...
          if (f.Body == null && !(c is TraitDecl && !f.IsStatic) &&
            ...
            if (Attributes.Contains(f.Attributes, "axiom") ||
                (!DafnyOptions.O.DisallowExterns && Attributes.Contains(f.Attributes, "extern"))){
        ...
        foreach (var trait in cl.TraitsObj){
          ...
          if (Attributes.ContainsBool(trait.Attributes, "handle", ref isHandle) && isHandle){
            ...
            Error(cl.tok,
              ...
              errorWr, cl.WhatKind, cl.Name, trait.Name);
          }
        }

        foreach (var member in cl.InheritedMembers.Concat(cl.Members)){
          if (!member.IsGhost && !member.IsStatic){
            Error(member.tok, "{0} '{1}' is marked as :handle, so all its non-static members must be ghost: {2}",
              errorWr, cl.WhatKind, cl.Name, member.Name);
          }
        }
      }
    }
    ...
    private void CheckForCapitalizationConflicts<T>(IEnumerable<T> canChange, IEnumerable<T> cantChange = null)
      where T : Declaration{
      if (cantChange == null){
    ...
    protected virtual string PrefixForForcedCapitalization{
      get => "Cap_";
    }
    ...
    void TrCasePatternOpt<VT>(CasePattern<VT> pat, Expression rhs, TargetWriter wr, bool inLetExprBody)
      where VT : IVariable{
      TrCasePatternOpt(pat, rhs, null, rhs.Type, rhs.tok, wr, inLetExprBody);
    }

    void TrCasePatternOpt<VT>(CasePattern<VT> pat, Expression rhs, string rhs_string, Type rhsType, Bpl.IToken rhsTok,
      TargetWriter wr, bool inLetExprBody) where VT : IVariable{
    ...
    /// </summary>
    protected void TypeName_SplitArrayName(Type type, TextWriter wr, Bpl.IToken tok, out string typeNameSansBrackets,
      out string brackets){
    ...
    static void TypeInitialization(Type type, Compiler /*?*/ compiler, TextWriter /*?*/ wr, Bpl.IToken /*?*/ tok,
      out bool hasSimpleZeroInitializer, out bool hasZeroInitializer, out bool initializerIsKnown,
      out string defaultValue,
      bool inAutoInitContext = false){
      Contract.Requires(type != null);
      Contract.Requires(compiler == null || (wr != null && tok != null));
      Contract.Ensures(!Contract.ValueAtReturn(out hasSimpleZeroInitializer) ||
                       Contract.ValueAtReturn(
                         out hasZeroInitializer)); // hasSimpleZeroInitializer ==> hasZeroInitializer
      Contract.Ensures(
        !Contract.ValueAtReturn(out hasZeroInitializer) ||
        Contract.ValueAtReturn(out initializerIsKnown)); // hasZeroInitializer ==> initializerIsKnown
      ...
      if (cl is OpaqueTypeDecl){
        ...
        if (td.Witness != null){
          ...
          bool ik;
          string dv;
          TypeInitialization(td.BaseType, null, null, null, out hasSimpleZeroInitializer, out hasZeroInitializer,
            out ik, out dv);
          initializerIsKnown = true;
          return;
        }
          ...
          string dv;
          TypeInitialization(td.BaseType, compiler, wr, udt.tok, out hasSimpleZeroInitializer, out hasZeroInitializer,
            out initializerIsKnown, out dv);
        ...
        if (td.Witness != null){
          ...
          if (ArrowType.IsPartialArrowTypeName(td.Name)){
            ...
            bool hs, hz;
            string dv;
            TypeInitialization(udt.TypeArgs.Last(), compiler, wr, udt.tok, out hs, out hz, out initializerIsKnown,
              out dv);
            return;
          }
          ...
          string dv;
          TypeInitialization(td.RhsWithArgument(udt.TypeArgs), compiler, wr, udt.tok, out hasSimpleZeroInitializer,
            out hasZeroInitializer, out initializerIsKnown, out dv);
          Contract.Assert(compiler == null || string.Equals(dv, defaultValue));
          return;
        }
      }
        ...
        // --- hasZeroInitializer ---
        hasSimpleZeroInitializer =
          false; // TODO: improve this one in special cases where one of the datatype values can be represented by "null"
        // --- initializerIsKnown ---
        if (cl is CoDatatypeDecl){
          hasZeroInitializer =
            false; // TODO: improve this one by laying down a _DafnyDefaultValue method when it's possible
          ...
          hasZeroInitializer = defaultCtor.Formals.TrueForAll(formal =>
            formal.IsGhost || HasZeroInitializer(Resolver.SubstType(formal.Type, subst)));
          initializerIsKnown = defaultCtor.Formals.TrueForAll(formal =>
            formal.IsGhost || InitializerIsKnown(Resolver.SubstType(formal.Type, subst)));
        }
    ...
    void TrStmt(Statement stmt, TargetWriter wr){
      ...
      if (stmt is PrintStmt){
        ...
        if (s.Rhs is HavocRhs){
          ...
          MemberSelectObjIsTrait = s.Lhs is MemberSelectExpr && ((MemberSelectExpr) s.Lhs).Obj.Type.IsTraitType &&
                                   !(((MemberSelectExpr) s.Lhs).Obj is ThisExpr);
        ...
        if (s.AssumeToken != null){
          ...
          var missingBounds = ComprehensionExpr.BoundedPool.MissingBounds(lhss, s.Bounds,
        ...
        foreach (var alternative in s.Alternatives){
          ...
          var thn = EmitIf(out guardWriter, true, wr);
          TrExpr(
            alternative.IsBindingGuard
              ? Translator.AlphaRename((ExistsExpr) alternative.Guard, "eg_d")
              : alternative.Guard, guardWriter, false);
    ...
    void CompileCollection(ComprehensionExpr.BoundedPool bound, IVariable bv, bool inLetExprBody,
      bool includeDuplicates,
      TargetWriter collectionWriter,
      List<ComprehensionExpr.BoundedPool> /*?*/ bounds = null, List<BoundVar> /*?*/ boundVars = null,
      int boundIndex = 0){
      Contract.Requires(bound != null);
      Contract.Requires(bounds == null || (boundVars != null && bounds.Count == boundVars.Count && 0 <= boundIndex &&
                                           boundIndex < bounds.Count));
      ...
      if (bound is ComprehensionExpr.BoolBoundedPool){
        ...
        collectionWriter.Write("{0}.AllSingletonConstructors{1}",
          TypeName_Companion(bv.Type, collectionWriter, bv.Tok, null), propertySuffix);
      }
      else{
        Contract.Assert(false);
        throw new cce.UnreachableException(); // unexpected BoundedPool type
      }
    }

    private Expression SubstituteBound(ComprehensionExpr.IntBoundedPool b, List<ComprehensionExpr.BoundedPool> bounds,
      List<BoundVar> boundVars, int index, bool lowBound){
    ...
    private bool CanSequentializeForall(List<BoundVar> bvs, List<ComprehensionExpr.BoundedPool> bounds,
      Expression range, Expression lhs, Expression rhs){
      // Given a statement
      //
      //   forall i, ... | R {
      //     L := E;
    ...
    private void TrAssignSuchThat(List<IVariable> lhss, Expression constraint,
      List<ComprehensionExpr.BoundedPool> bounds, int debuginfoLine, TargetWriter wr, bool inLetExprBody){
      ...
      bool needIterLimit = lhss.Count != 1 &&
                           bounds.Exists(bnd => (bnd.Virtues & ComprehensionExpr.BoundedPool.PoolVirtues.Finite) == 0);
      ...
      for (int i = 0; i < n; i++){
        var bound = bounds[i];
        Contract.Assert((bound.Virtues & ComprehensionExpr.BoundedPool.PoolVirtues.Enumerable) !=
    ...
    string CreateLvalue(Expression lhs, TargetWriter wr){
      ...
      if (lhs is IdentifierExpr){
        ...
        Contract.Assert(!ll.Member
          .IsInstanceIndependentConstant); // instance-independent const's don't have assignment statements
    ...
    void TrRhs(AssignmentRhs rhs, TargetWriter wr, TargetWriter wStmts){
      ...
      if (tRhs == null){
        var
          eRhs = (ExprRhs) rhs; // it's not HavocRhs (by the precondition) or TypeRhs (by the "if" test), so it's gotta be ExprRhs
        ...
        if (tRhs.InitCall != null){
          ...
          for (var d = 0; d < tRhs.ArrayDimensions.Count; d++){
            string len, p0, p1;
            GetSpecialFieldInfo(SpecialField.ID.ArrayLengthInt, tRhs.ArrayDimensions.Count == 1 ? null : (object) d,
              out len, out p0, out p1);
    ...
    void TrCallStmt(CallStmt s, string receiverReplacement, TargetWriter wr){
      ...
      if (s.Method == enclosingMethod && enclosingMethod.IsTailRecursive){
        ...
        var lvalues =
          new List<string /*?*/
          >(); // contains an entry for each non-ghost formal out-parameter, but the entry is null if the actual out-parameter is ghost
        ...
        var returnStyleOutCollector = outTmps.Count > 0 && returnStyleOuts && !SupportsMultipleReturns
          ? idGenerator.FreshId("_outcollector")
          : null;
    ...
    TargetWriter MatchCasePrelude(string source, UserDefinedType sourceType, DatatypeCtor ctor,
      List<BoundVar> arguments, int caseIndex, int caseCount, TargetWriter wr){
      ...
      if (expr is LiteralExpr){
        ...
        for (int i = 0; i < dtv.Arguments.Count; i++){
          ...
          if (!formal.IsGhost){
            ...
            var w = EmitCoercionIfNecessary(from: dtv.Arguments[i].Type, to: dtv.Ctor.Formals[i].Type, tok: dtv.tok,
              wr: wrArgumentList);
        ...
        if (e.Exact){
          ...
          for (int i = 0; i < e.LHSs.Count; i++){
            ...
            if (Contract.Exists(lhs.Vars, bv => !bv.IsGhost)){
              ...
              w = CreateIIFE_ExprBody(e.RHSs[i], inLetExprBody, e.RHSs[i].Type, e.RHSs[i].tok, e.Body.Type, e.Body.tok,
                rhsName, w);
          ...
          var missingBounds = ComprehensionExpr.BoolBoundedPool.MissingBounds(e.BoundVars.ToList<BoundVar>(),
            e.Constraint_Bounds, ComprehensionExpr.BoundedPool.PoolVirtues.Enumerable);
          if (missingBounds.Count != 0){
            foreach (var bv in missingBounds){
              Error(e.tok,
        ...
        Contract.Assert(
          e.Bounds != null); // for non-ghost quantifiers, the resolver would have insisted on finding bounds
        ...
        for (int i = 0; i < n; i++){
          ...
          TargetWriter newWBody = CreateLambda(new List<Type>{bv.Type}, e.tok, new List<string>{IdName(bv)}, Type.Bool,
            wBody, untyped: true);
        ...
        var typeArgs = TypeName_UDT(ArrowType.Arrow_FullCompileName,
          Util.Snoc(bvars.ConvertAll(bv => bv.Type), expr.Type), wr, Bpl.Token.NoToken);
        var boundVars = Util.Comma(bvars, IdName);
        wr = EmitBetaRedex(bvars.ConvertAll(IdName), fexprs, typeArgs, bvars.ConvertAll(bv => bv.Type), expr.Type,
          expr.tok, inLetExprBody, wr);
        wr = CreateLambda(e.BoundVars.ConvertAll(bv => bv.Type), Bpl.Token.NoToken, e.BoundVars.ConvertAll(IdName),
          e.Body.Type, wr);
    ...
    void CreateFreeVarSubstitution(Expression expr, out List<BoundVar> bvars, out List<Expression> fexprs,
      out Translator.Substituter su){
      Contract.Requires(expr != null);
    ...
    public virtual bool SupportsInMemoryCompilation{
      get => true;
    }
    ...
    /// </summary>
    public virtual bool CompileTargetProgram(string dafnyProgramName, string targetProgramText, string /*?*/ callToMain,
    ...
    /// </summary>
    public virtual bool RunTargetProgram(string dafnyProgramName, string targetProgramText, string /*?*/ callToMain,
      string /*?*/ targetFilename, ReadOnlyCollection<string> otherFileNames,
  ...
  public class BlockTargetWriter : TargetWriter{
    ...
    BraceStyle
      closeBraceStyle =
        BraceStyle.Newline; // must be set to its final value before the BlockTargetWriter is .Add'ed to an enclosing TargetWriter
