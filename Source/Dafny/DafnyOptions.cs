...
/printMode:<Everything|DllEmbed|NoIncludes|NoGhost>
    ...
    NoIncludes disables printing of {{:verify false}} methods incorporated via
...
/view:<view1, view2>
    ...
    If print before the module is resolved (/dprint), then everything in the
    module is printed.
...
/printTooltips
    Dump additional positional information (displayed as mouse-over tooltips
    by the VS plugin) to stdout as 'Info' messages.
/printIncludes:<None|Immediate|Transitive>
  ...
  ---- Dafny warning selection -----------------------------------------------

/warnShadowing  Emits a warning if the name of a declared variable caused
    another variable to be shadowed
/warningsAsErrors
    Treat warnings as errors.

  ---- Dafny language feature selection --------------------------------------

/noIncludes   Ignore include directives
/noExterns    Ignore extern and dllimport attributes
/allowGlobals Allow the implicit class '_default' to contain fields, instance
    functions, and instance methods.  These class members are declared at the
    module scope, outside of explicit classes.  This command-line option is
    provided to simplify a transition from the behavior in the language prior
    to version 1.9.3, from which point onward all functions and methods
    declared at the module scope are implicitly static and fields declarations
    are not allowed at the module scope.
/definiteAssignment:<n>
    0 - ignores definite-assignment rules; this mode is for testing only--it
        is not sound
    1 (default) - enforces definite-assignment rules for compiled variables
        and fields whose types do not support auto-initialization and for
        ghost variables and fields whose type is possibly empty
    ...
    3 - like 2, but also performs checks in the compiler that no
        nondeterministic statements are used; thus, a program that passes
        at this level 3 is one that the language guarantees that values seen
...
/mimicVerificationOf:<Dafny version>
    ...
    version of Dafny. Useful during migration to a newer version of Dafny when
    a Dafny program has proofs, such as methods or lemmas, that are unstable
    in the sense that their verification may become slower or fail altogether
    after logically irrelevant changes are made in the verification input.

    Accepted versions are: 3.3 (note that this turns off features that prevent
    classes of verification instability)

  ---- Dafny compilation options ---------------------------------------------
...
/compileTarget:<lang>
    ...
    Note that the C++ backend has various limitations
    (see Docs/Compilation/Cpp.md). This includes lack of support for
...
/spillTargetCode:<n>
    ...
    3 - Write the textual target program, regardless of verification outcome
        and /compile setting.
    Note, some compiler targets may (always or in some situations) write out
    the textual target program as part of compilation, in which case
...
/optimizeResolution:<n>
    ...
    2 (default) - As in 1, but only resolve method bodies in non-included Dafny
                  sources
/useRuntimeLib
    Refer to pre-built DafnyRuntime.dll in compiled assembly rather
    than including DafnyRuntime.cs verbatim.

  ---- Dafny test generation options -----------------------------------------

{TestGenOptions.Help}

  ---- Pass-through Boogie options -------------------------------------------
