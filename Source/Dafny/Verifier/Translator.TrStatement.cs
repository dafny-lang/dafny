  ...
  public partial class Translator {
    private void TrStmt(Statement stmt, BoogieStmtListBuilder builder, List<Variable> locals, ExpressionTranslator etran) {
      ...
      if (stmt is PredicateStmt predicateStmt) {
        ...
        // Halt recovery statements are currently internal-only AST nodes that cannot be
        // directly used in user Dafny code. They are only generated by rewriters, and verifying
        // their use is low value. For now we just verify the main body since it's simple to just
        // translate that, ignoring the "halt recovery" body.
        TrStmt(haltRecoveryStatement.Body, builder, locals, etran);
      } else {
