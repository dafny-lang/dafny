...
namespace Microsoft.Dafny {
  public partial class Translator {
    ...
    internal IToken GetTok(Expression e) {
      return flags.ReportRanges ? e.RangeToken : e.tok;
    }
    ...
    void CheckDefiniteAssignment(IdentifierExpr expr, BoogieStmtListBuilder builder) {
      ...
      if (definiteAssignmentTrackers.TryGetValue(expr.Var.UniqueName, out ie)) {
        builder.Add(Assert(GetTok(expr), ie, new PODesc.DefiniteAssignment($"variable '{expr.Var.Name}'", "here")));
      }
    }
      ...
      foreach (var formal in f.Formals.Where(formal => formal.DefaultValue != null)) {
        ...
        if (formal.IsOld) {
          ...
          if (wh != null) {
            ...
            builder.Add(Assert(GetTok(e), wh, desc));
          }
        }
      }
    ...
    Bpl.Expr CanCallAssumption(Expression expr, ExpressionTranslator etran) {
      ...
      if (expr is LiteralExpr || expr is ThisExpr || expr is IdentifierExpr || expr is WildcardExpr || expr is BoogieWrapper) {
        ...
        if (!(e.Function is SpecialFunction)) {
          ...
          Bpl.Expr canCallFuncAppl = new Bpl.NAryExpr(GetTok(expr), new Bpl.FunctionCall(canCallFuncID), args);
        ...
        if (expr is MapComprehension mc && mc.IsGeneralMapComprehension) {
          ...
          substMap = new Dictionary<IVariable, Expression>();
          for (var i = 0; i < mc.BoundVars.Count; i++) {
            var p = new Bpl.NAryExpr(GetTok(mc), new Bpl.FunctionCall(mc.ProjectionFunctions[i]), new List<Bpl.Expr> { F });
      ...
      if (expr is StaticReceiverExpr stexpr) {
        ...
        if (e.Obj.Type.IsRefType) {
          ...
          if (dtor.EnclosingCtors.Count == dtor.EnclosingCtors[0].EnclosingDatatype.Ctors.Count) {
            ...
            builder.Add(Assert(GetTok(expr), correctConstructor,
        ...
        if (!e.Member.IsStatic) {
          if (e.Member is TwoStateFunction) {
            ...
            if (wh != null) {
              ...
              builder.Add(Assert(GetTok(expr), wh, desc));
            }
        ...
        if (eSeqType.IsArrayType) {
          builder.Add(Assert(GetTok(e.Seq), Bpl.Expr.Neq(seq, predef.Null), new PODesc.NonNull("array")));
          if (!CommonHeapUse || etran.UsesOldHeap) {
            builder.Add(Assert(GetTok(e.Seq), MkIsAlloc(seq, eSeqType, etran.HeapExpr), new PODesc.IsAllocated("array", null)));
          }
        }
        Bpl.Expr e0 = null;
        if (eSeqType is MapType) {
          ...
          builder.Add(Assert(GetTok(expr), inDomain, new PODesc.ElementInDomain(), options.AssertKv));
          ...
          if (e.E0 != null) {
            ...
            builder.Add(Assert(GetTok(expr), InSeqRange(expr.tok, e0, e.E0.Type, seq, isSequence, null, !e.SelectOne), desc, options.AssertKv));
          }
          if (e.E1 != null) {
            ...
            builder.Add(Assert(GetTok(expr), InSeqRange(expr.tok, etran.TrExpr(e.E1), e.E1.Type, seq, isSequence, lowerBound, true), new PODesc.SequenceSelectRangeValid(isSequence ? "sequence" : "array"), options.AssertKv));
          }
        }
        ...
        builder.Add(Assert(GetTok(e.Array), Bpl.Expr.Neq(array, predef.Null), new PODesc.NonNull("array")));
        if (!CommonHeapUse || etran.UsesOldHeap) {
          builder.Add(Assert(GetTok(e.Array), MkIsAlloc(array, e.Array.Type, etran.HeapExpr), new PODesc.IsAllocated("array", null)));
        }
        ...
        if (e.ResolvedUpdateExpr != null) {
          ...
          if (collectionType is SeqType) {
            ...
            builder.Add(Assert(GetTok(e.Index), InSeqRange(expr.tok, index, e.Index.Type, seq, true, null, false), desc, options.AssertKv));
          } else {
          ...
          if (collectionType is SeqType) {
            ...
            builder.Add(Assert(GetTok(e.Value), Bpl.Expr.Le(Bpl.Expr.Literal(0), value), desc, options.AssertKv));
          } else {
            Contract.Assert(false);
          }
        }
        ...
        if (etran.UsesOldHeap) {
          ...
          if (wh != null) {
            ...
            builder.Add(Assert(GetTok(e.Function), wh, desc));
          }
          for (int i = 0; i < e.Args.Count; i++) {
            ...
            if (wh != null) {
              ...
              builder.Add(Assert(GetTok(ee), wh, desc));
            }
          }
        }
        ...
        if (!fnCoreType.IsArrowTypeWithoutPreconditions) {
          ...
          builder.Add(Assert(GetTok(expr), precond, new PODesc.PreconditionSatisfied(null)));
        }
          ...
          if (etran.UsesOldHeap) {
            if (!e.Function.IsStatic) {
              ...
              if (wh != null) {
                ...
                builder.Add(Assert(GetTok(e.Receiver), wh, desc));
              }
            }
            for (int i = 0; i < e.Args.Count; i++) {
              Expression ee = e.Args[i];
          ...
          if (codeContext != null && e.CoCall != FunctionCallExpr.CoCallResolution.Yes && !(e.Function is ExtremePredicate)) {
            ...
            if (ModuleDefinition.InSameSCC(calleeSCCLookup, codeContext)) {
              if (options.DoOnlyCoarseGrainedTerminationChecks) {
                builder.Add(Assert(GetTok(expr), Bpl.Expr.False, new PODesc.IsNonRecursive()));
              } else {
        ...
        builder.Add(Assert(GetTok(e.N), Bpl.Expr.Le(Bpl.Expr.Literal(0), etran.TrExpr(e.N)), desc));
        ...
        foreach (var fe in e.Frame) {
          ...
          if (ty.IsNonNullRefType) {
            ...
            builder.Add(Assert(GetTok(fe.E), BplImp(ante, nonNull), new PODesc.NonNull(description, description != "object")));
          }
          ...
          if (wh != null) {
            var desc = new PODesc.IsAllocated(description, "in the old-state of the 'unchanged' predicate",
              description != "object");
            builder.Add(Assert(GetTok(fe.E), BplImp(BplAnd(ante, nonNull), wh), desc));
          }
        }
      } else if (expr is UnaryExpr) {
        ...
        switch (e.ResolvedOp) {
          ...
          case BinaryExpr.ResolvedOpcode.Mul:
            ...
            if (e.ResolvedOp == BinaryExpr.ResolvedOpcode.Sub && e.E0.Type.IsBigOrdinalType) {
              ...
              builder.Add(Assert(GetTok(expr), rhsIsNat, new PODesc.OrdinalSubtractionIsNatural()));
              ...
              builder.Add(Assert(GetTok(expr), Bpl.Expr.Le(offset1, offset0), new PODesc.OrdinalSubtractionUnderflow()));
              ...
              if (e.ResolvedOp == BinaryExpr.ResolvedOpcode.Add) {
                builder.Add(Assert(GetTok(expr), Bpl.Expr.Lt(Bpl.Expr.Binary(BinaryOperator.Opcode.Add, e0, e1), Bpl.Expr.Literal(65536)), new PODesc.CharOverflow()));
              } else {
                Contract.Assert(e.ResolvedOp == BinaryExpr.ResolvedOpcode.Sub);  // .Mul is not supported for char
                builder.Add(Assert(GetTok(expr), Bpl.Expr.Le(e1, e0), new PODesc.CharUnderflow()));
              }
            }
            CheckResultToBeInType(expr.tok, expr, expr.Type, locals, builder, etran);
            break;
          ...
          case BinaryExpr.ResolvedOpcode.Mod: {
              ...
              CheckWellformed(e.E1, options, locals, builder, etran);
              builder.Add(Assert(GetTok(expr), Bpl.Expr.Neq(etran.TrExpr(e.E1), zero), new PODesc.DivisorNonZero(), options.AssertKv));
              CheckResultToBeInType(expr.tok, expr, expr.Type, locals, builder, etran);
            }
            break;
          ...
          case BinaryExpr.ResolvedOpcode.RightShift: {
              ...
              if (e.E1.Type.IsBitVectorType) {
                ...
                if (w < (BigInteger.One << e1Width)) {
                  ...
                  builder.Add(Assert(GetTok(expr), cmp, upperDesc, options.AssertKv));
                } else {
                ...
                builder.Add(Assert(GetTok(expr), Bpl.Expr.Le(Bpl.Expr.Literal(0), etran.TrExpr(e.E1)), positiveDesc, options.AssertKv));
                builder.Add(Assert(GetTok(expr), Bpl.Expr.Le(etran.TrExpr(e.E1), Bpl.Expr.Literal(w)), upperDesc, options.AssertKv));
              }
            }
            break;
          default:
        ...
        switch (e.Op) {
          ...
          case TernaryExpr.Opcode.PrefixNeqOp:
            if (e.E0.Type.IsNumericBased(Type.NumericPersuasion.Int)) {
              builder.Add(Assert(GetTok(expr), Bpl.Expr.Le(Bpl.Expr.Literal(0), etran.TrExpr(e.E0)), new PODesc.PrefixEqualityLimit(), options.AssertKv));
            }
            break;
          default:
          ...
          BplIfIf(e.tok, true, typeAntecedents, nextBuilder, newBuilder => {
            ...
            if (mc != null) {
              ...
              BplIfIf(e.tok, guard != null, BplAnd(guard, guardPrime), newBuilder, b => {
                ...
                b.Add(Assert(GetTok(mc.TermLeft), different, new PODesc.ComprehensionNoAlias()));
              });
            }
          });
        ...
        foreach (var missingCtor in me.MissingCases) {
          ...
          b.Add(Assert(GetTok(me), Bpl.Expr.False, new PODesc.MatchIsComplete("expression", missingStr)));
    ...
    void CheckWellformedSpecialFunction(FunctionCallExpr expr, WFOptions options, Bpl.Expr result, Type resultType, List<Bpl.Variable> locals,
      ...
      if (name == "RotateLeft" || name == "RotateRight") {
        ...
        builder.Add(Assert(GetTok(expr), Bpl.Expr.Le(Bpl.Expr.Literal(0), etran.TrExpr(arg)), new PODesc.ShiftLowerBound(), options.AssertKv));
        builder.Add(Assert(GetTok(expr), Bpl.Expr.Le(etran.TrExpr(arg), Bpl.Expr.Literal(w)), new PODesc.ShiftUpperBound(w), options.AssertKv));
      }
    }
    ...
    Bpl.Expr CheckWellformedLetExprWithResult(LetExpr e, WFOptions options, Bpl.Expr result, Type resultType, List<Bpl.Variable> locals,
      ...
      if (e.Exact) {
        ...
        if (e.Constraint_Bounds != null) {
          ...
          builder.Add(Assert(GetTok(e), etran.TrExpr(eq), new PODesc.LetSuchThanUnique()));
        }
    ...
    void CheckFunctionSelectWF(string what, BoogieStmtListBuilder builder, ExpressionTranslator etran, Expression e, string hint) {
      if (e is MemberSelectExpr sel && sel.Member is Function fn) {
        ...
        builder.Add(Assert(GetTok(e), assertion, new PODesc.ValidInRecursion(what, hint)));
      }
    }
    ...
    void AssertDistinctness(Expression lhsa, Expression lhsb, BoogieStmtListBuilder builder, ExpressionTranslator etran) {
      ...
      if (e != null) {
        builder.Add(Assert(GetTok(lhsa), e, new PODesc.DistinctLHS(Printer.ExprToString(lhsa),
      ...
      if (rhs is ExprRhs) {
        ...
        if (tRhs.ArrayDimensions == null) {
          ...
          foreach (Expression dim in tRhs.ArrayDimensions) {
            ...
            builder.Add(Assert(GetTok(dim), Bpl.Expr.Le(Bpl.Expr.Literal(0), etran.TrExpr(dim)), desc));
            i++;
          }
          if (tRhs.ElementInit != null) {
            ...
            builder.Add(Assert(GetTok(dim), Bpl.Expr.Eq(etran.TrExpr(dim), Bpl.Expr.Literal(tRhs.InitDisplay.Count)), desc));
    ...
    private bool TrSplitFunctionCallExpr(Expression expr, List<SplitExprInfo> splits, int heightLimit, bool inlineProtectedFunctions,
      ...
      if (functionHeight < heightLimit && f.Body != null && RevealedInScope(f)) {
        if (RefinementToken.IsInherited(fexp.tok, currentModule) &&
          ...
          Bpl.Expr canCall = new Bpl.NAryExpr(GetTok(expr), new Bpl.FunctionCall(canCallFuncID), args);
