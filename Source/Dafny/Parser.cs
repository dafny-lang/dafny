...
ref Attributes readsAttrs, ref Attributes modAttrs, ref Attributes decrAttrs) {
		...
		while (!(StartOf(16))) {SynErr(187); Get();}
		if (la.kind == 48) {
			Get();
			while (IsAttribute()) {
...
List<Expression> decreases, ref Attributes decAttrs, ref Attributes modAttrs, string caption) {
		...
		while (!(StartOf(19))) {SynErr(190); Get();}
		if (la.kind == 47) {
			Get();
			while (IsAttribute()) {
				Attribute(ref modAttrs);
	...
	void FunctionSpec(List<Expression/*!*/>/*!*/ reqs, List<FrameExpression/*!*/>/*!*/ reads, List<Expression/*!*/>/*!*/ ens, List<Expression/*!*/> decreases) {
		...
		while (!(StartOf(20))) {SynErr(194); Get();}
		if (la.kind == 49) {
			Get();
			Expression(out e, false, false);
			OldSemi();
	...
	void OneStmt(out Statement/*!*/ s) {
		...
		while (!(StartOf(22))) {SynErr(198); Get();}
		switch (la.kind) {
		case 50: {
	...
	void CaseStatement(out MatchCaseStmt/*!*/ c) {
		...
		while (IsNotEndOfCase()) {
			Stmt(body);
			while (!(StartOf(31))) {SynErr(236); Get();}
		}
