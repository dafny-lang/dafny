...
namespace Microsoft.Dafny {
  ...
  class tobiRewriter : IRewriter {
    internal override void PostResolve(Program program) {
      base.PostResolve(program);
      foreach (var moduleDefinition in program.Modules()) {
        foreach (var topLevelDecl in moduleDefinition.TopLevelDecls.OfType<TopLevelDeclWithMembers>()) {
          foreach (var function in topLevelDecl.Members.OfType<Function>()) {
            if (function.Body is NestedMatchExpr matchExpr) {
              var idPatterns = matchExpr.Cases.Select(matchCase => matchCase.Pat).OfType<IdPattern>();
              foreach (var idPattern in idPatterns) {
                if (char.IsLower(idPattern.Id[0]) == false && idPattern.Arguments == null) {
                  var lineNum = idPattern.Tok.line;
                  string message = "Variable name '" + idPattern + "' on line " + lineNum +
                                   " starts with a capital letter";
                  this.Reporter.Warning(MessageSource.Rewriter, matchExpr.tok, message);
                }
              }
            }
          }
    ...
    // Implementation
    // Approach - Produce a warning whenever a variable in a case expression starts with a capital
    // Implementation - Traverse through all cases and check every variable name to ensure the variable names start with a capital letter, if not display constructor match warning
    public tobiRewriter(ErrorReporter reporter) : base(reporter) {
    }
  }
