...
//			if (ch == EOL) { line++; col = 0; }
	...
	void CheckLiteral() {
		switch (t.val) {
			...
			case "match": t.kind = 92; break;
			case "assert": t.kind = 93; break;
			case "print": t.kind = 94; break;
			case "forall": t.kind = 95; break;
			case "parallel": t.kind = 96; break;
			case "modify": t.kind = 97; break;
			case "exists": t.kind = 118; break;
			case "in": t.kind = 120; break;
			case "false": t.kind = 125; break;
			case "true": t.kind = 126; break;
			case "null": t.kind = 127; break;
			case "this": t.kind = 128; break;
			case "fresh": t.kind = 129; break;
			case "old": t.kind = 130; break;
			case "then": t.kind = 131; break;
			default: break;
		}
	}
	  ...
	  Contract.Ensures(Contract.Result<Token>() != null);
		...
		switch (state) {
			...
			case 21:
				...
				else if (ch == 92) {AddCh(); goto case 52;}
				else {goto case 0;}
			case 22:
			...
			case 43:
				if (ch <= '&' || ch >= '(' && ch <= '/' || ch >= ':' && ch <= '>' || ch == '@' || ch >= '[' && ch <= '^' || ch == '`' || ch >= '{' && ch <= 65535) {apx++; AddCh(); goto case 44;}
				else {goto case 0;}
			case 44:
				{
					tlen -= apx;
			...
			case 50:
				recEnd = pos; recKind = 1;
				if (ch == 39 || ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 50;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 51:
			...
			case 61:
				...
				else if (ch == 'y') {AddCh(); goto case 62;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 62:
				recEnd = pos; recKind = 5;
				if (ch == 39 || ch == '0' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 6;}
				else if (ch >= '1' && ch <= '9') {AddCh(); goto case 63;}
				else {t.kind = 5; break;}
			case 63:
				recEnd = pos; recKind = 5;
				if (ch == 39 || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 50;}
