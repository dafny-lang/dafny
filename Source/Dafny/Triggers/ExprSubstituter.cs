    ...
    public override Expression Substitute(Expression expr) {
      ...
      if (expr is QuantifierExpr e) {
        ...
        foreach (var entry in usedSubstMap) {
          var eq = new BinaryExpr(Format.Generated(e.tok), BinaryExpr.ResolvedOpcode.EqCommon, entry.Item2, entry.Item1);
          newRange = newRange == null ? eq : new BinaryExpr(Format.Generated(e.tok), BinaryExpr.ResolvedOpcode.And, eq, newRange);
        ...
        if (expr is ForallExpr) {
          newExpr = new ForallExpr(e.tok, e.BodyEndTok, e.TypeArgs, newBoundVars, newRange, newTerm, newAttrs) { Bounds = newBounds };
        } else {
          Contract.Assert(expr is ExistsExpr);
          newExpr = new ExistsExpr(e.tok, e.BodyEndTok, e.TypeArgs, newBoundVars, newRange, newTerm, newAttrs) { Bounds = newBounds };
        }
        usedSubstMap.Clear();
