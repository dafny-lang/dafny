...
public class ProjectFilesTest : ClientBasedLanguageServerTest {
  ...
  public async Task ProjectFileErrorIsShown() {
    ...
    await CreateOpenAndResolveTestDocument(projectFileSource, DafnyProject.FileName);
  ...
  public async Task ProjectFileErrorIsShownFromDafnyFile() {
    ...
    await CreateOpenAndResolveTestDocument("method Foo() { }", Path.Combine(directory, "ProjectFileErrorIsShownFromDafnyFile.dfy"));
    var diagnostics = await diagnosticsReceiver.AwaitNextNotificationAsync(CancellationToken);
...
module [>Producer<]Oops {
  const x := 3
    ...
    var producer = await CreateOpenAndResolveTestDocument(producerSource, Path.Combine(tempDirectory, "producer.dfy"));
...
module Consumer {
  ...
  const y := x + 2
    ...
    var consumer = await CreateOpenAndResolveTestDocument(consumerSource, Path.Combine(tempDirectory, "consumer.dfy"));
    await CreateOpenAndResolveTestDocument("", Path.Combine(tempDirectory, DafnyProject.FileName));
  ...
  public async Task ProjectFileByItselfDiagnostics() {
    var tempDirectory = Path.GetRandomFileName();
    var projectFile = await CreateOpenAndResolveTestDocument("", Path.Combine(tempDirectory, DafnyProject.FileName));
...
function-syntax = 4";
    ...
    var noProjectFile = await CreateOpenAndResolveTestDocument(source, "orphaned.dfy");
    ...
    var sourceFile = await CreateOpenAndResolveTestDocument(source, Path.Combine(directory, "source.dfy"));
