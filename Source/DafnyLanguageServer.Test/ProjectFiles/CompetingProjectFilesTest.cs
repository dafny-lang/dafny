...
public class CompetingProjectFilesTest : ClientBasedLanguageServerTest {
  ...
  public async Task ProjectFileDoesNotOwnAllSourceFilesItUses() {
    ...
    Assert.Equal(outerProject.Uri, diagnostics.Uri);
    Assert.Contains("the referenced file", diagnostics.Diagnostics.First().Message);
  }
...
method Foo() {
  ...
  /// </summary>
  [Fact]
  public async Task NewProjectFileGrabsSourceFileOwnershipAfterEditOld() {
    var tempDirectory = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
    var nestedDirectory = Path.Combine(tempDirectory, "nested");
    Directory.CreateDirectory(nestedDirectory);
...
warn-shadowing = true
".TrimStart();
    var outerProject = await CreateAndOpenTestDocument(warnsShadowingProject, Path.Combine(tempDirectory, DafnyProject.FileName));

    var sourceFile = await CreateAndOpenTestDocument(hasShadowingSource, sourceFilePath);
    ...
    ApplyChange(ref outerProject, new Range(0, 0, 0, 0), "\n");
  ...
  /// </summary>
  [Fact]
  public async Task NewProjectFileGrabsSourceFileOwnershipAfterEditNew() {
    var tempDirectory = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
    var nestedDirectory = Path.Combine(tempDirectory, "nested");
    Directory.CreateDirectory(nestedDirectory);
    var sourceFilePath = Path.Combine(nestedDirectory, "source.dfy");
