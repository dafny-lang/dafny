...
".TrimStart();
      ...
      var project = await CreateOpenAndResolveTestDocument("", Path.Combine(directory, DafnyProject.FileName));
      var producer = await CreateOpenAndResolveTestDocument(producerSource, Path.Combine(directory, "producer.dfy"));
      var consumer = await CreateOpenAndResolveTestDocument(consumerSource, Path.Combine(directory, "consumer.dfy"));
...
method {:vcs_split_on_every_assert} Foo(x: int) {
  if (x == 0) {
    ...
    assert false;
      ...
      await CreateOpenAndResolveTestDocument(source);
