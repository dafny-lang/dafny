...
public class ProjectManagerDatabaseTest : ClientBasedLanguageServerTest {
  ...
  public async Task OpenAndCloseConcurrentlySeparateProjects() {
    ...
    for (int i = 0; i < documentsToLoadConcurrently; i++) {
      var documentItem = await CreateOpenAndResolveTestDocument(source, $"pmdtest1_{i}.dfy");
  ...
  public async Task ConcurrentProjectMigration() {
    ...
    for (int i = 0; i < documentsToLoadConcurrently; i++) {
      var documentItem = await CreateOpenAndResolveTestDocument(source, Path.Combine(directory, $"nested{i}/pmdtest2.dfy"));
      loadingDocuments.Add(documentItem);
    }

    // Create a project file for each test document.
    for (int i = 0; i < documentsToLoadConcurrently; i++) {
      await CreateOpenAndResolveTestDocument("", Path.Combine(directory, $"nested{i}/{DafnyProject.FileName}"));
    }
  ...
  public async Task OpenAndCloseConcurrentlySameProject() {
    ...
    for (int i = 0; i < documentsToLoadConcurrently; i++) {
      var documentItem = await CreateOpenAndResolveTestDocument(source, Path.Combine(directory, $"pmdtest3_{i}.dfy"));
      loadingDocuments.Add(documentItem);
    }
