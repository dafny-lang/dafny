...
".TrimStart();
      ...
      var documentItem = await CreateOpenAndResolveTestDocument(source);
    ...
    public async Task InvalidNewNameIsNoOp() {
      var documentItem = await CreateOpenAndResolveTestDocument("");
    ...
    public async Task RenameNonSymbolFails() {
      ...
      var documentItem = await CreateOpenAndResolveTestDocument("module Foo {}", Path.Combine(tempDir, "tmp.dfy"));
...
".TrimStart();
    ...
    private async Task AssertRangesRenamed(IEnumerable<string> sources, string tempDir, string newName) {
      var items = sources.Select(async (source, sourceIndex) => {
        ...
        var documentItem = await CreateOpenAndResolveTestDocument(cleanSource, Path.Combine(tempDir, $"tmp{sourceIndex}.dfy"));
