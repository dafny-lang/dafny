...
namespace Microsoft.Dafny.LanguageServer.Language {
  ...
  /// </remarks>
  public sealed class DafnyLangParser(
    DafnyOptions options,
    IFileSystem fileSystem,
    TelemetryPublisherBase telemetryPublisher,
    ILogger<DafnyLangParser> logger,
    ILogger<CachingParser> innerParserLogger)
    : IDafnyParser {
    private readonly DafnyOptions options = options;
    private readonly IFileSystem fileSystem = fileSystem;
    private readonly ILogger<DafnyLangParser> logger = logger;
    private readonly SemaphoreSlim mutex = new(1);
    private readonly ProgramParser programParser = options.Get(DafnyLangSymbolResolver.UseCaching)
