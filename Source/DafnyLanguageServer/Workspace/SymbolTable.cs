...
public class SymbolTable {
  ...
  public static SymbolTable CreateFrom(ILogger logger, Program program, CancellationToken cancellationToken) {
    ...
    var udt = visited.Where(n => n.ToString().Contains("Option<int>"));

    var notGenerated = visited.OfType<IHasReferences>().Where(v => !AutoGeneratedOrigin.Is(v.NavigationToken));
    var references = notGenerated
      .SelectMany(r => {
    ...
    var ud2 = references.Where(r => r.Referer.ToString().Contains("Option<int>"));
  ...
  private SymbolTable(IReadOnlyList<Reference> references, IReadOnlyList<IHasNavigationToken> hasNavigationTokens) {
    ...
    var symbols = hasNavigationTokens
      ...
      .Where(s => !AutoGeneratedOrigin.Is(s.NavigationToken)).Distinct();
