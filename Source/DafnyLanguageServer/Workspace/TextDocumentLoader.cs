...
#pragma warning restore CS1998
    ...
    private CompilationAfterResolution ResolveInternal(CompilationAfterParsing compilation, CancellationToken cancellationToken) {
      ...
      var symbols = program.Modules().Where(m => m is not DefaultModuleDefinition).SelectMany(SymbolExtensions.GetSymbols);
      var verifiables = symbols.OfType<ICanVerify>().Where(s => !AutoGeneratedToken.Is(s.RangeToken)).ToList();
