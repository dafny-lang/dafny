  ...
  private static IToken? GetMatchingEndToken(Dafny.Program program, string documentUri, int line, int col) {
    ...
    foreach (var module in program.Modules()) {
      foreach (var topLevelDecl in module.TopLevelDecls) {
        if (topLevelDecl is ClassLikeDecl classDecl &&
            (classDecl.StartToken.line == 0 ||
             (classDecl.StartToken.Filepath == documentUri && classDecl.StartToken.line <= line && line <= classDecl.EndToken.line))) {
