...
function MultiSet#FromSeq(Seq Box): MultiSet Box uses {
  axiom MultiSet#FromSeq(Seq#Empty(): Seq Box) == MultiSet#Empty(): MultiSet Box;
}
...
// update axiom
axiom (forall s: Seq Box, i: int, v: Box, x: Box ::
...
axiom (forall s: Seq Box, x: Box :: { MultiSet#FromSeq(s)[x] }
...
function Seq#Length(Seq Box): int;
axiom (forall s: Seq Box :: { Seq#Length(s) } 0 <= Seq#Length(s));

function Seq#Empty(): Seq Box uses {
  axiom Seq#Length(Seq#Empty(): Seq Box) == 0;
}

axiom (forall s: Seq Box :: { Seq#Length(s) }
...
//axiom (forall t: Ty :: {$Is(Seq#Empty(): Seq Box, TSeq(t))} $Is(Seq#Empty(): Seq Box, TSeq(t)));

function Seq#Singleton(Box): Seq Box;
axiom (forall t: Box :: { Seq#Length(Seq#Singleton(t)) } Seq#Length(Seq#Singleton(t)) == 1);

function Seq#Build(s: Seq Box, val: Box): Seq Box;
function Seq#Build_inv0(s: Seq Box) : Seq Box;
function Seq#Build_inv1(s: Seq Box) : Box;
axiom (forall s: Seq Box, val: Box ::
  { Seq#Build(s, val) }
...
axiom (forall s: Seq Box, i: int, v: Box :: { Seq#Index(Seq#Build(s,v), i) }
...
function Seq#Append(Seq Box, Seq Box): Seq Box;
axiom (forall s0: Seq Box, s1: Seq Box :: { Seq#Length(Seq#Append(s0,s1)) }
  Seq#Length(Seq#Append(s0,s1)) == Seq#Length(s0) + Seq#Length(s1));

function Seq#Index(Seq Box, int): Box;
axiom (forall t: Box :: { Seq#Index(Seq#Singleton(t), 0) } Seq#Index(Seq#Singleton(t), 0) == t);
axiom (forall s0: Seq Box, s1: Seq Box, n: int :: { Seq#Index(Seq#Append(s0,s1), n) }
  (n < Seq#Length(s0) ==> Seq#Index(Seq#Append(s0,s1), n) == Seq#Index(s0, n)) &&
  (Seq#Length(s0) <= n ==> Seq#Index(Seq#Append(s0,s1), n) == Seq#Index(s1, n - Seq#Length(s0))));

function Seq#Update(Seq Box, int, Box): Seq Box;
...
function Seq#Contains(Seq Box, Box): bool;
axiom (forall s: Seq Box, x: Box :: { Seq#Contains(s,x) }
  Seq#Contains(s,x) <==>
...
axiom (forall s0: Seq Box, s1: Seq Box, x: Box ::
  { Seq#Contains(Seq#Append(s0, s1), x) }
  Seq#Contains(Seq#Append(s0, s1), x) <==>
    Seq#Contains(s0, x) || Seq#Contains(s1, x));

axiom (forall s: Seq Box, v: Box, x: Box ::  // needed to prove things like '4 in [2,3,4]', see method TestSequences0 in SmallTests.dfy
  { Seq#Contains(Seq#Build(s, v), x) }
    Seq#Contains(Seq#Build(s, v), x) <==> (v == x || Seq#Contains(s, x)));

axiom (forall s: Seq Box, n: int, x: Box ::
...
function Seq#Equal(Seq Box, Seq Box): bool;
axiom (forall s0: Seq Box, s1: Seq Box :: { Seq#Equal(s0,s1) }
  Seq#Equal(s0,s1) <==>
    Seq#Length(s0) == Seq#Length(s1) &&
    (forall j: int :: { Seq#Index(s0,j) } { Seq#Index(s1,j) }
        0 <= j && j < Seq#Length(s0) ==> Seq#Index(s0,j) == Seq#Index(s1,j)));
axiom (forall a: Seq Box, b: Seq Box :: { Seq#Equal(a,b) }  // extensionality axiom for sequences
  Seq#Equal(a,b) ==> a == b);

function Seq#SameUntil(Seq Box, Seq Box, int): bool;
axiom (forall s0: Seq Box, s1: Seq Box, n: int :: { Seq#SameUntil(s0,s1,n) }
  Seq#SameUntil(s0,s1,n) <==>
    (forall j: int :: { Seq#Index(s0,j) } { Seq#Index(s1,j) }
        0 <= j && j < n ==> Seq#Index(s0,j) == Seq#Index(s1,j)));

function Seq#Take(s: Seq Box, howMany: int): Seq Box;
axiom (forall s: Seq Box, n: int :: { Seq#Length(Seq#Take(s,n)) }
  0 <= n && n <= Seq#Length(s) ==> Seq#Length(Seq#Take(s,n)) == n);
axiom (forall s: Seq Box, n: int, j: int ::
  {:weight 25}
...
function Seq#Drop(s: Seq Box, howMany: int): Seq Box;
axiom (forall s: Seq Box, n: int :: { Seq#Length(Seq#Drop(s,n)) }
...
axiom (forall s: Seq Box, n: int, k: int ::
  {:weight 25}
  { Seq#Index(s, k), Seq#Drop(s,n) }
  0 <= n && n <= k && k < Seq#Length(s) ==>
    Seq#Index(Seq#Drop(s,n), k-n) == Seq#Index(s, k));

axiom (forall s, t: Seq Box, n: int ::
...
axiom (forall s: Seq Box, i: int, j: int ::
        { Seq#Rank(Seq#Append(Seq#Take(s, i), Seq#Drop(s, j))) }
        0 <= i && i < j && j <= Seq#Length(s) ==> Seq#Rank(Seq#Append(Seq#Take(s, i), Seq#Drop(s, j))) < Seq#Rank(s) );

// Additional axioms about common things
axiom (forall s: Seq Box, n: int :: { Seq#Drop(s, n) }
        n == 0 ==> Seq#Drop(s, n) == s);
axiom (forall s: Seq Box, n: int :: { Seq#Take(s, n) }
        n == 0 ==> Seq#Take(s, n) == Seq#Empty());
axiom (forall s: Seq Box, m, n: int :: { Seq#Drop(Seq#Drop(s, m), n) }
