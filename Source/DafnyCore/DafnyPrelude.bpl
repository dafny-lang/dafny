...
//axiom (forall<T> t: Ty :: {$Is(Seq#Empty(): Seq, TSeq(t))} $Is(Seq#Empty(): Seq, TSeq(t)));

function Seq#Singleton(Box): Seq;
axiom (forall x: Box :: { Seq#Length(Seq#Singleton(x)) } Seq#Length(Seq#Singleton(x)) == 1);
...
axiom (forall ty: Ty, heap: Heap, len: int, init: HandleType ::
  { Seq#Length(Seq#Create(ty, heap, len, init)) }
