...
namespace Microsoft.Dafny.Compilers {
  ...
  public abstract class SinglePassCompiler : Plugins.Compiler {
      ...
      if (c is not TraitDecl || TraitRepeatsInheritedDeclarations) {
        ...
        foreach (var member in inheritedMembers.Select(memberx => (memberx as Function)?.ByMethodDecl ?? memberx)) {
          ...
          if (member.IsGhost) {
            ...
            var cfType = TypeUtil.SubstType(cf.Type, c.ParentFormalTypeParametersToActuals);
            if (cf.Rhs == null) {
            ...
            var fType = TypeUtil.SubstType(f.Type, c.ParentFormalTypeParametersToActuals);
    ...
    protected void EmitCallToInheritedConstRHS(ConstantField f, ConcreteSyntaxTree wr) {
      ...
      var calleeReceiverType = TypeUtil.SubstType(UserDefinedType.FromTopLevelDecl(f.tok, f.EnclosingClass), thisContext.ParentFormalTypeParametersToActuals);
    ...
    protected void EmitCallToInheritedMethod(Method method, ConcreteSyntaxTree wr) {
      ...
      var calleeReceiverType = TypeUtil.SubstType(UserDefinedType.FromTopLevelDecl(method.tok, method.EnclosingClass), thisContext.ParentFormalTypeParametersToActuals);
      ...
      if (pat.Var != null) {
        ...
        for (int i = 0; i < pat.Arguments.Count; i++) {
          ...
          if (formal.IsGhost) {
            ...
            Type targetType = TypeUtil.SubstType(formal.Type, substMap);
      ...
      if (expr is LetExpr) {
        ...
        if (!e.Function.IsStatic) {
          ...
          if (thisContext == null) {
            ...
            var instantiatedType = TypeUtil.SubstType(e.Receiver.Type, thisContext.ParentFormalTypeParametersToActuals);
    ...
    private static Type TypeOfLhs(Expression lhs) {
      ...
      if (lhs is IdentifierExpr) {
        ...
        return TypeUtil.SubstType(((Field)e.Member).Type, e.TypeArgumentSubstitutionsWithParents());
      } else if (lhs is SeqSelectExpr) {
      ...
      if (s.Method == enclosingMethod && enclosingMethod.IsTailRecursive) {
        ...
        for (int i = 0; i < s.Method.Outs.Count; i++) {
          ...
          if (!p.IsGhost) {
            ...
            outTmps.Add(target);
            var instantiatedType = TypeUtil.SubstType(p.Type, s.MethodSelect.TypeArgumentSubstitutionsWithParents());
            Type type;
        ...
        if (returnStyleOutCollector == null && outTmps.Count == 1 && returnStyleOuts) {
          var instantiatedFromType = TypeUtil.SubstType(outFormalTypes[0], s.MethodSelect.TypeArgumentSubstitutionsWithParents());
        ...
        for (int i = 0; i < s.Method.Ins.Count; i++) {
          ...
          if (!p.IsGhost) {
            ...
            var instantiatedToType = TypeUtil.SubstType(toType, s.MethodSelect.TypeArgumentSubstitutionsWithParents());
    ...
    void TrTailCallStmt(IToken tok, Method method, Expression receiver, List<Expression> args, string receiverReplacement, ConcreteSyntaxTree wr) {
      ...
      if (!method.IsStatic) {
        ...
        if (thisContext == null) {
          ...
          var instantiatedType = TypeUtil.SubstType(receiver.Type, thisContext.ParentFormalTypeParametersToActuals);
    ...
    protected internal void TrExpr(Expression expr, ConcreteSyntaxTree wr, bool inLetExprBody, ConcreteSyntaxTree wStmts) {
      ...
      if (expr is LiteralExpr) {
        ...
        if (thisContext != null) {
          var instantiatedType = TypeUtil.SubstType(expr.Type, thisContext.ParentFormalTypeParametersToActuals);
        ...
        if (sf != null) {
          ...
          if (!customReceiver && !e.Member.IsStatic) {
            ...
            if (e.Member is Function && typeArgs.Count != 0) {
              ...
              wr = CreateIIFE_ExprBody(etaReceiver, e.Obj.Type, e.Obj.tok, e.Obj, inLetExprBody, TypeUtil.SubstType(e.Type, typeMap), e.tok, wr, ref wStmts);
            ...
            if (customReceiver && e.Member is Function) {
              ...
              wr = CreateIIFE_ExprBody(customReceiverName, e.Obj.Type, e.Obj.tok, e.Obj, inLetExprBody, TypeUtil.SubstType(e.Type, typeMap), e.tok, wr, ref wStmts);
            }
      ...
      if (pat.Var != null) {
        ...
        for (int i = 0; i < pat.Arguments.Count; i++) {
          ...
          if (formal.IsGhost) {
            ...
            wr = TrCasePattern(arg, sw.ToString(), TypeUtil.SubstType(formal.Type, typeSubst), bodyType, wr, ref wStmts);
            k++;
          }
        }
      }
    ...
    void CompileFunctionCallExpr(FunctionCallExpr e, ConcreteSyntaxTree wr, bool inLetExprBody,
      ...
      var toType = thisContext == null ? e.Type : TypeUtil.SubstType(e.Type, thisContext.ParentFormalTypeParametersToActuals);
      ...
      for (int i = 0; i < e.Args.Count; i++) {
        if (!e.Function.Formals[i].IsGhost) {
          ...
          var instantiatedToType = TypeUtil.SubstType(e.Function.Formals[i].Type, e.TypeArgumentSubstitutionsWithParents());
