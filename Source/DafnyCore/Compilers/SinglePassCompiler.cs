...
namespace Microsoft.Dafny.Compilers {
  ...
  public abstract class ConcreteSinglePassCompiler : GenericSinglePassCompiler<ICanRender, ConcreteSyntaxTree, ICanRender, ICanRender>, Plugins.Compiler {
    ...
    protected override ConcreteSyntaxTree VariableDeclaration(string name, Type type, IToken token, ICanRender initialValue) {
      var result = new ConcreteSyntaxTree();
      DeclareLocalVar(name, type, token, false, initialValue.ToString(), result);
      return result;
    }

    protected override ICanRender AsStatement(ICanRender expression) {
      return expression;
    }

    protected override ICanRender Variable(string name) {
      return new LineSegment(name);
    }
    ...
    protected ConcreteSyntaxTree EmitAssignment(ILvalue wLhs, Type lhsType /*?*/, Type rhsType /*?*/,
      ConcreteSyntaxTree wr, IToken tok) {
      return ConvertToWriter(wr, value => Assignment(wLhs, lhsType, rhsType, value, tok));
    }
    ...
    protected override ConcreteSyntaxTree CreateStatementList() {
      return new ConcreteSyntaxTree();
    }

    protected override string EmitAssignmentLhs(Expression e, ConcreteSyntaxTree wr) {
      var wStmts = wr.Fork();
    ...
    protected override ILvalue MemberSelect(ICanRender obj, Type objType, MemberDecl member, List<TypeArgumentInstantiation> typeArgs, Dictionary<TypeParameter, Type> typeMap,
      Type expectedType, string additionalCustomParameter = null, bool internalAccess = false) {
      return EmitMemberSelect(writer => writer.Append(obj), objType, member, typeArgs, typeMap, expectedType,
        additionalCustomParameter, internalAccess);
    }
    ...
    protected internal void TrStmt(Statement stmt, ConcreteSyntaxTree wr, ConcreteSyntaxTree wStmts = null) {
      ...
      if (stmt is PrintStmt) {
        ...
        if (resolved.Count == 1) {
          ...
          List<ConcreteSyntaxTree> wRhss = lvalues.Select(_ => new ConcreteSyntaxTree()).ToList();
          wr.Append(MultiAssignment(lhss, lvalues, lhsTypes, wRhss, rhsTypes));
    ...
    protected virtual void EmitMultiSelect(AssignStmt s0, List<Type> tupleTypeArgsList, ConcreteSyntaxTree wr, string tup, int L) {
      ...
      var lvalue = ArraySelectAsLvalue(array, indices, tupleTypeArgsList[L - 1]);
      var rhs = new ConcreteSyntaxTree();
      wr.Append(lvalue.Write(rhs));
      EmitTupleSelect(tup, L - 1, rhs);
      EndStmt(wr);
    }
    ...
    private class CoercedLvalueImpl : ILvalue {
      ...
      public ICanRender Read() {
        return Compiler.CoercionIfNecessary(from, to, Token.NoToken, lvalue.Read());
      }

      public ICanRender Write(ICanRender value) {
        return lvalue.Write(value);
      }
    }
    ...
    ILvalue CreateLvalue(Expression lhs, ConcreteSyntaxTree wr, ConcreteSyntaxTree wStmts) {
      ...
      if (lhs is IdentifierExpr) {
        ...
        return ArraySelectAsLvalue(arr, new List<string>() { index }, ll.Type);
      } else {
        ...
        return ArraySelectAsLvalue(arr, indices, ll.Type);
      }
    }
      ...
      if (expr is LiteralExpr) {
        ...
        if (sf != null) {
          ...
          if (sf.IsStatic && !SupportsStaticsInGenericClasses && sf.EnclosingClass.TypeArgs.Count != 0) {
            ...
            wr.Append(EmitMemberSelect(writeObj, e.Obj.Type, e.Member, typeArgs, e.TypeArgumentSubstitutionsWithParents(), expr.Type).Read());
          }
          ...
          if (!customReceiver && !e.Member.IsStatic) {
            ...
            wr.Append(EmitMemberSelect(obj, e.Obj.Type, e.Member, typeArgs, typeMap, expr.Type).Read());
          } else {
            ...
            wr.Append(EmitMemberSelect(obj, e.Obj.Type, e.Member, typeArgs, typeMap, expr.Type, customReceiverName).Read());
          }
        }
