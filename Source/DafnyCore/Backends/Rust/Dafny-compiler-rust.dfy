...
module {:extern "DCOMP"} DafnyToRustCompiler {
  ...
  // To be used when escaping class fields, datatype constructors and their destructors' formals, externs, paths, function/method names, etc.
  // Subtlety: while datatype destructors in introduction position are escaped like this,
  // datatype destructors in elimination position are escaped using escapeDtor to avoid field punning (which Rust can't handle)
  // as a result, arms of datatype destruction must use escapeVar to get the non-punned variable (otherwise you'll get unresolved identifiers)
  ...
  // it accounts for variables generated by escapeDtor
  function escapeVar(f: Name): string {
    var r := f.dafny_name;
  ...
  // In case a field name clashes with a reserved Rust field, it can't use punning
  ...
  // Module-/type-level objects pass through normal name escaping
  function escapeType(n: Name): string {
    escapeName(n)
  }
  ...
  */
  ...
  class COMP {
    ...
    method GenModule(mod: Module, containingPath: seq<Ident>) returns (s: R.Mod)
      ...
      modifies this
    {
      var modName := escapeType(mod.name);
      if mod.body.None? {
    ...
    method GenTypeParam(tp: TypeArgDecl) returns (typeArg: Type, typeParam: R.TypeParamDecl)
    {
      ...
      typeParam := R.TypeParamDecl(escapeType(tp.name.id), genTpConstraint);
    }
      ...
      modifies this
    {
      ...
      s := [
        R.TraitDecl(
          R.Trait(
            typeParamDecls, R.TypeApp(R.TIdentifier(escapeType(t.name)), typeParams),
            parents,
            implBody
          ))];
    }
      ...
      var resultingType :=
        UserDefined(
          ResolvedType(
            ...
            c.attributes, [], []));
      var newtypeName := escapeType(c.name);
      s := [
        R.StructDecl(
          R.Struct(
            [
    ...
    {
      var typeParamsSeq, rTypeParams, rTypeParamsDecls, whereConstraints := GenTypeParameters(c.typeParams);
      var synonymTypeName := escapeType(c.name);
    ...
    {
      var typeParamsSeq, rTypeParams, rTypeParamsDecls, whereConstraints := GenTypeParameters(c.typeParams);
      var datatypeName := escapeType(c.name);
      ...
      for i := 0 to |c.ctors| {
        ...
        for j := 0 to |ctor.args| {
          ...
          var fieldName := escapeVar(dtor.formal.name);
    ...
    method GenType(c: Type, genTypeContext: GenTypeContext) returns (s: R.Type) {
      match c {
        ...
        case TypeArg(Ident(name)) => s := R.TIdentifier(escapeType(name));
        case Primitive(p) => {
          match p {
          ...
          case Some(outVars) => {
            if m.outVarsAreUninitFieldsToAssign {
              ...
              for outI := 0 to |outVars| {
                ...
                var outName := escapeVar(outVar.id);
      ...
      modifies this
    {
      match stmt {
        ...
        case Assign(lhs, expression) => {
          ...
          if lhs.Ident? {
            var rustId := escapeVar(lhs.ident.id);
        ...
        case Call(on, name, typeArgs, args, maybeOutVars) => {
          ...
          if maybeOutVars.Some? && |maybeOutVars.value| == 1 {
            var outVar := escapeVar(maybeOutVars.value[0].id);
            ...
            for outI := 0 to |outVars| {
              var outVar := escapeVar(outVars[outI].id);
      ...
      match e {
        ...
        case DatatypeValue(datatypeType, typeArgs, variant, isCo, values) => {
          ...
          for i := 0 to |values| {
            ...
            if isCo {
              ...
              assignments := assignments + [R.AssignIdentifier(escapeName(Name(name)), R.RawExpr(wasAssigned))];
            } else {
              var recursiveGen, _, recIdents := GenExpr(value, selfIdent, env, OwnershipOwned);

              assignments := assignments + [R.AssignIdentifier(escapeName(Name(name)), recursiveGen)];
              readIdents := readIdents + recIdents;
            }
          }
        ...
        case SelectFn(on, field, isDatatype, isStatic, isConstant, arguments) => {
          ...
          if isStatic {
            ...
            s := s + "| " + onString + "::" + escapeName(field) + (if isConstant then "()" else "") + "(";
            for i := 0 to |args| {
              if i > 0 {
                s := s + ", ";
              }
            ...
            s := s + "move |" + args + "| {\n";
            s := s + "callTarget." + escapeName(field) +
        ...
        case IIFE(name, tpe, value, iifeBody) => {
          ...
          readIdents := readIdents + (bodyIdents - {escapeVar(name.id)});
          r := R.Block(
            R.DeclareVar(R.CONST, escapeVar(name.id), Some(valueTypeGen), Some(valueGen))
            .Then(bodyGen));
          r, resultingOwnership := FromOwned(r, expectedOwnership);
          return;
        }
