...
public class AutoReqFunctionRewriter : IRewriter {
  ...
  Expression Andify(RangeToken tok, List<Expression> exprs) {
    Expression ret = Expression.CreateBoolLiteral(tok.MakeAutoGenerated(), true);
  ...
  List<Expression> GenerateAutoReqs(Expression expr) {
    ...
    if (expr is LiteralExpr) {
      ...
      foreach (MatchCaseExpr caseExpr in e.Cases) {
        ...
        MatchCaseExpr c = Expression.CreateMatchCase(caseExpr, Andify(caseExpr.RangeToken, GenerateAutoReqs(caseExpr.Body)));
        newMatches.Add(c);
      }

      reqs.Add(Expression.CreateMatch(e.RangeToken, e.Source, newMatches, e.Type));
      ...
      switch (e.ResolvedOp) {
        ...
        case BinaryExpr.ResolvedOpcode.Or:
          ...
          foreach (var req in GenerateAutoReqs(e.E1)) {
            ...
            reqs.Add(Expression.CreateImplies(Expression.CreateNot(e.E1.RangeToken, e.E0), req));
          }
          break;
      ...
      if (e.Exact) {
        ...
        if (new_reqs.Count > 0) {
          reqs.Add(Expression.CreateLet(e.RangeToken, e.LHSs, e.RHSs, Andify(e.RangeToken, new_reqs), e.Exact));
        }
      } else {
      ...
      if (auto_reqs.Count > 0) {
        Expression allReqsSatisfied = Andify(e.Term.RangeToken, auto_reqs);
      ...
      var auto_reqs = GenerateAutoReqs(e.Term);
      if (auto_reqs.Count > 0) {
        reqs.Add(Expression.CreateQuantifier(new ForallExpr(e.RangeToken, e.BoundVars, e.Range, Andify(e.Term.RangeToken, auto_reqs), e.Attributes), true));
      }
      ...
      reqs.Add(Expression.CreateITE(e.Test, Andify(e.Thn.RangeToken, GenerateAutoReqs(e.Thn)), Andify(e.Els.RangeToken, GenerateAutoReqs(e.Els))));
      ...
      var newCases = e.Cases.Select(cas =>
        new NestedMatchCaseExpr(cas.RangeToken, cas.Pat, Andify(cas.Body.RangeToken, GenerateAutoReqs(cas.Body)), cas.Attributes)).ToList();
      var nestedMatchExpr = new NestedMatchExpr(e.RangeToken, e.Source, newCases, e.UsesOptionalBraces);
