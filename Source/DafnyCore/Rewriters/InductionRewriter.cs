    ...
    if (args == null) {
      ...
      foreach (var arg in args) {
        ...
        if (ie != null) {
          ...
          if (0 <= j) {
            Reporter.Warning(MessageSource.Rewriter, ErrorId.rw_induction_arguments_quantifier_mismatch.ToString(), arg.tok,
          ...
          Reporter.Warning(MessageSource.Rewriter, ErrorId.rw_induction_arguments_lemma_mismatch.ToString(), arg.tok,
            "lemma parameters given as :induction arguments must be given in the same order as in the lemma; ignoring attribute");
          return;
        }

        Reporter.Warning(MessageSource.Rewriter, ErrorId.rw_invalid_induction_attribute.ToString(), arg.tok,
