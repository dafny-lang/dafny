...
public class AutoContractsRewriter : IRewriter {
  ...
  void ProcessClassPreResolve(ClassDecl cl) {
    ...
    if (!cl.Members.Exists(member => member is Field && member.Name == "Repr")) {
      ...
      var repr = new Field(cl.RangeToken.MakeAutoGenerated(), "Repr", true, ty, null);
      cl.Members.Add(repr);
    ...
    if (!cl.Members.Exists(member => member is Function && member.Name == "Valid" && !member.IsStatic)) {
      var valid = new Predicate(cl.RangeToken, "Valid", false, true, new List<TypeParameter>(), new List<Formal>(), null,
    ...
    foreach (var member in cl.Members) {
      ...
      var tok = member.RangeToken.MakeAutoGenerated();
  ...
  void ProcessClassPostResolve(ClassDecl cl) {
    ...
    var clTok = cl.RangeToken.MakeAutoGenerated();
      ...
      if (member is Function && member.Name == "Valid" && !member.IsStatic) {
        ...
        if (ctor.Body != null) {
          ...
          if (ctor.RefinementBase == null) {
            ...
            Statement s = new AssignStmt(member.RangeToken, Repr, new ExprRhs(e.RangeToken, e));
            s.IsGhost = true;
            sbs.AppendStmt(s);
          }
        ...
        if (member.IsGhost || IsSimpleQueryMethod(m)) {
          ...
          if (m.Mod.Expressions.Count == 0) {
            // modifies Repr
            m.Mod.Expressions.Add(new FrameExpression(Repr.RangeToken, Repr, null));
            format += "\nmodifies {1}";
  ...
  void AddSubobjectReprs(RangeToken tok, IToken endCurlyTok, List<Tuple<Field, Field, Function>> subobjects, BlockStmt block, int hoverTextFromHere,
    ...
    foreach (var ff in subobjects) {
      ...
      // Repr := Repr + ...;
      Statement s = new AssignStmt(tok, Repr, new ExprRhs(rhs.RangeToken, rhs));
      s.IsGhost = true;
  ...
  /// </summary>
  Expression IsNotNull(RangeToken tok, Expression expr) {
    Contract.Requires(tok != null);
  ...
  /// </summary>
  public static Expression ValidCall(RangeToken tok, Expression receiver, Function Valid, ICallable callingContext) {
    Contract.Requires(tok != null);
  ...
  public static BinaryExpr BinBoolExpr(RangeToken tok, BinaryExpr.ResolvedOpcode rop, Expression e0, Expression e1) {
