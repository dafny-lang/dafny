  ...
  void ProcessClassPreResolve(TopLevelDeclWithMembers cl) {
    ...
    foreach (var member in cl.Members) {
      ...
      if (member is Function && member.Name == "Valid" && !member.IsStatic) {
        ...
        valid.Ens.Insert(0, new AttributedExpression(tok, post));
        ...
        f.Req.Insert(0, new AttributedExpression(tok, valid));
        ...
        // ensures Valid();
        var valid = CreateUnresolvedValidCall(tok);
        ctor.Ens.Insert(0, new AttributedExpression(tok, valid));
        // ensures fresh(Repr);
        var freshness = new FreshExpr(tok, CreateUnresolvedThisRepr(tok));
        ctor.Ens.Insert(1, new AttributedExpression(tok, freshness));
      ...
      if (member is Function && member.Name == "Valid" && !member.IsStatic) {
        ...
        if (member.IsGhost || IsSimpleQueryMethod(m)) {
          if (m.RefinementBase == null) {
            ...
            m.Req.Insert(0, new AttributedExpression(tok, valid));
          ...
          // ensures Valid()
          m.Ens.Insert(0, new AttributedExpression(tok, valid));
          ...
          m.Ens.Insert(1, new AttributedExpression(tok, freshness));
