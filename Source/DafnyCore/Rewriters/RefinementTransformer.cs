...
namespace Microsoft.Dafny {
  public class RefinementOrigin : OriginWrapper {
    ...
    public static bool IsInherited(IOrigin tok, ModuleDefinition m) { // TODO inline
      return tok.IsInherited(m);
    }
    ...
    public override IOrigin WithVal(string newVal) {
      return new RefinementOrigin(WrappedOrigin.WithVal(newVal), InheritingModule);
    }
  ...
  public class RefinementTransformer : IRewriter {
    ...
    Method CloneMethod(Method previousMethod, List<AttributedExpression> moreEnsures, Specification<Expression> decreases, BlockStmt newBody, bool checkPreviousPostconditions, Attributes moreAttributes) {
      ...
      var newRange = currentMethod.Origin.MakeRefined(moduleUnderConstruction);
        ...
        if (j == oldStmt.Count) {
          ...
          if (cur is SkeletonStatement) {
            ...
            if (doMerge) {
              ...
              i++; j++;
              if (addedAssert != null) {
                body.Add(new AssertStmt(new BoogieGenerator.ForceCheckOrigin(addedAssert.Origin), addedAssert, null, null));
              }
            } else {
              MergeAddStatement(cur, body);
              i++;
            ...
            if (oldS is AssignStatement) {
              ...
              if (LeftHandSidesAgree(s.Lhss, nw.Lhss)) {
                ...
                var tok = new RangeToken(addedAssert.Origin.StartToken, addedAssert.Origin.EndToken);
    ...
    void CheckIsOkayUpdateStmt(ConcreteAssignStatement stmt, ModuleDefinition m) {
      foreach (var lhs in stmt.Lhss) {
        ...
        if (l is IdentifierExpr) {
          ...
          if ((ident.Var is LocalVariable && RefinementOrigin.IsInherited(((LocalVariable)ident.Var).Tok, m)) || ident.Var is Formal) {
          ...
          if (RefinementOrigin.IsInherited(member.tok, m)) {
