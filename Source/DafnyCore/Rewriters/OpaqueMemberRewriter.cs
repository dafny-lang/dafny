...
public class OpaqueMemberRewriter : IRewriter {
  ...
  protected void AnnotateRevealFunction(Method lemma, Function f) {
    ...
    Expression receiver;
    if (f.IsStatic) {
      receiver = new StaticReceiverExpr(f.RangeToken, (TopLevelDeclWithMembers)f.EnclosingClass, true);
    } else {
      receiver = new ImplicitThisExpr(f.RangeToken);
    ...
    var nameSegment = new NameSegment(f.RangeToken, f.NameNode, f.TypeArgs.Count == 0 ? null : typeApplication);
    var rr = new MemberSelectExpr(f.RangeToken, receiver, f.NameNode);
    rr.Member = f;
    ...
    rr.Type = new ArrowType(f.RangeToken, args, new IntType());
    nameSegment.ResolvedExpression = rr;
    nameSegment.Type = rr.Type;
    LiteralExpr low = new LiteralExpr(f.RangeToken, 1);
    LiteralExpr hi = new LiteralExpr(f.RangeToken, 2);
  ...
  private void GenerateRevealLemma(MemberDecl m, List<MemberDecl> newDecls) {
    if (m is Function f) {
      ...
      var amount = new LiteralExpr(m.RangeToken, 0);
    ...
    if (m is ConstantField c && c.Rhs != null) {
      ens.Add(new AttributedExpression(new BinaryExpr(c.RangeToken, BinaryExpr.Opcode.Eq, new NameSegment(c.RangeToken, c.NameNode, null), c.Rhs)));
    }
    Method reveal;
    if (m is TwoStateFunction) {
      reveal = new TwoStateLemma(m.RangeToken, "reveal_" + m.NameNode, m.HasStaticKeyword, new List<TypeParameter>(), new List<Formal>(), new List<Formal>(), new List<AttributedExpression>(),
      ...
      reveal = new Lemma(m.RangeToken, "reveal_" + m.NameNode, m.HasStaticKeyword, new List<TypeParameter>(), new List<Formal>(), new List<Formal>(), new List<AttributedExpression>(),
        new Specification<FrameExpression>(new List<FrameExpression>(), null), ens,
        new Specification<Expression>(new List<Expression>(), null), null, lemma_attrs, null);
    }
    newDecls.Add(reveal);
