...
public class OpaqueMemberRewriter : IRewriter {
  ...
  protected void ProcessOpaqueClassMembers(TopLevelDeclWithMembers c) {
    ...
    foreach (var member in c.Members.Where(member => member is Function or ConstantField)) {
      if (!ShouldBeRevealed(member)) {
        ...
        // Nothing to do
      } else if (!RefinementOrigin.IsInherited(member.tok, c.EnclosingModuleDefinition)) {
  ...
  private void GenerateRevealLemma(MemberDecl m, List<MemberDecl> newDecls) {
    ...
    if (m is TwoStateFunction) {
      reveal = new TwoStateLemma(m.RangeToken.MakeAutoGenerated(), m.NameNode.Prepend(HideRevealStmt.RevealLemmaPrefix), isStatic,
      ...
      reveal = new Lemma(m.RangeToken.MakeAutoGenerated(), m.NameNode.Prepend(HideRevealStmt.RevealLemmaPrefix), isStatic, new List<TypeParameter>(),
