...
public class OpaqueMemberRewriter : IRewriter {
  ...
  private void GenerateRevealLemma(MemberDecl m, List<MemberDecl> newDecls) {
    ...
    if (m is TwoStateFunction) {
      reveal = new TwoStateLemma(m.RangeToken.MakeAutoGenerated(), m.NameNode.Prepend(RevealStmt.RevealLemmaPrefix), isStatic, false,
        ...
        new Specification<Expression>(new List<Expression>(), null), new List<(Expression, bool)>(), null, lemma_attrs, null);
    } else {
      reveal = new Lemma(m.RangeToken.MakeAutoGenerated(), m.NameNode.Prepend(RevealStmt.RevealLemmaPrefix), isStatic, false, new List<TypeParameter>(),
