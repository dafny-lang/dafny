...
public class AllOpaqueRevealStmtInserter : IRewriter {
  ...
  private void AddMethodReveals(Method m, ErrorReporter reporter) {
    ...
    if (addedReveals.Any()) {
      reporter.Message(MessageSource.Rewriter, ErrorLevel.Info, null, m.tok,   addedReveals.Count.ToString() + " reveal stmt(s) inserted: \n" + RenderRevealStmts(addedReveals));
    }
  }
  ...
  private void AddFunctionReveals(Function f, ErrorReporter reporter) {
    ...
    if (addedReveals.Any()) {
      reporter.Message(MessageSource.Rewriter, ErrorLevel.Info, null, f.tok, addedReveals.Count.ToString() + " reveal stmt(s) inserted: \n" + RenderRevealStmts(addedReveals));
    }
  }
