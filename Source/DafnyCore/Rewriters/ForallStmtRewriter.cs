...
public class ForallStmtRewriter : IRewriter {
  ...
  internal class ForAllStmtVisitor : TopDownVisitor<bool> {
    ...
    private static void VisitCall(ForallStmt s) {
      ...
      var expr = new ForallExpr(new AutoGeneratedOrigin(s.Tok), s.BoundVars, s.Range, term, s.Attributes) {
    ...
    private void VisitAssign(ForallStmt stmt) {
      ...
      if (Fi != null) {
        ...
        if (vals != null) {
          foreach (var val in vals) {
            ...
            var expr = new ForallExpr(stmt.Tok, jList, val.Range,
      ...
      if (!usedInversion) {
        var expr = new ForallExpr(stmt.Tok, stmt.BoundVars, stmt.Range,
    ...
    private static void VisitProof(ForallStmt s) {
      ...
      s.EffectiveEnsuresClauses = new List<Expression> {
        new ForallExpr(s.Tok, s.BoundVars, s.Range, term, s.Attributes) {
          Type = Type.Bool, // resolve here
          Bounds = s.Bounds
        }
      };
