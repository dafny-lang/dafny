    ...
    protected override bool VisitOneStmt(Statement stmt, ref bool st) {
      if (stmt is ForallStmt && ((ForallStmt)stmt).CanConvert) {
        ...
        if (s.Kind == ForallStmt.BodyKind.Proof) {
          Expression term = s.Ens.Count != 0 ? s.Ens[0].E : Expression.CreateBoolLiteral(new AutoGeneratedToken(s.Tok), true);
          ...
          Expression term = s0.Method.Ens.Count != 0 ? substituter.Substitute(s0.Method.Ens[0].E) : Expression.CreateBoolLiteral(new AutoGeneratedToken(s.Tok), true);
