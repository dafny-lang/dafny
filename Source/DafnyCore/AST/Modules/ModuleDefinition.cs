...
Generate module names in the older A_mB_mC style instead of the current A.B.C scheme".TrimStart()) {
  ...
  public ModuleDefinition(Cloner cloner, ModuleDefinition original) : base(cloner, original) {
    ...
    PrefixIds = original.PrefixIds.ToList();
  ...
  public ModuleDefinition(IOrigin tok, Name name, List<Token> prefixIds, ModuleKindEnum moduleKind, bool isFacade,
        ...
        foreach (DatatypeCtor ctor in dt.Ctors) {
          ...
          foreach (var formal in ctor.Formals) {
            ...
            if (!localDuplicate && previousMember is DatatypeDestructor) {
              ...
              dtor = new DatatypeDestructor(formal.Origin, ctor, formal, /* should be formal.NameNode */ new Name(formal.StartToken, formal.Name), "dtor_" + formal.CompileName,
