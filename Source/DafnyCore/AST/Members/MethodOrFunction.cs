...
public abstract class MethodOrFunction : MemberDecl, ICodeContainer {
  ...
  protected MethodOrFunction(IOrigin rangeToken, Name name, bool hasStaticKeyword, bool isGhost,
  ...
  public void ResolveMethodOrFunction(INewOrOldResolver resolver) {
    ...
    if (!AutoGeneratedToken.Is(Origin) && HasVerifyFalseAttribute) {
      resolver.Reporter.Warning(MessageSource.Verifier, ResolutionErrors.ErrorId.none, Tok,
    ...
    } else if (!AutoGeneratedToken.Is(Origin) && Bodyless && !IsVirtual && !IsAbstract
