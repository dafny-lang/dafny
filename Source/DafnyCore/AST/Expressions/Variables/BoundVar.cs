  ...
  public static void ExtractSingleRange(List<QuantifiedVar> qvars, out List<BoundVar> bvars, [CanBeNull] out Expression range) {
    ...
    foreach (var qvar in qvars) {
      ...
      if (qvar.Domain != null) {
        ...
        var inDomainExpr = new BinaryExpr(domainWithToken.Origin, BinaryExpr.Opcode.In, new IdentifierExpr(bvar.Origin, bvar), domainWithToken);
        range = range == null ? inDomainExpr : new BinaryExpr(domainWithToken.Origin, BinaryExpr.Opcode.And, range, inDomainExpr);
      }
      ...
      if (qvar.Range != null) {
        ...
        range = range == null ? qvar.Range : new BinaryExpr(rangeWithToken.Origin, BinaryExpr.Opcode.And, range, rangeWithToken);
      }
    }
  }
}
