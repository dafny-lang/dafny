...
public abstract class Declaration : RangeNode, IAttributeBearingDeclaration, IDeclarationOrUsage {
  ...
  public override IEnumerable<Assumption> Assumptions() {
    if (HasAxiomAttribute && !HasAutoGeneratedAttribute) {
      yield return new Assumption(tok, AssumptionDescription.HasAxiomAttribute);
    }

    if (HasVerifyFalseAttribute && !HasAutoGeneratedAttribute) {
      yield return new Assumption(tok, AssumptionDescription.HasVerifyFalseAttribute);
    }
  }
...
public abstract class TopLevelDeclWithMembers : TopLevelDecl {
  ...
  public override IEnumerable<Assumption> Assumptions() {
    return Members.SelectMany(m => m.Assumptions());
  }
}
...
public class TraitDecl : ClassDecl {
  ...
  public override IEnumerable<Assumption> Assumptions() {
    ...
    if (Attributes.Find(Attributes, "termination") is Attributes ta &&
        ...
        termCheck == false) {
      yield return new Assumption(tok, AssumptionDescription.HasTerminationFalseAttribute);
    }
  }
}
