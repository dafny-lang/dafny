    ...
    public class CustomDirectoryInfoWrapper : DirectoryInfoBase
    {
        ...
        public override IEnumerable<FileSystemInfoBase> EnumerateFileSystemInfos()
        {
            if (_directoryInfo.Exists)
            {
                ...
                try
                {
                    fileSystemInfos = _directoryInfo.EnumerateFileSystemInfos("*", new EnumerationOptions {
                      MatchType = MatchType.Win32, AttributesToSkip = 0, IgnoreInaccessible = true, RecurseSubdirectories = false
                    });
                }
        ...
        public override DirectoryInfoBase? GetDirectory(string name)
        {
            ...
            else
            {
                ...
                if (dirs.Length == 1)
                {
                    return new CustomDirectoryInfoWrapper(dirs[0], isParentPath);
                }
