  ...
  public partial class Printer {
    ...
    public void PrintExtendedExpr(Expression expr, int indent, bool isRightmost, bool endWithCloseParen) {
      ...
      if (expr is ITEExpr) {
        ...
        PrintExpressionList(e.RHSs, true, parensAroundArgList: false);
        wr.Write(";");
        if (e.Origin is AutoGeneratedOrigin) {
          wr.Write(" */");
        }
    ...
    /// An indent of -1 means print the entire expression on one line.
    /// </summary>
    void PrintExpr(Expression expr, int contextBindingStrength, bool fragileContext, bool isRightmost,
      bool isFollowedBySemicolon, int indent, string keyword = null, int resolv_count = 2) {
      ...
      */
      ...
      if (expr is StaticReceiverExpr) {
        ...
        if (printArgs) {
          PrintActualArguments(dtv.Bindings, null, null, parensAroundArgList: false);
        }
        ...
        PrintExpressionList(e.Elements, false, parensAroundArgList: false);
        wr.Write(e is SetDisplayExpr || e is MultiSetDisplayExpr ? "}" : "]");
        ...
        bool parensNeeded =
          !e.Lhs.IsImplicit && ParensNeeded(opBindingStrength, contextBindingStrength, fragileContext);
        ...
        if (!e.Lhs.IsImplicit) {
          ...
          if (e.Lhs.Type is ResolverIdentifierExpr.ResolverTypeType) {
            Contract.Assert(e.Lhs is NameSegment ||
                            e.Lhs is ExprDotName); // these are the only expressions whose .Type can be ResolverType_Type
            if (options.DafnyPrintResolvedFile != null && options.PrintMode == PrintModes.Everything) {
              ...
              var optionalTypeArgs =
                e.Lhs is NameSegment ns ? ns.OptTypeArguments : ((ExprDotName)e.Lhs).OptTypeArguments;
        ...
        int opBindingStrength = BindingStrengthSuffix;
        bool parensNeeded =
          !e.Obj.IsImplicit && ParensNeeded(opBindingStrength, contextBindingStrength, fragileContext);
        ...
        */
        ...
        if (e.Op == UnaryOpExpr.Opcode.Cardinality) {
          ...
          switch (e.Op) {
            case UnaryOpExpr.Opcode.Not:
              op = "!";
              opBindingStrength = BindingStrengthUnaryPrefix;
              break;
            default:
        ...
        if (e.LHSs.Exists(lhs => lhs != null && lhs.Var != null && lhs.Var.IsGhost)) {
          wr.Write("ghost ");
        }
        ...
        if (e.Lhs != null) {
          if (e.Lhs.Var != null && e.Lhs.Var.IsGhost) {
            wr.Write("ghost ");
          }
        ...
        if (e.SplitQuantifier != null) {
          PrintExpr(e.SplitQuantifierExpression, contextBindingStrength, fragileContext, isRightmost,
            isFollowedBySemicolon, indent, keyword, resolv_count);
          return;
        }
        ...
        int ind = indent < 0
          ? IndentAmount
          : indent; // if the expression was to be printed on one line, instead print the .S part at indentation IndentAmount (not pretty, but something)
        ...
        wr.Write(" then ");
        var thenBranch = options.DafnyPrintResolvedFile == null && ite.IsBindingGuard
          ? ((LetExpr)ite.Thn).Body
          : ite.Thn;
        ...
        var e = (MatchExpr)expr;
        if (options.DafnyPrintResolvedFile == null && e.OrigUnresolved != null) {
          PrintExpr(e.OrigUnresolved, contextBindingStrength, fragileContext, isRightmost, isFollowedBySemicolon,
            indent);
        } else {
          var parensNeeded = !isRightmost && !e.UsesOptionalBraces;
          if (parensNeeded) {
        ...
        PrintExpr(e.Resolved, contextBindingStrength, fragileContext, isRightmost, isFollowedBySemicolon, indent,
          keyword);
        ...
        var parensNeeded =
          ...
          decreasesToExpr.OldExpressions.Count != 1 ||
          decreasesToExpr.NewExpressions.Count != 1; // always parenthesize non-simple expressions
        ...
        wr.Write("`[");
        for (var i = 0; i < e.Indices.Count; i++) {
          if (i > 0) {
            wr.Write(", ");
          }

          PrintExpr(e.Indices[i], BindingStrengthSuffix, false, false, false, -1);
        }
