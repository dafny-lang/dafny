    ...
    if (rhs is ExprRhs) {
      ...
      if (allocateArray.ElementInit != null) {
        ...
        // cool
                 ...
                 !allocateArray.EType.HasCompilableValue) {
        ...
        var desc = new ArrayInitEmpty(allocateArray.EType.ToString(), allocateArray.ArrayDimensions);
      ...
      if (allocateArray.ElementInit != null) {
        CheckElementInit(tok, true, allocateArray.ArrayDimensions, allocateArray.EType, allocateArray.ElementInit, nw, builder, etran, new WFOptions());
      } else if (allocateArray.InitDisplay != null) {
        int ii = 0;
        foreach (var v in allocateArray.InitDisplay) {
          var EE_ii = etran.TrExpr(v);
          // assert EE_ii satisfies any subset-type constraints;
          CheckSubrange(v.Origin, EE_ii, v.Type, allocateArray.EType, v, builder);
          // assume nw[ii] == EE_ii;
          var ai = ReadHeap(tok, etran.HeapExpr, nw, GetArrayIndexFieldName(tok, [Bpl.Expr.Literal(ii)]));
          builder.Add(new Bpl.AssumeCmd(tok, Bpl.Expr.Eq(UnboxUnlessInherentlyBoxed(ai, allocateArray.EType), AdaptBoxing(tok, EE_ii, v.Type, allocateArray.EType))));
          ii++;
        }
      }
      Bpl.Cmd heapAllocationRecorder = null;
