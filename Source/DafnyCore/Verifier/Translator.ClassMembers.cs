    ...
    private void SetAssertionOnlyFilter(Node m) {
      ...
      m.Visit(node => {
        if (node is AssertStmt assertStmt &&
          ...
          if (rangesOnly.FindIndex(r => r.Contains(node.RangeToken.StartToken.pos)) is var x && x >= 0) {
            if (assertOnlyKind == AssertStmt.AssertOnlyKind.Before) {// Just shorten the previous range
              rangesOnly[x] = new RangeToken(rangesOnly[x].StartToken, node.RangeToken.EndToken);
              return true;
            }
