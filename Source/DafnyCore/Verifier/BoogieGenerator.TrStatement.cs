  ...
  public partial class BoogieGenerator {
    private void TrStmt(Statement stmt, BoogieStmtListBuilder builder, List<Variable> locals, ExpressionTranslator etran) {
      ...
      if (stmt is PredicateStmt predicateStmt) {
        ...
        builder.Add(TrAssumeCmdWithDependenciesAndExtend(etran, s.tok, pat.Expr, e => Expr.Eq(e, rIe), "variable declaration", AssumptionForm.VariableDeclaration));
    ...
    private void TrAssumeStmt(AssumeStmt assumeStmt, BoogieStmtListBuilder builder, List<Variable> locals, ExpressionTranslator etran) {
      ...
      builder.Add(TrAssumeCmdWithDependencies(etran, assumeStmt.Tok, assumeStmt.Expr, "assume statement", AssumptionForm.AssumeStatement,
    ...
    private void TrAssertStmt(PredicateStmt stmt, BoogieStmtListBuilder builder, List<Variable> locals, ExpressionTranslator etran) {
      ...
      if (!splitHappened) {
        ...
        foreach (var split in splits) {
          if (split.IsChecked) {
            ...
            var desc = new PODesc.AssertStatement(stmt.Expr, errorMessage, successMessage);
            (proofBuilder ?? b).Add(AssertNS(ToDafnyToken(flags.ReportRanges, tok).ToRange(), split.E, desc, stmt.Tok,
      ...
      if (splitHappened || proofBuilder != null) {
        if (assertStmt != null && assertStmt.Label != null) {
          ...
          b.Add(TrAssumeCmdWithDependencies(etran, stmt.Tok, stmt.Expr, "assert statement", AssumptionForm.AssertStatement));
          stmtContext = StmtType.NONE;
        }
      }
      ...
      if (defineFuel) {
        ...
        stmtContext = StmtType.ASSUME;
        adjustFuelForExists = true;
        builder.Add(TrAssumeCmdWithDependencies(etran, stmt.Tok, stmt.Expr, "assert statement", AssumptionForm.AssertStatement));
        stmtContext = StmtType.NONE;
      }
      ...
      if (stmt.Lines.Count > 0) {
        ...
        for (int i = stepCount; 0 <= --i;) {
          ...
          if (stmt.Steps[i] is BinaryExpr && (((BinaryExpr)stmt.Steps[i]).ResolvedOp == BinaryExpr.ResolvedOpcode.Imp)) {
            ...
            b.Add(TrAssumeCmdWithDependencies(etran, stmt.Tok, CalcStmt.Lhs(stmt.Steps[i]), "calc statement step", AssumptionForm.CalcStep));
          }
          // hint:
        ...
        if (stmt.Steps.Count > 1) {
          builder.Add(TrAssumeCmdWithDependencies(etran, stmt.Tok, stmt.Result, "calc statement result", AssumptionForm.CalcResult));
        }
      }
      CurrentIdGenerator.Pop();
      this.fuelContext = FuelSetting.PopFuelContext();
    ...
    private void TrIfStmt(IfStmt stmt, BoogieStmtListBuilder builder, List<Variable> locals, ExpressionTranslator etran) {
      ...
      b = new BoogieStmtListBuilder(this, options);
      if (stmt.IsBindingGuard) {
        b.Add(TrAssumeCmdWithDependenciesAndExtend(etran, guard.tok, guard, Expr.Not, "if statement binding guard", AssumptionForm.IfBinding));
      }
      if (stmt.Els == null) {
    ...
    void TrForallProof(ForallStmt s, BoogieStmtListBuilder definedness, BoogieStmtListBuilder exporter, List<Variable> locals, ExpressionTranslator etran) {
      ...
      definedness.Add(TrAssumeCmdWithDependencies(etran, s.Range.tok, s.Range, "forall statement range", AssumptionForm.ForallRange));
      ...
      foreach (var ens in s.Ens) {
        ...
        ensuresDefinedness.Add(TrAssumeCmdWithDependencies(etran, ens.E.tok, ens.E, "forall statement ensures clause", AssumptionForm.ForallEnsures));
      }
    ...
    void TrLoop(LoopStmt s, Expression Guard, BodyTranslator/*?*/ bodyTr,
      ...
      foreach (AttributedExpression loopInv in s.Invariants) {
        ...
        invDefinednessBuilder.Add(TrAssumeCmdWithDependencies(etran, loopInv.E.tok, loopInv.E, "loop invariant", AssumptionForm.LoopInvariant));
    ...
    void TrAlternatives(List<GuardedAlternative> alternatives, Bpl.Cmd elseCase0, Bpl.StructuredCmd elseCase1,
      ...
      for (int i = alternatives.Count; 0 <= --i;) {
        ...
        if (alternative.IsBindingGuard) {
          ...
          b.Add(TrAssumeCmdWithDependencies(etran, alternative.Guard.tok, alternative.Guard, "alternative guard", AssumptionForm.AlternativeGuard));
        }
        var prevDefiniteAssignmentTrackerCount = definiteAssignmentTrackers.Count;
