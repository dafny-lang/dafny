...
namespace Microsoft.Dafny {
  public partial class Resolver {
    ...
    public void ResolveExpressionX(Expression expr, ResolutionContext resolutionContext) {
      ...
      if (expr is ParensExpression) {
        ...
        if (!ty.IsDatatype) {
          ...
          var (ghostLet, compiledLet) = ResolveDatatypeUpdate(e, ty.AsDatatype, resolutionContext,
    ...
    (Expression, Expression) ResolveDatatypeUpdate(DatatypeUpdateExpr datatypeUpdateExpr, DatatypeDecl dt, 
      ...
      var root = datatypeUpdateExpr.Root;
      var memberUpdates = datatypeUpdateExpr.Updates;
          ...
          if (!classMembers[dt].TryGetValue(destructor_str, out member)) {
            ...
            if (intersection.Count == 0) {
              ...
              if (destructor.IsGhost) {
                ...
                var xVar = new BoundVar(datatypeUpdateExpr.RangeToken.MakeAutoGenerated(), xName, destructor.Type.Subst(subst));
                var x = new IdentifierExpr(datatypeUpdateExpr.RangeToken.MakeAutoGenerated(), xVar);
      ...
      foreach (var ctor in candidateResultCtors) {
        if (ctor.Formals.Exists(f => !f.HasName)) {
          reporter.Error(MessageSource.Resolver, datatypeUpdateExpr.Tok,
      ...
      var desugaringForGhostContext = DesugarDatatypeUpdate(datatypeUpdateExpr.RangeToken, root, dt, candidateResultCtors, rhsBindings, resolutionContext);
      ...
      var desugaringForCompiledContext = DesugarDatatypeUpdate(datatypeUpdateExpr.RangeToken, root, dt, nonGhostConstructors, rhsBindings, resolutionContext);
    ...
    Expression ResolveExprDotCall(IHasOptionalTypeArguments dotName, Name name, Expression receiver, Type receiverTypeBound/*?*/,
      MemberDecl member, List<ActualBinding> args, ResolutionContext resolutionContext, bool allowMethodCall) {
      ...
      if (r == null) {
        ...
        if (fnType == null) {
          ...
          if (callee != null) {
            ...
            var rr = new FunctionCallExpr(e.RangeToken, mse.MemberNameNode, mse.Obj, e.Bindings, atLabel) {
              Function = callee,
