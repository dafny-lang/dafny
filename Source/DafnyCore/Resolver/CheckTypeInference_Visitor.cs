...
partial class Resolver {
  ...
  void CheckTypeInference(Expression expr, ICodeContext codeContext) {
    ...
    var c = new CheckTypeInferenceVisitor(this, codeContext);
    c.Visit(expr);
  }
...
class CheckTypeInferenceVisitor : ResolverBottomUpVisitor {
  readonly ICodeContext codeContext;

  public CheckTypeInferenceVisitor(Resolver resolver, ICodeContext codeContext)
    : base(resolver) {
  ...
  protected override void VisitOneStmt(Statement stmt) {
    if (stmt is VarDeclStmt) {
      ...
      s.Bounds = Resolver.DiscoverBestBounds_MultipleVars(s.BoundVars, s.Range, true, ComprehensionExpr.BoundedPool.PoolVirtues.Enumerable);
      ...
      if (s.AssumeToken == null) {
        ...
        s.Bounds = Resolver.DiscoverBestBounds_MultipleVars(varLhss, s.Expr, true, ComprehensionExpr.BoundedPool.PoolVirtues.None);
      }
      foreach (var lhs in s.Lhss) {
  ...
  protected override void VisitOneExpr(Expression expr) {
    if (expr is LiteralExpr) {
      ...
      if (whereToLookForBounds != null) {
        e.Bounds = Resolver.DiscoverBestBounds_MultipleVars_AllowReordering(e.BoundVars, whereToLookForBounds, polarity, ComprehensionExpr.BoundedPool.PoolVirtues.None);
