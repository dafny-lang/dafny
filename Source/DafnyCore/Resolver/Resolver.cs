...
namespace Microsoft.Dafny {
  public partial class Resolver {
        ...
        foreach (var com in ModuleDefinition.AllExtremeLemmas(declarations)) {
          ...
          if (com.Body != null) {
            ...
            if (k.Type.IsBigOrdinalType) {
              ...
              var recursiveCall = new CallStmt(com.tok, com.RangeToken, new List<Expression>(), methodSel, recursiveCallArgs.ConvertAll(e => new ActualBinding(null, e)));
              ...
              var forallBody = new BlockStmt(com.tok, com.RangeToken, new List<Statement>() { recursiveCall });
              var forallStmt = new ForallStmt(com.tok, com.RangeToken, bvs, attrs, range, new List<AttributedExpression>(), forallBody);
              els = new BlockStmt(com.BodyStartTok, mainBody.RangeToken, new List<Statement>() { forallStmt });
            } else {
    ...
    private Expression makeTemp(String prefix, AssignOrReturnStmt s, ResolutionContext resolutionContext, Expression ex) {
      ...
      var locvar = new LocalVariable(s.Tok, s.RangeToken, temp, ex.Type, false);
      ...
      var up = new UpdateStmt(s.Tok, s.RangeToken, idlist, rhss);
      s.ResolvedStatements.Add(new VarDeclStmt(s.Tok, s.RangeToken, lhss, up));
      return id;
    }
    ...
    private void ResolveAssignOrReturnStmt(AssignOrReturnStmt s, ResolutionContext resolutionContext) {
      ...
      var lhss = new List<LocalVariable>() { new LocalVariable(s.Tok, s.RangeToken, temp, new InferredTypeProxy(), false) };
      // "var temp ;"
      s.ResolvedStatements.Add(new VarDeclStmt(s.Tok, s.RangeToken, lhss, null));
      ...
      // " temp, ... := MethodOrExpression, ...;"
      UpdateStmt up = new UpdateStmt(s.Tok, s.RangeToken, lhss2, rhss2);
      if (expectExtract) {
      ...
      if (s.KeywordToken != null) {
        ...
        if (s.KeywordToken.Token.val == "expect") {
          ...
          ss = new ExpectStmt(s.Tok, s.RangeToken, notFailureExpr, new IdentifierExpr(s.Tok, temp), s.KeywordToken.Attrs);
        } else if (s.KeywordToken.Token.val == "assume") {
          ss = new AssumeStmt(s.Tok, s.RangeToken, notFailureExpr, s.KeywordToken.Attrs);
        } else if (s.KeywordToken.Token.val == "assert") {
          ss = new AssertStmt(s.Tok, s.RangeToken, notFailureExpr, null, null, s.KeywordToken.Attrs);
        } else {
        ...
        s.ResolvedStatements.Add(
          // "if temp.IsFailure()"
          new IfStmt(s.Tok, s.RangeToken, false, VarDotMethod(s.Tok, temp, "IsFailure"),
            // THEN: { out := temp.PropagateFailure(); return; }
            new BlockStmt(s.Tok, s.RangeToken, new List<Statement>() {
              new UpdateStmt(s.Tok, s.RangeToken,
                new List<Expression>() { ident },
                new List<AssignmentRhs>() {new ExprRhs(VarDotMethod(s.Tok, temp, "PropagateFailure"))}
                ),
              new ReturnStmt(s.Tok, s.RangeToken, null),
            }),
            // ELSE: no else block
            null
          ));
