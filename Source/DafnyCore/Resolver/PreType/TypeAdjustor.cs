...
public class TypeAdjustorVisitor : ASTVisitor<IASTVisitorContext> {
  ...
  abstract class TypeDependency {
    protected readonly UpdatableTypeProxy lhsProxy;
    private readonly string description;
    private readonly IToken tok;

    protected Type Lhs {
      get => lhsProxy.T;
      set => lhsProxy.T = value;
    }
    ...
    /// <summary>
    /// Recomputes the RHS type, possibly updating intermediate types inside the RHS.
    /// Returns whether or not anything changed.
    /// </summary>
    public abstract bool ComputeRhs();

    protected TypeDependency(UpdatableTypeProxy lhsProxy, string description, IToken tok) {
      this.lhsProxy = lhsProxy;
  ...
  class TypeDependencyFixed : TypeDependency {
    private readonly Type rhsType;

    public TypeDependencyFixed(UpdatableTypeProxy lhsProxy, Type type, string description, IToken tok)
      : base(lhsProxy, description, tok) {
      rhsType = type;
    }

    public override bool ComputeRhs() {
#if SOON
      var previousLhs = Lhs;
      Lhs = PreTypeToTypeVisitor.Join(Lhs, rhsType);
      return Lhs.Equals(rhsType);
#else
      return false;
#endif
    }
    ...
    public override void DebugPrint(TextWriter output) {
      var bound = PreTypeConstraints.Pad($"%{lhsProxy.UniqueId} == {rhsType}", 27);
      var value = PreTypeConstraints.Pad(UpdatableTypeProxy.ToStringAsBottom(lhsProxy), 20);
      output.WriteLine($"    {bound}  {value}    {TokDescription()}");
    }
  }

  class TypeDependencyOnExpression : TypeDependency {
    public readonly Expression Rhs;

    public TypeDependencyOnExpression(UpdatableTypeProxy lhsProxy, Expression rhs, string description, IToken tok)
      : base(lhsProxy, description, tok) {
      Rhs = rhs;
    }
...
#if SOON
      var previousLhs = Lhs;
      Lhs = PreTypeToTypeVisitor.Join(Lhs, Rhs.Type);
      return Lhs.Equals(Rhs.Type);
#else
      return false;
#endif
    }
    ...
    public override void DebugPrint(TextWriter output) {
      var bound = PreTypeConstraints.Pad($"%{lhsProxy.UniqueId} :> {UpdatableTypeProxy.ToStringAsUpdatableTypeProxy(Rhs.UnnormalizedType)}", 27);
      var value = PreTypeConstraints.Pad(UpdatableTypeProxy.ToStringAsBottom(lhsProxy), 20);
      output.WriteLine($"    {bound}  {value}    {TokDescription()}");
    }
  }
  ...
  private readonly List<TypeDependency> constraints = new();

  private void AddConstraint(UpdatableTypeProxy proxy, Expression rhs, string description, IToken tok) {
#if SOON
    constraints.Add(new TypeDependencyOnExpression(proxy, rhs, description, tok));
#endif
  }

  private void AddConstraintFixed(UpdatableTypeProxy proxy, Type type, string description, IToken tok) {
#if SOON
    constraints.Add(new TypeDependencyFixed(proxy, type, description, tok));
#endif
  }
  ...
  public void DebugPrint() {
    ...
    foreach (var constraint in constraints) {
      constraint.DebugPrint(systemModuleManager.Options.OutputWriter);
    }
  ...
  public void Solve(bool debugPrint) {
    ...
    do {
      ...
      foreach (var typeDependency in constraints) {
        var (proxy, type, _) = typeDependency;
        var join = Join(proxy, type);
        if ((proxy.IsBottomType && !AdjustableType.NormalizesToBottom(join)) || !Type.Equal_Improved(proxy, join)) {
          if (debugPrint) {
            systemModuleManager.Options.OutputWriter.WriteLine(
              $"DEBUG: updating proxy %{proxy.UniqueId} to {AdjustableType.ToStringAsAdjustableType(type)}" +
              $" ({AdjustableType.ToStringAsBottom(proxy)} \\/ {AdjustableType.ToStringAsBottom(type)})");
          }
          proxy.T = join;
  ...
  public UpdatableTypeProxy PreType2UpdatableType(PreType preType, Type lowerBound, string description, IToken tok) {
    ...
    AddConstraintFixed(proxy, lowerBound, $"{description}", tok);
    return proxy;
  }
