      ...
      foreach (var callable in ModuleDefinition.AllCallables(moduleDefinition.TopLevelDecls)) {
        if ((callable is Method method) && Attributes.Contains(method.Attributes, "test")) {
          ...
          if (method.Ins.Count != 0) {
            Reporter.Error(MessageSource.Rewriter, ErrorId.rw_test_already_has_main_method.ToString(), method.tok,
          ...
          switch (method.Outs.Count) {
            case > 1:
              Reporter.Error(MessageSource.Rewriter, ErrorId.rw_test_method_has_too_many_returns.ToString(),
               method.tok,
