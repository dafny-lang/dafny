...
public class RunAllTestsMainMethod : IRewriter {
  ...
  internal override void PostResolve(Program program) {
    ...
    foreach (var moduleDefinition in program.CompileModules) {
      foreach (var callable in ModuleDefinition.AllCallables(moduleDefinition.TopLevelDecls)) {
        if ((callable is Method method) && Attributes.Contains(method.Attributes, "test")) {
          ...
          var passedBlock = new BlockStmt(tok, tok.ToRange(), Util.Singleton(passedStmt));
          ...
          if (resultVarExpr != null) {
            ...
            tryBodyStatements.Add(new IfStmt(tok, tok.ToRange(), false, failureGuardExpr, failedBlock, passedBlock));
          } else {
            tryBodyStatements.Add(passedBlock);
          }

          var tryBody = new BlockStmt(tok, tok.ToRange(), tryBodyStatements);
          ...
          // }
          //
          var haltMessageVar = new LocalVariable(tok, tok.ToRange(), "haltMessage", Type.String(), false) {
            type = Type.String()
          };
    ...
    mainMethod.Body = new BlockStmt(tok, tok.ToRange(), mainMethodStatements);
  }
  ...
  private BlockStmt PrintTestFailureStatement(IToken tok, Expression successVarExpr, Expression failureValueExpr) {
    ...
    var failSuiteStmt =
      new AssignStmt(tok, tok.ToRange(), successVarExpr, new ExprRhs(Expression.CreateBoolLiteral(tok, false)));
    return new BlockStmt(tok, tok.ToRange(), Util.List<Statement>(failedPrintStmt, failSuiteStmt));
  }
}