...
partial class Resolver {
  ...
  public void ReportWarning(MessageSource source, ErrorId errorId, IToken t, string msg, params object[] args) {
    reporter.Warning(source, errorId.ToString(), t, msg, args);
  }
  ...
  public void ReportError(MessageSource source, ErrorId errorId, INode t, string msg, params object[] args) {
    reporter.Error(source, errorId.ToString(), t, msg, args);
  }
  ...
  class CheckTypeInferenceVisitor : ASTVisitor<TypeInferenceCheckingContext> {
      ...
      if (decl is NewtypeDecl newtypeDecl) {
        if (newtypeDecl.Var != null) {
          if (!IsDetermined(newtypeDecl.BaseType.NormalizeExpand())) {
            resolver.ReportError(MessageSource.Resolver, ErrorId.r_newtype_base_undetermined, newtypeDecl.tok, "newtype's base type is not fully determined; add an explicit type for '{0}'",
        ...
        if (!IsDetermined(subsetTypeDecl.Rhs.NormalizeExpand())) {
          resolver.ReportError(MessageSource.Resolver, ErrorId.r_subset_type_base_undetermined, subsetTypeDecl.tok,
          ...
          if (member is DatatypeDestructor dtor) {
            ...
            for (var i = 1; i < dtor.CorrespondingFormals.Count; i++) {
              ...
              if (!Type.Equal_Improved(rolemodel.Type, other.Type)) {
                resolver.ReportError(MessageSource.Resolver, ErrorId.r_shared_destructors_have_different_types, other,
      ...
      if (expr is LiteralExpr) {
        ...
        if (e.Type.IsBitVectorType || e.Type.IsBigOrdinalType) {
          ...
          if (e.Type.IsBitVectorType && MaxBV(e.Type.AsBitVectorType.Width) < absN) {
            resolver.ReportError(MessageSource.Resolver, ErrorId.r_literal_too_large_for_bitvector, e.tok, "literal ({0}) is too large for the bitvector type {1}", absN, e.Type);
          }
          ...
          if (n < 0 || e.tok.val == "-0") {
            ...
            resolver.ReportError(MessageSource.Resolver, ErrorId.r_no_unary_minus_in_case_patterns, e.tok, "unary minus (-{0}, type {1}) not allowed in case pattern", absN, e.Type);
          }
        }
        ...
        if (expr is StaticReceiverExpr stexpr) {
          foreach (Type t in stexpr.Type.TypeArgs) {
            if (t is InferredTypeProxy && ((InferredTypeProxy)t).T == null) {
              resolver.ReportError(MessageSource.Resolver, ErrorId.r_type_parameter_undetermined, stexpr.tok, "type of type parameter could not be determined; please specify the type explicitly");
            }
          }
        }
        ...
        foreach (var bv in e.BoundVars) {
          if (!IsDetermined(bv.Type.Normalize())) {
            resolver.ReportError(MessageSource.Resolver, ErrorId.r_bound_variable_undetermined, bv.tok,
        ...
        if (e is ExistsExpr && e.Range == null) {
          ...
          if (binBody != null && binBody.Op == BinaryExpr.Opcode.Imp) {  // check Op, not ResolvedOp, in order to distinguish ==> and <==
            ...
            resolver.ReportWarning(MessageSource.Resolver, ErrorId.r_exists_quantifier_warning, e.tok,
        ...
        if (e.Member is Function || e.Member is Method) {
          ...
          foreach (var p in Util.Concat(e.TypeApplication_AtEnclosingClass, e.TypeApplication_JustMember)) {
            ...
            if (!IsDetermined(p.Normalize())) {
              resolver.ReportError(MessageSource.Resolver, ErrorId.r_type_parameter_not_determined, e.tok,
        ...
        foreach (var p in Util.Concat(e.TypeApplication_AtEnclosingClass, e.TypeApplication_JustFunction)) {
          ...
          if (!IsDetermined(p.Normalize())) {
            var hint = e.Name.StartsWith("reveal_")
              ...
              : "";
            resolver.ReportError(MessageSource.Resolver, ErrorId.r_function_type_parameter_undetermined, e.tok,
        ...
        foreach (var p in e.LHSs) {
          foreach (var x in p.Vars) {
            if (!IsDetermined(x.Type.Normalize())) {
              resolver.ReportError(MessageSource.Resolver, ErrorId.r_bound_variable_type_undetermined, x.tok, $"the type of the bound variable '{x.Name}' could not be determined");
            } else {
        ...
        if (e.ToType.IsRefType) {
          ...
          if (fromType.IsSubtypeOf(e.ToType, false, true) || e.ToType.IsSubtypeOf(fromType, false, true)) {
            ...
            resolver.ReportError(MessageSource.Resolver, ErrorId.r_never_succeeding_type_cast, e.tok,
        ...
        if (fromType.IsSubtypeOf(e.ToType, false, true)) {
          ...
          resolver.ReportError(MessageSource.Resolver, ErrorId.r_never_succeeding_type_test, e.tok,
            "a type test to '{0}' must be from a compatible type (got '{1}')", e.ToType, fromType);
        } else if (!e.ToType.IsRefType) {
          resolver.ReportError(MessageSource.Resolver, ErrorId.r_unsupported_type_test, e.tok,
            ...
            switch (e.ResolvedOp) {
              ...
              case BinaryExpr.ResolvedOpcode.EqCommon: {
                  ...
                  if (nntUdf != null) {
                    ...
                    resolver.ReportWarning(MessageSource.Resolver, ErrorId.r_trivial_null_test, e.tok,
              ...
              case BinaryExpr.ResolvedOpcode.InMultiSet: {
                  ...
                  if (((CollectionType)ty).Arg.IsNonNullRefType) {
                    var non = sense ? "" : "non-";
                    var b = sense ? "false" : "true";
                    resolver.ReportWarning(MessageSource.Resolver, ErrorId.r_trivial_null_inclusion_test, e.tok,
              ...
              case BinaryExpr.ResolvedOpcode.InMap: {
                  ...
                  if (((MapType)ty).Domain.IsNonNullRefType) {
                    var b = sense ? "false" : "true";
                    resolver.ReportWarning(MessageSource.Resolver, ErrorId.r_trivial_map_null_inclusion_test, e.tok,
    ...
    public bool CheckTypeIsDetermined(IToken tok, Type t, string what) {
      ...
      if (t is TypeProxy) {
        ...
        if (!UnderspecifiedTypeProxies.Contains(proxy)) {
          ...
          resolver.ReportError(MessageSource.Resolver, ErrorId.r_var_type_undetermined, tok, "the type of this {0} is underspecified", what);
    ...
    public void CheckContainsNoOrdinal(ErrorId errorId, IToken tok, Type t, string errMsg) {
      ...
      if (t.IsBigOrdinalType) {
        resolver.ReportError(MessageSource.Resolver, errorId, tok, errMsg);
      }
