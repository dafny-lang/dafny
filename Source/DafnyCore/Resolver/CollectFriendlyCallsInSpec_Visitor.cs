    ...
    if (expr is FunctionCallExpr fexp) {
      if (cp == CallingPosition.Positive) {
        if (IsCoContext ? fexp.Function is GreatestPredicate : fexp.Function is LeastPredicate) {
          var extremePredicate = (ExtremePredicate)fexp.Function;
          if (Context.KNat != extremePredicate.KNat) {
            KNatMismatchError(expr.tok, Context.Name, Context.TypeOfK, extremePredicate.TypeOfK);
          } else {
