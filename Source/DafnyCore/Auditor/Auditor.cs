...
public class Auditor : IRewriter {
  ...
  private string GenerateHtmlReport(AuditReport report) {
    ...
    if (templateStream is null) {
      Reporter.Warning(MessageSource.Verifier, null, Token.NoToken, "Embedded HTML template not found. Returning raw HTML.");
      return table;
    }
    ...
    if (reportFileName is null && reportFormat is null) {
      foreach (var (_, assumptions) in report.AllAssumptions()) {
        foreach (var assumption in assumptions) {
          Reporter.Warning(MessageSource.Verifier, null, assumption.Tok, assumption.Warning());
        }
      }
    } else {
