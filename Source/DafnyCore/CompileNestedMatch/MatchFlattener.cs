...
public class MatchFlattener : IRewriter {
  ...
  private Expression CompileNestedMatchExpr(NestedMatchExpr nestedMatchExpr) {
    ...
    if (compiledMatch is null) {
      ...
      var result = new MatchExpr(nestedMatchExpr.RangeToken, nestedMatchExpr.Source, new List<MatchCaseExpr>(), nestedMatchExpr.UsesOptionalBraces);
  ...
  private IEnumerable<NestedMatchCaseStmt> FlattenNestedMatchCaseStmt(NestedMatchCaseStmt c) {
    foreach (var pat in FlattenDisjunctivePatterns(c.Pat)) {
      yield return new NestedMatchCaseStmt(c.RangeToken, pat,
        c.Body,
        c.Attributes);
    }
  }
    ...
    switch (pat) {
      ...
      case IdPattern p:
        if (inDisjunctivePattern && p.ResolvedLit == null && p.Arguments == null && !p.IsWildcardPattern) {
          return new IdPattern(p.RangeToken, FreshTempVarName("_", null), null, p.IsGhost);
        }
        var args = p.Arguments?.ConvertAll(a => RemoveIllegalSubpatterns(a, inDisjunctivePattern));
        return new IdPattern(p.RangeToken, p.Id, p.Type, args, p.IsGhost) { ResolvedLit = p.ResolvedLit, BoundVar = p.BoundVar };
  ...
  private IEnumerable<NestedMatchCaseExpr> FlattenNestedMatchCaseExpr(NestedMatchCaseExpr c) {
    foreach (var pat in FlattenDisjunctivePatterns(c.Pat)) {
      yield return new NestedMatchCaseExpr(c.RangeToken, pat, c.Body, c.Attributes);
    }
  }
  ...
  private CaseBody CompileHeadsContainingConstructor(MatchCompilationState mti, MatchingContext context, Cons<Expression> matchees,
    ...
    foreach (var ctor in constructorByName.Values) {
      ...
      var freshPatBV = ctor.Formals.ConvertAll(
        x => CreateBoundVariable(headMatchee.RangeToken, x.Type.Subst(subst), mti.CodeContext.CodeContext));
        ...
        if (head is IdPattern idPattern) {
          if (ctor.Name.Equals(idPattern.Id) && idPattern.Arguments != null) {
            ...
            var freshArgs = ctor.Formals.ConvertAll(x =>
              CreateFreshBindingPattern(idPattern.RangeToken, x.Type.Subst(subst), mti.CodeContext.CodeContext, x.IsGhost));
      ...
      var freshMatchees = freshPatBV.ConvertAll(x => new IdentifierExpr(x.RangeToken, x));
      ...
      var tok = body.Tok ?? headMatchee.RangeToken.MakeAutoGenerated();
  ...
  private MatchCase CreateMatchCase(RangeToken tok, DatatypeCtor ctor, List<BoundVar> freshPatBV, CaseBody bodyContainer, bool fromBoundVar) {
  ...
  private BoundVar CreateBoundVariable(RangeToken tok, Type type, ICodeContext codeContext) {
    var name = FreshTempVarName("_mcc#", codeContext);
    return new BoundVar(new RangeToken(new AutoGeneratedToken(tok.StartToken), tok.EndToken), name, type);
  }

  private IdPattern CreateFreshBindingPattern(RangeToken tok, Type type, ICodeContext codeContext, bool isGhost = false) {
    var name = FreshTempVarName("_mcc#", codeContext);
    return new IdPattern(new RangeToken(new AutoGeneratedToken(tok.StartToken), tok.EndToken), name, type, null, isGhost);
  }
  ...
  record CaseBody(RangeToken Tok, Node Node, Attributes Attributes = null);

  private CaseBody PackBody(RangeToken tok, PatternPath path) {
  ...
  private class MatchCompilationState {
    ...
    public readonly RangeToken[] CaseTok;
    public readonly int[] CaseCopyCount;

    public RangeToken Tok => Match switch {
      NestedMatchExpr matchExpr => matchExpr.RangeToken,
      NestedMatchStmt matchStmt => matchStmt.RangeToken,
    ...
    public MatchCompilationState(Node match, IReadOnlyList<NestedMatchCase> flattenedCases, ResolutionContext codeContext,
      ...
      this.Match = match;
      this.CaseTok = flattenedCases.Select(c => c.RangeToken).ToArray();
  ...
  private PatternPath LetBind(IdPattern var, Expression genExpr, PatternPath bodyPath) {
    ...
    if (genExpr is IdentifierExpr idExpr) {
      if (idExpr.Name.StartsWith("_")) {
        expr = new IdentifierExpr(var.RangeToken, idExpr.Var);
      }
    }
    if (bodyPath is StmtPatternPath stmtPath) {
      ...
      caseLocal.type = type;
      var casePattern = new CasePattern<LocalVariable>(caseLocal.RangeToken, caseLocal);
      ...
      var substitutions = new Dictionary<IVariable, Expression>() {
        { var.BoundVar, new IdentifierExpr(var.BoundVar.RangeToken, caseLocal)}
      };
    ...
    if (bodyPath is ExprPatternPath exprPath) {
      ...
      var cPat = new CasePattern<BoundVar>(cBVar.RangeToken, cBVar);
      ...
      var letExpr = new LetExpr(cBVar.RangeToken, cPats, exprs, exprPath.Body, true);
