...
namespace Microsoft.Dafny.Triggers {
  internal class QuantifierCollector : TopDownVisitor<OldExpr/*?*/> {
    ...
    internal readonly List<ComprehensionContainerTriggerWriter> quantifierCollections = new List<ComprehensionContainerTriggerWriter>();
    ...
    protected override bool VisitOneExpr(Expression expr, ref OldExpr/*?*/ enclosingOldContext) {
      ...
      if (expr is ComprehensionExpr e && e.BoundVars.Count > 0 && !quantifiers.Contains(e)) {
        if (e is SetComprehension or MapComprehension) {
          quantifiers.Add(e);
          quantifierCollections.Add(new ComprehensionContainerTriggerWriter(e, Enumerable.Repeat(e, 1), reporter));
          ...
          if (quantifier.SplitQuantifier != null) {
            ...
            quantifierCollections.Add(new ComprehensionContainerTriggerWriter(e, collection, reporter));
            quantifiers.UnionWith(quantifier.SplitQuantifier);
          } else {
            quantifierCollections.Add(new ComprehensionContainerTriggerWriter(e, Enumerable.Repeat(quantifier, 1), reporter));
          }
        }
      }
