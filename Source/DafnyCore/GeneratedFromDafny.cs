...
module {:extern ""System""} {:compile false} {:options ""-functionSyntax:4""} System {
  ...
  type {:extern ""String""} CsString(==,!new) {
    predicate {:extern} Contains(needle: CsString)
  ...
  class {:extern ""String""} {:compile false} String {
    static function method Concat(s1: CsString, s2: CsString): (r: CsString)
      ensures r.Contains(s1)
      ensures r.Contains(s2)
  }

  lemma ContainsTransitive()
    ensures forall s1: CsString, s2: CsString, s3: CsString {:trigger s1.Contains(s3), s2.Contains(s3)} {:trigger s2.Contains(s3), s1.Contains(s2)} | s1.Contains(s2) && s2.Contains(s3) :: s1.Contains(s3)
  {
    forall s1: CsString, s2: CsString, s3: CsString | s1.Contains(s2) && s2.Contains(s3) {
...
module {:extern ""Microsoft.Dafny""} {:compile false} {:options ""-functionSyntax:4""} MicrosoftDafny {
  ...
  trait {:extern ""IToken""} {:compile false} IToken {
    ...
    predicate Valid()
      reads this, allTokens
      decreases |allTokens|
    {
      |allTokens| > 0 &&
      allTokens[0] == this &&
      ((|allTokens| == 1 && Next == null) || (|allTokens| > 1 && Next == allTokens[1] && Next.allTokens == allTokens[1..] && Next.Valid()))
    }

    lemma /*{:_induction this}*/ AlltokenSpec(i: int)
      requires Valid()
    ...
    lemma /*{:_induction this}*/ TokenNextIsIPlus1(middle: IToken, i: int)
      requires Valid()
      requires 0 <= i < |allTokens|
    ...
    lemma /*{:_induction this, middle}*/ TokenNextIsNullImpliesLastToken(middle: IToken, i: int)
  ...
  class {:extern ""TriviaFormatterHelper""} {:compile false} TriviaFormatterHelper {
    static predicate method EndsWithNewline(input: CsString)
  }
}
  ...
  module {:extern ""Dafny""} Dafny {
    ...
    module {:extern ""Formatting""} Formatting {
      ...
      trait IIndentationFormatter {
        function method GetNewLeadingTrivia(token: IToken): CsString
          decreases token

        function method GetNewTrailingTrivia(token: IToken): CsString
          decreases token

        method GetNewLeadingTrailingTrivia(token: IToken) returns (newLeadingTrivia: CsString, newTrailingTrivia: CsString)
      ...
      method ReindentProgramFromFirstToken(firstToken: IToken, reindent: IIndentationFormatter) returns (s: CsString)
        ...
        ensures forall token: IToken {:trigger token.val} {:trigger token in firstToken.allTokens} | token in firstToken.allTokens :: s.Contains(token.val)
        decreases firstToken, reindent
      {
        var token: IToken? := firstToken;
        var sb := new CsStringBuilder();
        ghost var i := 0;
        ghost var allTokens := firstToken.allTokens;
        while token != null
          invariant 0 <= i <= |allTokens|
          invariant if token == null then i == |allTokens| else token.Valid() && i < |allTokens| && token == allTokens[i]
          invariant forall t: IToken? {:trigger t.val} {:trigger t in allTokens[0 .. i]} | t in allTokens[0 .. i] :: sb.built.Contains(t.val)
          decreases if token == null then 0 else |token.allTokens|
        {
          ...
          var newLeadingTrivia, newTrailingTrivia := reindent.GetNewLeadingTrailingTrivia(token);
          ghost var sPrev := sb.built;
          sb.Append(newLeadingTrivia);
          sb.Append(token.val);
          sb.Append(newTrailingTrivia);
          ContainsTransitive();
          assert {:split_here} forall t: IToken? {:trigger t.val} | t in allTokens[0 .. i + 1] :: sb.built.Contains(t.val);
          token := token.Next;
          i := i + 1;
        }
        s := sb.ToString();
  ...
  public partial class __default {
    public static System.String ReindentProgramFromFirstToken(Microsoft.Dafny.IToken firstToken, Formatting.IIndentationFormatter reindent) {
      ...
      while ((_0_token) != (object)((Microsoft.Dafny.IToken)null)) {
        ...
        System.String _out0;
        System.String _out1;
        (reindent).GetNewLeadingTrailingTrivia(_0_token, out _out0, out _out1);
        _2_newLeadingTrivia = _out0;
        _3_newTrailingTrivia = _out1;
