...
namespace DafnyServer {
  public class VersionCheck {
    private readonly string _currentVersion = "1.9.16";
    ...
    public void Check() {
      try {
        ...
        using (var reader = new StreamReader(content)) {
          DataContractJsonSerializer j = new DataContractJsonSerializer(typeof(List<Release>));
          var data = (List<Release>)j.ReadObject(reader.BaseStream);
          string latestVersionStr = data[0].Name.Substring(1);

          var currentVersion = SemVersion.Parse(_currentVersion);
          var latestVersion = SemVersion.Parse(latestVersionStr);
          if (SemVersion.Compare(currentVersion, latestVersion) >= 0) {
            Console.WriteLine("Latest version installed");
          } else {
            Console.WriteLine("UPDATE_NECESSARY");
          }
        }
