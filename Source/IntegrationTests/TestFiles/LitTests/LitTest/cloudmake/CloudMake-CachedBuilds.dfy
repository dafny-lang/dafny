...
abstract module M0 {
  ...
  */
  ...
  lemma {:induction false} {:rlimit 3000000} EvalLemma(expr: Expression, st: State, env: Env, useCache: bool) returns (outExpr: Expression, outSt: State)
    requires ValidState(st) && ValidEnv(env)
    requires useCache ==> ConsistentCache(st)
    ensures
  ...
  lemma {:rlimit 1500000} StateCorrespondence_Ctor(stOrig: State, st: State, sts: set<State>, stC: State, stsC: set<State>)
