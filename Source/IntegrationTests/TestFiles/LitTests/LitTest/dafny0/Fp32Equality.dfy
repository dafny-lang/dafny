...
// - For IEEE 754 semantics, use fp32.Equal() instead
...
method TestCompiledEqualityPreconditions() {
  ...
  var pos_zero: fp32 := 0.0;   // Bit pattern: 0x0000000000000000
  var neg_zero: fp32 := -0.0;  // Bit pattern: 0x8000000000000000 (sign bit set!)
  ...
  // For IEEE 754 semantics (mathematical equality), use fp32.Equal:
  var eq4 := fp32.Equal(pos_zero, neg_zero);
  assert eq4;  // IEEE 754: +0 == -0 is true (same mathematical value)
  print "+0.0 == -0.0 (IEEE 754 via fp32.Equal): ", eq4, "\n";

  // Dafny's == works fine when zeros have matching signs (same bit pattern):
  var pos_zero2: fp32 := 0.0;  // Also 0x0000000000000000
...
// Test 2: fp32.Equal static method (no preconditions required)
method TestStaticEqualMethod() {
  ...
  // Basic equality
  var eq1 := fp32.Equal(x, x);
  assert eq1;
  print "fp32.Equal(1.0, 1.0): ", eq1, "\n";

  // NaN comparisons (IEEE 754 semantics)
  var eq2 := fp32.Equal(nan, nan);
  assert !eq2;  // NaN != NaN
  print "fp32.Equal(NaN, NaN): ", eq2, "\n";

  var eq3 := fp32.Equal(x, nan);
  assert !eq3;
  print "fp32.Equal(1.0, NaN): ", eq3, "\n";

  // Infinity comparisons
  var eq4 := fp32.Equal(inf, inf);
  assert eq4;
  print "fp32.Equal(+Inf, +Inf): ", eq4, "\n";

  var eq5 := fp32.Equal(inf, ninf);
  assert !eq5;
  print "fp32.Equal(+Inf, -Inf): ", eq5, "\n";

  // Zero comparisons
  var eq6 := fp32.Equal(0.0, -0.0);
  assert eq6;  // IEEE 754: +0 == -0
  print "fp32.Equal(+0.0, -0.0): ", eq6, "\n";
}

// Test 3: Signed Zero Precondition Pattern
method CheckEqualityWithDafny(a: fp32, b: fp32) returns (canUseEqEq: bool, result: bool)
{
  // First check for NaN
  if a.IsNaN || b.IsNaN {
    canUseEqEq := false;
...
method TestSignedZeroPreconditionPattern() {
  ...
  print "  Note: They ARE equal per IEEE 754: ", fp32.Equal(pos_zero, neg_zero), "\n";
  ...
  // Pattern 2: When you need IEEE 754 semantics, always use fp32.Equal
  print "\nFor IEEE 754 semantics, always use fp32.Equal:\n";
  print "fp32.Equal(+0, -0) = ", fp32.Equal(pos_zero, neg_zero), " (different bits, same value)\n";
  print "fp32.Equal(+0, +0) = ", fp32.Equal(pos_zero, pos_zero), " (same bits, same value)\n";
}

// Test 4: Dafny == vs IEEE 754 Comparison Table
method TestDafnyVsIEEE754() {
  print "=== Dafny's == vs IEEE 754 (fp32.Equal) ===\n";
  ...
  print "Comparison               | IEEE 754 (fp32.Equal) | Dafny == (compiled) | Dafny == (ghost)\n";
  print "-------------------------|-----------------------|---------------------|------------------\n";

  // NaN comparisons
  print "NaN == NaN               | ";
  print fp32.Equal(nan, nan), "               | ";
  print "FORBIDDEN            | ";
  print "true (reflexive)\n";

  print "1.0 == NaN               | ";
  print fp32.Equal(one, nan), "               | ";
  print "FORBIDDEN            | ";
  print "false\n";

  // Zero comparisons
  print "+0.0 == -0.0             | ";
  print fp32.Equal(pos_zero, neg_zero), "                | ";
  print "FORBIDDEN            | ";
  print "false (bitwise)\n";

  print "+0.0 == +0.0             | ";
  print fp32.Equal(pos_zero, pos_zero), "                | ";
  print (pos_zero == pos_zero), "                | ";
  print "true\n";

  // Regular comparisons
  print "1.0 == 1.0               | ";
  print fp32.Equal(one, one), "                | ";
  print (one == one), "                | ";
  print "true\n";

  print "+Inf == +Inf             | ";
  print fp32.Equal(pos_inf, pos_inf), "                | ";
...
ghost method TestGhostReflexiveEquality() {
  ...
  // In ghost context, we can still use fp32.Equal for IEEE semantics
  assert !fp32.Equal(nan, nan);  // IEEE 754 semantics
  assert fp32.Equal(x, x);
}

// Test 6: Equality in specifications
method TestEqualityInSpecifications(x: fp32, y: fp32)
...
ghost predicate ReflexiveEqual(x: fp32) {
  x == x  // Always true in ghost context, even for NaN
}

ghost function TestGhostFunction(x: fp32): bool {
  x == x  // Reflexive in ghost context
}

// Test 8: Lemmas with fp32 equality
lemma ReflexivityLemma(x: fp32)
...
method TestDisequality() {
  ...
  // Use negation of fp32.Equal for NaN comparisons
  var neq2 := !fp32.Equal(x, nan);
  assert neq2;
  print "!fp32.Equal(1.0, NaN): ", neq2, "\n";
}

// Test 10: Method contracts with fp32 parameters
method ProcessPair(a: fp32, b: fp32) returns (equal: bool)
...
method TestArithmeticSignedZeros() {
  ...
  // Note: Explicit fp32 type needed for new resolver type inference
  var pos_zero_computed: fp32 := 1.0 - 1.0;

  // Arithmetic that produces negative zero
  var neg_zero_computed: fp32 := -0.0 * 1.0;  // Preserves sign of zero
  ...
  var ieee_equal := fp32.Equal(pos_zero_computed, neg_zero_computed);
  assert ieee_equal;
...
method TestDerivedValuePreconditions() {
  ...
  // Or just use fp32.Equal for safety (handles all special cases)
  var safe := fp32.Equal(diff, neg_diff);
  print "fp32.Equal(diff, neg_diff): ", safe, " (IEEE 754: +0 == -0)\n";
}
