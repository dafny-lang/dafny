...
method TestExactFp32Literals() {
  ...
  var half: fp32 := 0.5;         // Binary: 0.1
  var quarter: fp32 := 0.25;     // Binary: 0.01
  var eighth: fp32 := 0.125;     // Binary: 0.001
  assert half * 2.0 == 1.0;
  ...
  var one_point_five: fp32 := 1.5;       // Binary: 1.1
  var three_quarters: fp32 := 0.75;      // Binary: 0.11
  var five_eighths: fp32 := 0.625;       // Binary: 0.101
...
method TestApproximateFp32Literals() {
  ...
  assert a + b != c;  // 1.1 + 2.2 != 3.3 in fp32
  ...
  var neg_a: fp32 := ~-1.1;
  assert neg_a.IsNegative;
  assert -neg_a == a;  // Negation is exact, so -(-1.1) == 1.1
}
...
method TestApproximateLiteralSyntaxRules() {
  ...
  var pos_approx: fp32 := ~0.1;
  var neg_approx: fp32 := ~-0.1;
  ...
  var x: fp32 := ~0.1;
  var y: fp32 := x;  // No ~ needed on variable
  var z: fp32 := x + y;  // No ~ needed on expression
...
method TestTypeInference() {
  ...
  var exact_fp32: fp32 := 0.5;    // Explicitly fp32

  // Type inference with approximate literals
  var approx_inferred := ~0.1;    // Must be fp32 due to ~
  var approx_fp32: fp32 := ~0.1;  // Explicitly fp32

  // Verify approximate literals force fp32 type
  assert approx_inferred == approx_fp32; // Same type, same value

  // Operations preserve type
  var sum := approx_inferred + approx_fp32;
  assert sum == approx_inferred + approx_fp32; // Tautology but shows type works
}
...
method TestFinancialCalculations() {
  // Computing sales tax
  var price: fp32 := ~19.99;
  var taxRate: fp32 := ~0.0825;  // 8.25%
  var tax: fp32 := price * taxRate;
  var total: fp32 := price + tax;
...
method TestScientificComputations() {
  // Circle calculations
  var radius: fp32 := 5.0;  // Exact
  var pi: fp32 := ~3.14159265358979323846;
  var area: fp32 := pi * radius * radius;
  var circumference: fp32 := 2.0 * pi * radius;
...
method TestRealVsFp32Arithmetic() {
  ...
  // fp32 arithmetic has rounding errors
  var fp32_a: fp32 := ~1.1;
  var fp32_b: fp32 := ~2.2;
  var fp32_sum: fp32 := fp32_a + fp32_b;
  var fp32_c: fp32 := ~3.3;

  // The sum exists and is finite
  assert fp32_sum.IsFinite;
  assert fp32_c.IsFinite;

  assert fp32_sum != fp32_c;

  // We can also verify using IEEE 754 equality
  assert !fp32.Equal(fp32_sum, fp32_c);
}
...
method TestLiteralPrecisionBoundaries() {
  ...
  var max_safe_int: fp32 := 16777216.0;  // 2^24 for fp32

  // Verify it's the boundary - beyond this, not all integers are representable
  assert max_safe_int + 1.0 == max_safe_int;  // 2^24 + 1 rounds back to 2^24
  assert max_safe_int + 2.0 == 16777218.0;  // 2^24 + 2 is representable
  assert max_safe_int - 1.0 == 16777215.0;  // Previous int is exactly representable

  // Exact power of 2 fractions
  var tiny_exact: fp32 := 0.00006103515625;  // 2^-14
