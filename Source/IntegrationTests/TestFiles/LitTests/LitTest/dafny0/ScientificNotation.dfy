...
method BasicScientificNotation() {
  ...
  // Verify they're all equal where expected
  assert a == b == c;
  assert d == e;
  assert f == g == h;
}
...
method LargeExponents() {
  ...
  // Verify values (using approximate comparisons for very large/small numbers)
  assert a == 10000000000.0;
  assert b == 250000000.0;
  assert c == 0.0000000001;
  assert d == 0.000000025;
  
  // Verify scientific notation arithmetic
  assert a / 1.0e10 == 1.0;
  assert b / 1.0e8 == 2.5;
  assert c * 1.0e10 == 1.0;
  assert d * 1.0e8 == 2.5;
...
method EdgeCases() {
  ...
  // Zero with trailing dot
  var c := 0.;         // 0.0
  
  // Very small and large values
  var e := 1.0e-100;   // Very small positive
